...
本体系的核心定位是**生态护城河**：通过对外公开稳定、完整、可验证的接口契约，使第三方 Agent、工具链、运行时实现、操作系统发行版能够遵循同一套语义与 Agent 生态互联互通，从而把 Airymax 的语义层确立为业界事实标准。
...
- **agentrt-liunx = 微内核**：agentrt-liunx 基于 Linux 6.6 内核基线进行微内核化改造（sched_ext + eBPF + io_uring），将 VFS、网络栈、部分驱动移到用户态。
...
| 06 | [06-airymax-scheduling-standard.md](./06-airymax-scheduling-standard.md) | 调度 | SCHED_EXT、任务描述符、vtime Q16.16、1024 并发 |
...
| Agent 调度类 | SCHED_AGENT / SCHED_EXT | 见 06 标准 |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.0 | 2026-07-09 | 初始草案：6 主题标准 + 总索引，形成开放标准护城河 |
