...
YAML 模板文件
    │
    ▼
Prompt Loader 加载 (agentrt_prompt_template_load)
    │
    ▼
渲染变量替换 (agentrt_prompt_render)
    │
    ▼
...
agentrt_prompt_template_load("ecosystem/prompts/templates/system/coding_agent.yaml", &tmpl);

// 准备变量
agentrt_prompt_variable_t vars[] = {
...
// 渲染
agentrt_prompt_rendered_t *rendered = NULL;
agentrt_prompt_render(tmpl, vars, 6, &rendered);
...
// 清理
agentrt_prompt_rendered_free(rendered);
...
```c
// agentrt_prompt_loader_config_t 默认值
typedef struct {
