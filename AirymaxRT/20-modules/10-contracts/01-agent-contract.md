...
- **认知观（C维度）**: 智能体 (Agent) 契约体现了"双思考系统（GRAD 计划级批判循环）"，通过 `models` 字段明确区分 t2（主思考/生成）、t1-f（快思考/语境终裁）、t1-p（专业思考/逻辑验证）的配置，支持认知层的策略选择和资源匹配
...
3. **认知观维度** → 契约对双思考系统（GRAD 计划级批判循环）的支持，明确区分 t2（主思考/生成）、t1-f（快思考/语境终裁）、t1-p（专业思考/逻辑验证）的能力配置
...
| `models` | object | 是 | 模型配置，定义 Agent 使用的认知模型配置（双思考系统，默认 GRAD 计划级批判循环，由 grad_coordinator 协调 t2/t1-f/t1-p 三组件） | 2.4 节 |
...
定义 Agent 使用的认知模型配置，体现双思考系统（GRAD 计划级批判循环）理论。实际架构由 grad_coordinator 实现三权分立：t2（生成）/ t1-f（语境终裁）/ t1-p（逻辑验证）。
...
> **注意**: 模型名称应与 `agentrt/manager/models.yaml` 中定义的名称一致。`system1` 对应 t1-f（快思考-语境终裁），`system2` 对应 t2（主思考-生成）。GRAD 计划级批判循环（默认）由 grad_coordinator 协调 t2/t1-f/t1-p 三组件，t1-p（专业思考/逻辑验证）由系统自动配置；`!enable_grad` 时回退 TC3 文本级循环。

#### 2.4.2 双思考三权分立协同机制

根据双思考系统（GRAD 计划级批判循环）理论 [2]，三组件协同的工作方式如下:

- **t2（生成者）**: 对应 `system2`，负责构造/修正 DAG 计划（骨架 → 增量补丁），耗能高
- **t1-f（语境终裁者）**: 对应 `system1`，负责结合历史语境与目标 G 对模型 C 的判决终审，耗能低
- **t1-p（逻辑验证者）**: 由系统自动配置，负责确定性四验（E-01 因果 / E-02 死锁 / E-03 资源 / E-04 目的漂移），验证 DAG 计划的结构正确性
