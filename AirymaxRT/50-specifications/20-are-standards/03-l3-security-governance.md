...
Airymax 原生实现：`agentrt/atoms/syscall/include/agentrt_sandbox.h`（`agentrt_sandbox_*`）。原生权限枚举 `PERM_ALLOW/PERM_DENY/PERM_ASK` 与本标准 `ARE_PERM_*` 一一对应。
...
> 参考实现：`agentrt/atoms/syscall/include/syscalls.h`。原生 `agentrt_syscall_num_t` 枚举与本标准一致。
...
错误码类型为 `int32_t`（`agentrt_error_t` / `are_error_t`），成功为 0，错误为负值。分段如下：

| 段 | 范围 | 类别 | 示例宏 | 值 | 含义 |
|----|------|------|--------|----|------|
| **通用** | -1 ~ -99 | 基础错误（参数、内存、权限、超时等） | `AGENTRT_ERR_INVALID_PARAM` | -2 | 参数无效 |
| | | | `AGENTRT_ERR_OUT_OF_MEMORY` | -4 | 内存不足 |
| | | | `AGENTRT_ERR_PERMISSION_DENIED` | -10 | 权限不足 |
| | | | `AGENTRT_ERR_TIMEOUT` | -8 | 操作超时 |
| | | | `AGENTRT_ERR_NOT_IMPLEMENTED` | -30 | 未实现 |
| **系统** | -100 ~ -199 | 平台、线程、socket、文件 | `AGENTRT_ERR_SYS_NOT_INIT` | -101 | 系统未初始化 |
| | | | `AGENTRT_ERR_SYS_MUTEX` | -105 | 互斥锁错误 |
| | | | `AGENTRT_ERR_SYS_SOCKET` | -109 | socket 错误 |
| **内核** | -200 ~ -299 | IPC、任务、调度、内存 | `AGENTRT_ERR_KERN_IPC` | -201 | 内核 IPC 错误 |
| | | | `AGENTRT_ERR_KERN_TASK` | -202 | 任务错误 |
| **服务** | -300 ~ -399 | 服务状态、依赖、负载均衡 | `AGENTRT_ERR_SVC_NOT_READY` | -301 | 服务未就绪 |
| | | | `AGENTRT_ERR_SVC_BUSY` | -302 | 服务忙（背压） |
| **LLM** | -400 ~ -499 | LLM provider、限流、上下文 | `AGENTRT_ERR_LLM_NO_PROVIDER` | -401 | 无可用 provider |
| | | | `AGENTRT_ERR_LLM_RATE_LIMIT` | -403 | 限流 |
| | | | `AGENTRT_ERR_LLM_CONTEXT_LEN` | -404 | 上下文超长 |
| **执行** | -500 ~ -599 | 工具执行、沙箱、超时 | `AGENTRT_ERR_EXEC_NOT_FOUND` | -501 | 工具未找到 |
| | | | `AGENTRT_ERR_EXEC_SANDBOX` | -505 | 沙箱违规 |
| **记忆** | -600 ~ -699 | 记忆读写、查询、演化 | `AGENTRT_ERR_MEM_WRITE` | -601 | 记忆写入失败 |
| | | | `AGENTRT_ERR_MEM_FULL` | -605 | 记忆存储满 |
| **安全** | -700 ~ -799 | 权限、净化、审计、沙箱 | `AGENTRT_ERR_SEC_VIOLATION` | -701 | 安全违规 |
| | | | `AGENTRT_ERR_SEC_SANITIZE` | -702 | 净化失败 |
| | | | `AGENTRT_ERR_SEC_QUOTA` | -706 | 配额超限 |
| | | | `AGENTRT_ERR_SEC_PATH_TRAV` | -709 | 路径遍历攻击 |
| **协议** | -800 ~ -889 | 编排、协调、补偿 | `AGENTRT_ERR_COORD_PLAN_FAIL` | -801 | 编排计划失败 |
| | | | `AGENTRT_ERR_COORD_RETRY_EXCEED` | -806 | 重试超限 |
...
| # | 禁令 | 说明 |
|---|------|------|
| 1 | **禁止多套并行错误码系统** | 不得在 SDK、daemon、atoms 中定义独立的错误码枚举（如 `sdk_error_t`、`daemon_error_t`），所有代码必须使用 `agentrt_error_t` |
| 2 | **禁止本地 `#define` 重定义错误码** | 不得在 `.c`/`.h` 文件中 `#define AGENTRT_EINVAL (-1)` 之类，必须 `#include "error.h"` |
| 3 | **禁止与字面量直接比较** | 调用方应使用语义宏（如 `if (rc == AGENTRT_ERR_TIMEOUT)`），禁止 `if (rc == -8)` |
...
2. 在 `agentrt_error_str()` 函数中添加可读描述；
...
为兼容 POSIX 习惯，`error.h` 提供部分别名（如 `AGENTRT_EINVAL` → `AGENTRT_ERR_INVALID_PARAM` 的等价映射）。这些别名**仅作过渡**，新代码必须使用 `AGENTRT_ERR_*` 形式。
...
| 沙箱 | `agentrt/atoms/syscall/include/agentrt_sandbox.h`（`agentrt_sandbox_*`） |
| 系统调用号 | `agentrt/atoms/syscall/include/syscalls.h`（`agentrt_syscall_num_t`） |
| 错误码权威源 | `agentrt/commons/utils/error/include/error.h` |
| 类型定义权威源 | `agentrt/commons/include/agentrt_types.h`（`agentrt_error_t`） |
