...
**最新**: 2026-06-09
**状态**: 模板
**路径**: OpenAirymax/Docs/50-specifications/30-coding-standard/03-code-comment-template.md
---
...
#ifndef AGENTRT_MEMORY_MANAGER_H
#define AGENTRT_MEMORY_MANAGER_H
...
| 相关接口 | 可选 | 相关函数、参考文档链接 | `@see agentrt_mem_free()` |
 ...
 * @example
 * // 基本用法
 * void* buffer = agentrt_mem_alloc(1024, AGENTRT_MEM_FLAG_ZERO);
 * if (buffer == NULL) {
 *     log_error("Memory allocation failed");
 *     return AGENTRT_ERR_NO_MEMORY;
 * }
 * // 使用 buffer...
 * agentrt_mem_free(buffer);
 */
void* agentrt_mem_alloc(size_t size, uint32_t flags);
```
 ...
 */
typedef struct agentrt_task_context {
