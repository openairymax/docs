...
>
> **推荐替代方案**（优先级从高到低）：
> - `AGENTRT_STRNCPY_TERM(dst, src, dst_size)` —— Airymax 安全拷贝宏，自动保证 null 终止（定义在 `memory_compat.h`）
...
| 宏名称 | 功能 | 说明 |
|--------|------|------|
| `AGENTRT_MALLOC(size)` | 带返回值检查的 malloc | 分配失败时自动记录日志并返回 NULL |
| `AGENTRT_CALLOC(nmemb, size)` | 带返回值检查的 calloc | 零初始化内存，分配失败时自动记录日志 |
| `AGENTRT_REALLOC(ptr, size)` | 安全 realloc（使用临时指针） | 避免原指针丢失 |
| `AGENTRT_STRDUP(s)` | 带返回值检查的 strdup | 分配失败时自动记录日志 |
| `AGENTRT_STRNDUP(s, n)` | 带返回值检查的 strndup | 分配失败时自动记录日志 |
| `AGENTRT_STRNCPY_TERM(dst, src, dst_size)` | 安全字符串拷贝 | 自动保证 null 终止 |
| `AGENTRT_MEMCPY_SAFE(dst, dst_size, src, src_size)` | 安全内存拷贝 | 带边界检查，防止缓冲区溢出 |
...
int process_data(data_t* data) {
    if (!data) return AGENTRT_EINVAL;     // ✅ 空指针检查
    if (!data->buffer) return AGENTRT_EINVAL;  // ✅ 成员空指针检查
    // ... 处理逻辑
}
```
...
agentrt_thread_create(&thread, func, arg);
agentrt_mutex_lock(&mutex);
agentrt_cond_wait(&cond, &mutex);
...
当 `AGENTRT_USE_STDATOMIC` 未定义时，使用 `platform.h` 提供的跨平台原子宏。
...
agentrt_mutex_lock(&cache_lock);
agentrt_mutex_lock(&registry_lock);
// ... 操作 ...
agentrt_mutex_unlock(&registry_lock);
agentrt_mutex_unlock(&cache_lock);
```
...
- 对共享资源使用 `agentrt_mutex_t` 或原子操作保护
...
int process_input(const char* input, size_t len) {
    if (!input) return AGENTRT_EINVAL;
    if (len > MAX_INPUT_SIZE) return AGENTRT_EINVAL;  // ✅ 长度检查
    if (len == 0) return AGENTRT_EINVAL;              // ✅ 空输入检查
...
if (count > 0 && size > SIZE_MAX / count) {
    return AGENTRT_ENOMEM;  // overflow
}
size_t total = count * size;
```
...
if (strstr(user_path, "..") != NULL) {
    return AGENTRT_EINVAL;  // 路径遍历攻击
}
if (user_path[0] == '/') {
    return AGENTRT_EINVAL;  // 绝对路径
}
char safe_path[256];
...
// ... 使用密码 ...
AGENTRT_SECURE_ZERO(password, sizeof(password));  // 推荐：跨平台安全清零宏
...
> **⚠️ explicit_bzero 可移植性说明**：`explicit_bzero` 在 Linux/glibc 上可用，但在 Windows/MSVC 上不可用。Airymax 提供 `AGENTRT_SECURE_ZERO` 宏（定义在 `memory_compat.h`）作为跨平台替代方案，在 Linux 上使用 `explicit_bzero`，在 Windows 上使用 `SecureZeroMemory` 或 volatile 指针方案。所有需要安全清零的代码必须使用 `AGENTRT_SECURE_ZERO` 而非直接调用 `explicit_bzero`。
...
// 2. 使用绝对路径加载动态库
void* handle = dlopen("/usr/lib/agentrt/libplugin.so", RTLD_NOW);
if (!handle) {
    log_error("dlopen failed: %s", dlerror());
    return AGENTRT_ELOAD;
}
```
...
- `dlopen()` 必须使用绝对路径
- 程序启动时必须清理上述危险环境变量
- 使用 `RPATH`/`RUNPATH`（`-Wl,-rpath,/usr/lib/agentrt`）替代 `LD_LIBRARY_PATH`
