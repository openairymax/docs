...
**6 个 [SC] 共享头文件**（SSoT 权威清单，对齐 `docs/AirymaxRT/50-engineering-standards/120-cross-project-code-sharing.md` §2.1，两端逐字节相同）：
...
> **补充共享文件**（非 [SC] 核心头文件）：`bpf_struct_ops.h` 位于 `include/airymax/bpf_struct_ops.h`，含 struct_ops 状态机 + common_value，用于 agentrt 用户态 BPF loader 与 agentrt-linux 内核 struct_ops 框架之间的状态同步。两端共享代码但**不属于上述 6 个 [SC] 核心头文件**（详见 `120-cross-project-code-sharing.md` §2.2）。
