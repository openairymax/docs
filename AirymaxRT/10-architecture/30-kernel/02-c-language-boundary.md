...
│   │                                                                         │   │
│   │   ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌─────────┐  │   │
│   │   │CoreKern  │  │CoreLoop  │  │Cupolas   │  │Task Queue│  │Context  │  │   │
...
| **Cupolas** | 安全穹顶框架，可插拔的服务加载与生命周期管理。 |
...
| **MemoryRovol** | 四层记忆管理系统 (L1 Raw → L2 Feature → L3 Structure → L4 Pattern)，管理 token budget 与 memory budget。注意：此 L1-L4 为**记忆抽象层级**（原始/特征/结构/模式），与 [SC] `memory_types.h` 中的**缓存层级** (`AIRY_MEM_L1_HOT`/`L2_WARM`/`L3_COLD`/`L4_PMEM`) 是不同概念，不应混淆。 |
