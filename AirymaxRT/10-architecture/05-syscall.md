...
| 特性 | 说明 |
| :--- | :--- |
| **统一入口** | 单一系统调用接口 `agentrt_syscall_invoke()` |
...
1. **POSIX ABI 兼容性** [IEEE Std 1003.1]:
   ...
   Airymax 实现:
   - `agentrt_syscall_invoke()` → 类似 POSIX `syscall()`
   - `agentrt_error_t` → 类似 `errno`
   - JSON 参数 → 扩展的灵活接口
...
**乔布斯美学体现**:
- **极简主义**: 单一函数 `agentrt_syscall_invoke()` 封装所有功能
...
- **极简主义** → 单一函数 `agentrt_syscall_invoke()` 封装所有功能，体现 **简约至上 (Simplicity First)** 原则
...
|--------------|----------|----------|----------|
| 统一入口 | `agentrt_syscall_invoke()` 单一函数 | 设计美学 | 简约至上 |
 ...
 */
agentrt_error_t agentrt_syscall_invoke(
 ...
 */
agentrt_error_t sys_task_submit(
...
char* task_id;
agentrt_error_t err = sys_task_submit(task_json, &task_id);
if (err == AGENTRT_OK) {
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_task_query(
    const char* task_id,
    char** status_json);
```
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_task_cancel(
    const char* task_id);
```
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_memory_write(
...
char* record_id;
agentrt_error_t err = sys_memory_write(memory_json, &record_id);
```
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_memory_search(
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_memory_get(
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_memory_delete(
    const char* record_id);
```
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_session_create(
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_session_get(
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_session_close(
    const char* session_id);
```
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_session_list(
    char** sessions_json);
```
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_telemetry_metrics(
 ...
 * @return agentrt_error_t
 */
agentrt_error_t sys_telemetry_traces(
    const char* trace_id,
    char** traces_json);
```
...
typedef enum {
    ...
    // 通用错误
    AGENTRT_ERR_INVALID_PARAM = 1001,
    AGENTRT_ERR_OUT_OF_MEMORY = 1002,
    AGENTRT_ERR_TIMEOUT = 1003,
    
    // 任务相关错误
    AGENTRT_ERR_TASK_NOT_FOUND = 2001,
    AGENTRT_ERR_TASK_ALREADY_EXISTS = 2002,
    AGENTRT_ERR_TASK_CANCELLED = 2003,
    
    // 记忆相关错误
    AGENTRT_ERR_MEMORY_NOT_FOUND = 3001,
    AGENTRT_ERR_MEMORY_WRITE_FAILED = 3002,
    AGENTRT_ERR_MEMORY_SEARCH_FAILED = 3003,
    
    // 会话相关错误
    AGENTRT_ERR_SESSION_NOT_FOUND = 4001,
    AGENTRT_ERR_SESSION_EXPIRED = 4002,
    
    // 系统调用错误
    AGENTRT_ERR_SYSCALL_UNKNOWN = 5001,
    AGENTRT_ERR_SYSCALL_FAILED = 5002
} agentrt_error_t;
```
...
**示例**:
```c
agentrt_error_t err = sys_task_submit(params, &task_id);
if (err != AGENTRT_OK) {
...
agentrt_cognition_process(input, &plan);
...
agentrt_error_t sys_memory_write(
    ...
    agentrt_layer1_raw_write(g_layer1, 
...
| 模块 | 功能 | 完成度 | 状态 |
| :--- | :--- | :--- | :--- |
| **统一入口** | `agentrt_syscall_invoke()` | 100% | ✅ |
...
#### 问题：系统调用返回未知错误
**症状**: `AGENTRT_ERR_SYSCALL_UNKNOWN`  
**排查**:
1. 检查 syscall 名称是否正确
2. 验证参数 JSON 格式
3. 查看 syslog 中的详细错误
...
- 启用 Debug 日志：`export AGENTRT_LOG_LEVEL=DEBUG`
