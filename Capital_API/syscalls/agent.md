...
Agent 管理系统调用提供 Agent 实例的创建、销毁、调用和查询接口。Agent 遵循 Thinkdual 双思考系统（t1-f 快思考 / t2 慢思考）。所有函数使用 `agentos_sys_agent_*` 前缀。
...
| 函数 | 描述 |
|------|------|
| `agentos_sys_agent_spawn()` | 创建 Agent 实例 |
| `agentos_sys_agent_terminate()` | 销毁 Agent 实例 |
| `agentos_sys_agent_invoke()` | 调用 Agent 执行任务 |
 ...
 *
 * @ownership 调用者拥有 *out_agent_id，通过 agentos_sys_free() 释放
 * @threadsafe 是
 * @reentrant 否
 */
AGENTOS_API agentos_error_t agentos_sys_agent_spawn(const char *agent_spec,
                                                     char **out_agent_id);
```
...
agentos_error_t err = agentos_sys_agent_spawn(spec, &agent_id);
if (err == AGENTOS_OK) {
 ...
 * @reentrant 否
 */
AGENTOS_API agentos_error_t agentos_sys_agent_invoke(const char *agent_id,
                                                      const char *input,
                                                      size_t input_len,
                                                      char **out_output);
```
...
agentos_error_t err = agentos_sys_agent_invoke(
 ...
 * @return agentos_error_t (AGENTOS_OK=0 成功，负值=错误)
 *
 * @ownership 调用者拥有 *out_agent_ids，通过 agentos_sys_free() 释放
 * @threadsafe 是
 * @reentrant 否
 */
AGENTOS_API agentos_error_t agentos_sys_agent_list(char ***out_agent_ids,
                                                    size_t *out_count);
```
...
char **agent_ids = NULL;
size_t count = 0;
agentos_error_t err = agentos_sys_agent_list(&agent_ids, &count);
if (err == AGENTOS_OK) {
...
3. **系统调用初始化**：使用 Agent 相关 API 前需调用 `agentos_syscalls_init()`。
4. **Agent 安全**：Agent 操作受 [Cupolas 安全穹顶](../../AgentRT/agentos/cupolas/include/cupolas.h) 权限控制。
...
- [架构设计：Thinkdual 双思考系统](../../Capital_Architecture/thinkdual.md)
