...
直接运行 `agentos`（不带参数）将显示所有可用命令的概览。
...
# 自定义 Heapstore 数据目录
export AGENTOS_HEAPSTORE_ROOT="/data/agentrt/heapstore"
```
...
```bash
# 启动所有服务
agentos service start

# 启动指定服务
agentos service start agent-manager
```
...
```bash
# 停止所有服务
agentos service stop

# 停止指定服务
agentos service stop memory-store
```
...
# 重启指定服务
agentos service restart gateway
```
...
```bash
agentos service status task-scheduler
```
...
**环境变量：** `AGENTOS_ENDPOINT` — 决定健康检查的目标地址。
...
```bash
agentos agent register my-custom-agent
```
...
```bash
agentos agent start a1b2c3d4e5
```
...
```bash
# 提交自然语言任务
agentos task submit "Review the latest PR for security vulnerabilities"

# 提交 JSON 格式任务
agentos task submit '{"type":"code_review","target":"PR#1234","rules":["security","style"]}'
```
...
```bash
agentos task status t6e7f8g9h0
```
...
```bash
agentos task cancel t6e7f8g9h0
```
...
```bash
agentos task result t1a2b3c4d5
```
...
**环境变量：** `AGENTOS_ENDPOINT` — 决定查询的 Gateway 地址。如果 Gateway 不可达，CLI 将显示默认协议列表。
...
```bash
agentos protocol info openjiuwen
```
...
```bash
agentos protocol test <proto> [-v|--verbose]
```
...
# 详细模式
agentos protocol test openai -v
```
...
Raw response:
{
  "object": "list",
  "data": [
    {"id": "gpt-4o", "object": "model", "created": 1700000000, "owned_by": "agentos"},
    {"id": "gpt-4o-mini", "object": "model", "created": 1700000000, "owned_by": "agentos"}
  ]
}
```
...
```bash
agentos protocol detect --content <json_string> [--content-type <mime_type>]
```
...
```bash
# 检测 JSON-RPC 请求
agentos protocol detect -c '{"jsonrpc":"2.0","method":"task.submit","params":{},"id":1}'

# 检测 OpenAI 请求
agentos protocol detect -c '{"model":"gpt-4o","messages":[{"role":"user","content":"Hello"}],"temperature":0.7}'

# 通过管道从 stdin 输入
echo '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"web_search"}}' | agentos protocol detect
```
...
```bash
agentos protocol send <proto> <method> [params]
```
...
# 通过 JSON-RPC 提交任务
agentos protocol send jsonrpc task.submit '{"description":"Analyze code quality"}'

# 通过 OpenAI 协议发送聊天请求
agentos protocol send openai chat/completions '{"model":"gpt-4o","messages":[{"role":"user","content":"Hello"}]}'

# 通过 A2A 发送发现请求
agentos protocol send a2a discovery_request
```
...
**环境变量：** `AGENTOS_ENDPOINT` — 决定消息发送的目标 Gateway。
...
**环境变量：** `AGENTOS_ENDPOINT` — 决定查询统计信息的目标 Gateway。如果 Gateway 不可达，将显示零值统计。
...
```bash
agentos protocol transform <src> <tgt> [data]
```
...
agentos protocol transform jsonrpc openai

# 演示 JSON-RPC 到 MCP 的转换
agentos protocol transform jsonrpc mcp

# 带自定义数据
agentos protocol transform openai jsonrpc '{"model":"gpt-4o","messages":[{"role":"user","content":"Hi"}]}'
```
...
```bash
agentos protocol capabilities <proto>
```
...
```bash
agentos protocol capabilities openai
```
...
**环境变量：** `AGENTOS_ENDPOINT` — 影响显示的端点地址。
...
```bash
agentos config get gateway.port
agentos config get memory.max_size
agentos config get logging.level
```
...
```bash
agentos config set logging.level debug
agentos config set gateway.max_connections 1000
agentos config set memory.ttl 3600
```
...
**环境变量：** `AGENTOS_ENDPOINT` — 决定查询指标的目标 Gateway。
...
**环境变量：** `AGENTOS_ENDPOINT` — 决定查询告警的目标 Gateway。
...
→ Opening Grafana dashboard...
  URL: http://localhost:3000/d/agentos/agentos-overview
```
...
[100/100] Linking CXX executable agentos-gateway
...
```bash
agentos db migrate [--target=<version>]
```
...
| 环境变量 | 说明 |
|:---------|:-----|
| `AGENTOS_HEAPSTORE_ROOT` | 指定 Heapstore 数据目录（默认：`~/.agentrt/heapstore`） |
...
```bash
agentos db migrate-rollback --force
```
...
→ Usage: agentos db migrate-rollback --force
```
...
**环境变量：** `AGENTOS_ENDPOINT` — 影响显示的 Gateway 地址。
...
# 显示特定命令的帮助（等同于 -h 选项）
agentos service --help
agentos protocol --help
agentos db --help
```
