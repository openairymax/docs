...
AgentOS 的安全架构基于 **Cupolas 四层纵深防御模型**，每一层在 Rust SDK 中均有对应的代码实现与约束：
...
- **系统观**：Rust SDK 作为 AgentOS 生态的客户端组件，必须在不可信网络环境中运行，所有外部交互均视为敌对信道。
- **内核观**：Rust 的所有权模型与借用检查器是编译期安全保证的核心，不可通过 `unsafe` 绕过而不提供安全论证。
- **认知观**：AgentOS 的认知层（Cognition Layer, 0x2xxx 错误码）处理意图解析与 DAG 构建，认知输入必须经过 D3 输入净化管道。
...
//! # 安全边界
//!
//! 本模块包含与 C 库 `libagentos_sandbox` 的 FFI 绑定。
//!
//! ## 不变量
//! - 所有传入指针必须非空且对齐
...
//!
//! ## 安全封装
//! - `Sandbox::new()` 封装了 `unsafe fn agentos_sandbox_create()`
//! - 公共 API 中无 `unsafe fn`
pub mod ffi_bindings {
    // ...
}
...
AgentOS Rust SDK 使用 `rustls-tls` 而非 `openssl`，此约束通过 `cargo deny` 强制执行。
...
✅ **正确**：强制使用 rustls（与 AgentOS SDK 一致）
...
```toml
# deny.toml — AgentOS Rust SDK 依赖安全策略
[advisories]
...
reason = "AgentOS 安全策略要求使用 rustls-tls，禁止 openssl 依赖链"
...
错误消息不得泄露内部实现细节（堆栈跟踪、文件路径、数据库结构、密钥片段等）。AgentOS 错误体系基于 `AgentOSError` 枚举，采用错误码 + 通用消息的模式。

### 6.2 AgentOSError 的安全格式化

SDK 中的 `AgentOSError` 采用 `[错误码] 消息` 格式，错误码使用十六进制分类体系：
...
```rust
return Err(AgentOSError::http(
    &format!("Database connection failed: postgres://admin:password@db.internal:5432/agentos - {}", err)
));
```
...
return Err(AgentOSError::http("服务暂时不可用")); // 对外消息
```

### 6.3 From 实现中的信息安全

当前 SDK 的 `From<reqwest::Error>` 实现直接将底层错误消息传递到 `AgentOSError`：

```rust
impl From<reqwest::Error> for AgentOSError {
    fn from(err: reqwest::Error) -> Self {
        if err.is_timeout() {
            AgentOSError::timeout(&err.to_string())  // 可能泄露内部 URL
        }
        // ...
    }
}
...
impl From<reqwest::Error> for AgentOSError {
    fn from(err: reqwest::Error) -> Self {
        if err.is_timeout() {
            AgentOSError::timeout("请求超时")  // 不暴露内部 URL
        } else if err.is_connect() {
            AgentOSError::connection_refused("连接被拒绝")  // 不暴露主机信息
        } else if err.is_request() {
            AgentOSError::network("网络错误")  // 通用消息
        } else {
            AgentOSError::Other("内部错误")  // 最小信息
        }
    }
}
```
...
AgentOS SDK 使用 `Arc<Mutex<T>>` 实现线程安全的共享状态（如 `Telemetry`、`PluginRegistry`、`ProtocolStats`）。
...
AgentOS SDK 的所有公共接口均要求 `Send + Sync`：
...
### 8.1 Rust 所有权模型与 AgentOS 所有权语义的映射

AgentOS 的领域模型与 Rust 所有权系统天然契合：

| AgentOS 概念 | Rust 所有权映射 | 安全保证 |
|-------------|----------------|---------|
...
当与 C 库交互时（如 `libagentos_sandbox`），必须遵守以下规则：

❌ **错误**：未验证 FFI 返回值

```rust
extern "C" {
    fn agentos_sandbox_create() -> *mut SandboxHandle;
}

let handle = unsafe { agentos_sandbox_create() };
...
extern "C" {
    fn agentos_sandbox_create() -> *mut SandboxHandle;
    fn agentos_sandbox_destroy(handle: *mut SandboxHandle);
}
...
impl Sandbox {
    pub fn new() -> Result<Self, AgentOSError> {
        // SAFETY: agentos_sandbox_create 是线程安全的 C 函数，
        // 返回值可能为 NULL（创建失败），必须检查。
        let handle = unsafe { agentos_sandbox_create() };
        NonNull::new(handle)
            .map(|h| Sandbox { handle: h })
            .ok_or_else(|| AgentOSError::with_code(CODE_PERMISSION_DENIED, "沙箱创建失败"))
    }
}
...
impl Drop for Sandbox {
    fn drop(&mut self) {
        ...
        unsafe { agentos_sandbox_destroy(self.handle.as_ptr()) }
    }
}
```
...
AgentOS SDK 的 HTTP 客户端强制使用 `rustls-tls`，禁止 `openssl`/`native-tls`。
...
if response_text.len() > MAX_RESPONSE_BODY_SIZE {
    return Err(AgentOSError::invalid_response("响应体超过大小限制"));
}
```
...
// 路径参数应验证格式
fn task_url(endpoint: &str, task_id: &str) -> Result<String, AgentOSError> {
    if !task_id.starts_with("task-") || task_id.contains('/') || task_id.contains("..") {
        return Err(AgentOSError::invalid_parameter("无效的任务 ID 格式"));
    }
...
AgentOS 的安全事件日志必须包含以下字段（与 `policy.yaml` 中 `audit` 配置一致）：
