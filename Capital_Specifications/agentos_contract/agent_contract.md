...
- **内核观（K维度）**: 契约设计遵循微核心"最小信息原则"，只包含系统决策所必需的信息，保证内核纯净和服务外置
- **认知观（C维度）**: 智能体 (Agent) 契约体现了"Thinkdual 认知双思系统"，通过 `models` 字段明确区分 t2（主思考/深度推理）、t1-f（快思考/流式验证）、t1-p（专业思考/仲裁）的配置，支持认知层的策略选择和资源匹配
...
3. **认知观维度** → 契约对 Thinkdual 认知双思系统的支持，明确区分 t2（主思考）、t1-f（快思考）、t1-p（专业思考）的能力配置
...
| `models` | object | 是 | 模型配置，定义 Agent 使用的认知模型配置（Thinkdual 认知双思系统，由 triple_coordinator 协调 t2/t1-f/t1-p 三组件） | 2.4 节 |
...
定义 Agent 使用的认知模型配置，体现 Thinkdual 认知双思系统理论。实际架构由 triple_coordinator 实现 t2/t1-f/t1-p 三组件协同。
...
根据 Thinkdual 认知双思系统理论 [2]，三组件协同的工作方式如下:
...
| 引用规范 | 关系说明 |
|---------|---------||
| [架构设计原则](../../ARCHITECTURAL_PRINCIPLES.md) | 本规范是架构原则在 AgentOS 生态的具体实现，特别是微核心思想和安全内生原则 |
...
[2] AgentOS 认知层理论。../../Basic_Theories/EN_02_Cognition_Theory.md  
