...
**版本**: 0.1.1 (W18)
**状态**: 已实现
**路径**: OpenAirymax/Docs/Capital_Guides/coreloopthree_dag_integration.md
**版权**: (c) 2026 SPHARX Ltd. All Rights Reserved.

## 概述

CoreLoopThree（三层认知循环）在 0.1.1 版本中集成了 taskflow_advanced DAG 工作流引擎，提供结构化的复杂工作流编排能力。与 `agentos_loop_submit()`（自然语言单任务提交）不同，DAG API 接受结构化的 `taskflow_workflow_t` 工作流定义，支持条件分支、并行汇聚、循环迭代等 DAG 模式。
...
```
                    ...
                    │                                  │
  自然语言任务 ────►│  agentos_loop_submit()           │
                    │  (reactive/reflective planner)   │
                    │                                  │
  结构化 DAG ─────►│  agentos_loop_submit_dag()       │
...
- CoreLoopThree 在 `agentos_loop_create()` 时内部创建 `taskflow_engine_t*`，在 `agentos_loop_destroy()` 时销毁
...
### 1.1 agentos_loop_submit_dag

```c
agentos_error_t agentos_loop_submit_dag(agentos_core_loop_t *loop,
...
**返回**: `AGENTOS_SUCCESS` 成功，`AGENTOS_EINVAL` 参数错误，`AGENTOS_ENOTINIT` 引擎未初始化

### 1.2 agentos_loop_dag_register_handler

```c
agentos_error_t agentos_loop_dag_register_handler(
    agentos_core_loop_t *loop, const char *name,
    taskflow_task_handler_t handler, void *user_data);
```

注册 DAG 任务处理器。工作流节点的 `task_handler_name` 与此处的 `name` 匹配以路由执行。

### 1.3 agentos_loop_dag_wait

```c
agentos_error_t agentos_loop_dag_wait(agentos_core_loop_t *loop,
...
### 1.4 agentos_loop_dag_status

```c
agentos_error_t agentos_loop_dag_status(agentos_core_loop_t *loop,
                                         const char *execution_id,
                                         char **out_state,
                                         double *out_progress);
```
...
void dag_example(void)
{
    ...
    if (err != AGENTOS_SUCCESS || !loop) return;

    /* 注册任务处理器 */
    err = agentos_loop_dag_register_handler(loop, "process_data", my_handler, NULL);
    if (err != AGENTOS_SUCCESS) { agentos_loop_destroy(loop); return; }

    /* 构造单节点工作流 */
    taskflow_workflow_t wf;
    AGENTOS_MEMSET(&wf, 0, sizeof(wf));
    AGENTOS_STRNCPY_TERM(wf.id, "wf_example_001", sizeof(wf.id));
    AGENTOS_STRNCPY_TERM(wf.name, "Example Workflow", sizeof(wf.name));
    ...
    wf.nodes[0].task_handler_name = AGENTOS_STRDUP("process_data");
    wf.initial_node_id = AGENTOS_STRDUP("node_start");

    /* 提交 DAG */
    char *exec_id = NULL;
    err = agentos_loop_submit_dag(loop, &wf, "{\"input\":\"data\"}", &exec_id);
    ...
    /* 等待完成并获取结果 */
    char *result = NULL;
    err = agentos_loop_dag_wait(loop, exec_id, 0, &result);
    if (err == AGENTOS_SUCCESS) {
        printf("DAG result: %s\n", result);
        AGENTOS_FREE(result);
    }
    ...
    char *state = NULL;
    double progress = 0.0;
    agentos_loop_dag_status(loop, exec_id, &state, &progress);
    printf("State: %s, Progress: %.0f%%\n", state ? state : "?", progress * 100);
    AGENTOS_FREE(state);

    /* 清理 */
    AGENTOS_FREE(exec_id);
    agentos_loop_destroy(loop);  /* 内部销毁 taskflow_engine，释放 workflow 资源 */
}
```
...
| 场景 | 谁释放 workflow 资源 |
|------|---------------------|
| `submit_dag()` 成功 | `agentos_loop_destroy()`（通过 engine destroy） |
...
| 参数 | 释放者 |
|------|--------|
| `out_execution_id` | 调用者 `AGENTOS_FREE()` |
| `out_result_json` | 调用者 `AGENTOS_FREE()` |
| `out_state` | 调用者 `AGENTOS_FREE()` |

### 4.3 handler 输出所有权

handler 的 `output_json` 参数：handler 内部分配（如 `AGENTOS_STRDUP`），引擎在 `taskflow_engine_start()` 中将所有权转移到 `execution.output_json`，最终由 `taskflow_engine_destroy()` 释放。**handler 不应自行释放 output_json**。
...
`taskflow_engine_start()` 对单节点工作流（`initial_node_id` 指向的节点）会**同步执行** handler，执行完毕后状态直接置为 COMPLETED/FAILED。此时 `agentos_loop_dag_wait()` 会检测到已完成状态，直接返回结果，不调用 `run_to_completion()`。
...
完整的集成测试位于 `AgentRT/agentos/atoms/coreloopthree/tests/unit/test_dag_integration.c`，包含 4 个测试用例：
...
| `test_dag_param_validation` | 参数验证：NULL 参数返回 AGENTOS_EINVAL |
...
| 文件 | 说明 |
|------|------|
| `agentos/atoms/coreloopthree/include/loop.h` | DAG API 声明（4 个函数） |
| `agentos/atoms/coreloopthree/src/loop.c` | DAG API 实现 + engine 生命周期 |
| `agentos/atoms/taskflow/include/taskflow_advanced.h` | taskflow 类型定义 |
| `agentos/atoms/taskflow/src/taskflow_advanced.c` | taskflow 引擎实现 |
| `agentos/atoms/coreloopthree/tests/unit/test_dag_integration.c` | W18.3 集成测试 |
| `agentos/atoms/coreloopthree/CMakeLists.txt` | 编译配置（链接 agentos_taskflow） |
| `agentos/daemon/common/CMakeLists.txt` | svc_common include taskflow 路径 |
