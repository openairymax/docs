...
- **节点 C（基础设施层）**: monit_d, observe_d, info_d, notify_d, hook_d, plugin_d
...
| 扩展 daemon | 扩展 | hook_d | 9301 | 2 |
| 扩展 daemon | 扩展 | plugin_d | 9302 | 2 |
...
Layer 1（核心服务 - 依赖 Layer 0）:
  ├── sched_d     (调度守护进程 → 依赖 observe_d)
  └── channel_d   (通道守护进程)

Layer 2（Agent 服务 - 依赖 Layer 1）:
  ├── llm_d       (LLM 守护进程 → 依赖 sched_d)
  ├── tool_d      (工具守护进程)
  ├── hook_d      (钩子守护进程)
  └── plugin_d    (插件守护进程)
...
Layer 4（网关 - 依赖 Layer 2 和 Layer 3）:
  └── gateway_d   (网关守护进程 → 依赖 llm_d, tool_d, market_d)
```
...
| llm_d, tool_d, plugin_d | 30 | Unix Socket |
| hook_d | 20 | Unix Socket |
