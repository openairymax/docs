...
> **v1.0.1 Capability Folding 实现注记**：上述 `cte.c` / `cspace.c` / `mdb.c` / `cnode_ops.c` 保留了 seL4 风格 CSpace+MDB 派生树的设计语义（ES-SEL4-05\~09 设计溯源），但 v1.0.1 Capability Folding 后工程实现已**简化**为：
>
...
v1.0.1 Capability Folding 决策对 seL4 风格 CSpace/MDB/radix tree 进行了工程简化，**保留设计语义、简化实现路径**。以下为 v1.1 实现概览（SSoT：[03-capability-model.md](../110-security/03-capability-model.md) + [07-ipc-fastpath.md](../30-interfaces/07-ipc-fastpath.md)）：
...
> **v1.0.1 Capability Folding 说明**：原 8 个 seL4 风格 IPC 原语 syscall（`airy_sys_send`/`recv`/`nbsend`/`nbrecv`/`reply_recv`/`yield`/`reply`/`notify`）在 v1.1 全部移除，IPC 数据传递完全由 io\_uring `IORING_OP_URING_CMD` 承载（见下方数据面表）。能力校验折叠到数据面 fastpath C-S9 内联 Badge 校验，`airy_sys_call` 语义收窄为 sec_d 专属管理入口。seL4 借鉴映射保留作设计溯源参考。
