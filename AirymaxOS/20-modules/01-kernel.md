...
**\[SC] 对接**：`struct airy_ipc_msg_hdr`（128B）定义在 `include/uapi/linux/airymax/ipc.h`，包含 magic（0x41524531 'ARE1'）、opcode、flags、trace\_id、timestamp\_ns、src\_task、dst\_task、payload\_len、reserved\[84] 等字段。与 SSoT 逐字节一致。
...
| 消息头结构     | `struct airy_ipc_msg_hdr`                            | 9 字段              | magic(\_\_u32) + opcode(\_\_u16) + flags(\_\_u16) + trace\_id(\_\_u64) + timestamp\_ns(\_\_u64) + src\_task(\_\_u64) + dst\_task(\_\_u64) + payload\_len(\_\_u32) + reserved[84](__u8) |
...
AirymaxOS 用户态 **12 daemon** 在内核侧的切入点（daemon 命名后缀 `_d`，例外 `macro_superv` / `config_daemon`）：
...
| `macro_superv` | 宏观监管（系统级看门狗 + daemon 重启） | systemd watchdog + panic 回调 | systemd watchdog |
...
| `config_daemon` | 统一配置管理（运行时参数热更新） | sysfs + procfs 接口 | sysfs + procfs |
...
**说明**：
- logger_d / audit_d / gateway_d / macro_superv / vfs_d / net_d / dev_d / config_daemon 共 8 个 daemon **不直接占用 syscall 槽位**，通过 io_uring 数据面 / char dev / eBPF ringbuf / sysfs 等通道与内核协作。
