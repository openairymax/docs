...
> **核心约束**：IRON-9 v2 同源且部分代码共享——与 agentrt 用户态 atoms/corekern 通过 \[SC] 共享契约层（6 头文件）+ \[SS] 语义同源层协作，\[IND] 内核态 方案 C-Prime（用户态调度器）/io\_uring/eBPF/Rust 实现独立。**Syscall 架构**：12 核心 + 12 预留 = 24 槽位（Capability Invocation 统一入口 + IPC 数据面/控制面分离）。**命名统一**：所有 \[SC] 共享标识符使用 `AIRY_*` 前缀，C 函数使用 `airy_sys_*` 前缀。\
> **子仓编号**：01\
...
3. **Agent 感知调度（方案 C-Prime）\[SS]**：采用方案 C-Prime（SCHED\_DEADLINE/SCHED\_FIFO/EEVDF + seL4 MCS 映射，标准 6.6 用户态调度器，替代主线 6.12+ 的 sched\_ext）实现 AIRY\_SCHED\_AGENT 策略，允许用户态定义调度策略。任务描述符与优先级语义 \[SC] 与 agentrt 共享。
...
7. **\[SC] 共享契约层物理宿主 \[IND]**：IRON-9 v2 全部 10 个 \[SC] 共享契约头文件物理宿主在 `kernel/include/airymax/`，其他子仓通过 `-I` 引用确保单一数据源。
...
| \[SC] 共享契约层 | —         | 10 个头文件（IRON-9 v2）     | 单一物理宿主，禁止重复定义         |
...
| \[SC] 共享契约层                  | —        | 10 个头文件           | IRON-9 v2 单一数据源 |
...
| 数据流          | 内核切入点                                                                  | 同源标注   |
| ------------ | ---------------------------------------------------------------------- | ------ |
| 调度数据流        | 方案 C-Prime 用户态调度器 + AIRY\_SCHED\_AGENT + sub-scheduler                      | \[SS]  |
...
| ---------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| **\[SC] 共享契约层**  | 完全共享代码             | 10 个头文件（详见 §10.1）：syscalls.h / ipc.h / sched.h / security\_types.h / memory\_types.h / cognition\_types.h                                                                                                                                                                                | `kernel/include/airymax/`（单一物理宿主） |
| **\[SS] 语义同源层**  | 高层 API 语义同源，签名独立演进 | 方案 C-Prime 25+ 调度回调语义、io\_uring ring 创建/提交/完成/注册、MSG\_RING 跨环消息、SQPOLL 状态机、DEFER\_TASKRUN、eBPF struct\_ops 注册、bpf\_prog 生命周期、bpf\_link 生命周期、bpf\_map\_ops 回调表、ringbuf reserve/submit、kfunc 注册模式 等 30+ 项                                                                                 | 各自独立实现                            |
| **\[IND] 完全独立层** | 完全独立               | 用户态调度类注册、用户态调度器 enable/disable、调度上下文追踪、fallback 回退机制、cgroup 集成、core-sched 集成、debug dump；io-wq 工作队列、NO\_MMAP、REGISTERED\_FD\_ONLY、URING\_CMD；JIT 后端、trampoline 本机码生成、verifier 实现、CFS 钩子（不引入）、cfi\_stubs、KABI\_RESERVE（不采用）；VFS/网络/驱动用户态化改造；Rust 驱动框架 | 各自独立仓库                            |
...
| 调度模型           | MicroCoreRT 实时调度                | AIRY\_SCHED\_AGENT（方案 C-Prime + seL4 MCS 映射）               | \[SS]  |
...
- 保留 agentrt 的"实时性"目标（通过方案 C-Prime 的 sub-scheduler 在 cgroup 上附加实时策略）\[SS]。
- 保留 agentrt 的"消息传递"通信范式（升级为 io\_uring 零拷贝实现 + 8 IPC 原语控制面）\[SS]。
- 任务描述符与优先级语义 \[SC] 共享，确保两端调度语义一致。
- IPC 消息头与操作码 \[SC] 共享，确保两端通信协议一致。
- struct\_ops 状态机 \[SC] 共享，使 agentrt 用户态可解析用户态调度器在线状态（方案 C-Prime 复用状态枚举）。
...
│   ├── user-sched-agent/   # AIRY_SCHED_AGENT 用户态调度器策略（方案 C-Prime）[SS]
...
### 3.1 patches/user-sched-agent \[SS]

存放 AIRY\_SCHED\_AGENT 策略的用户态调度器源码。任务描述符与优先级 \[SC] 与 agentrt 共享：

- `sched_agent.c`：核心调度器逻辑（基于方案 C-Prime，SCHED\_FIFO/SCHED\_DEADLINE + cgroup cpuset）\[SS]。
...
- `zerocopy.c`：基于 MSG\_ZEROCOPY 与 IORING_OP_URING_CMD 语义映射的零拷贝路径 \[SS]。
...
- **Suspend/Resume**：通过方案 C-Prime 的 `agent_runnable`/`agent_stopping` 回调控制。
...
> **设计依据**：seL4 调度器设计（ES-SEL4-04 零策略内核）+ Linux 6.6 方案 C-Prime（用户态调度器）。代码证据 `src/kernel/thread.c`（752 行）、`src/object/tcb.c:32-50`（checkPrio）、`src/object/schedcontext.c`（444 行）。
...
| ------------ | ------------- | ----------------------------- |
| SchedContext | 常量带宽管理        | 方案 C-Prime sub-scheduler 带宽配额 |
| refill 机制    | refill + 带宽管理 | cgroup cpu.max/cpu.weight     |
| 常量带宽 vs 突发   | 参数化           | AIRY\_SCHED\_AGENT 策略可配             |
...
### 9.1 方案 C-Prime（用户态调度器）

- **方案 C-Prime**：基于 Linux 6.6 标准内核原生 SCHED\_DEADLINE/SCHED\_FIFO/EEVDF + seL4 MCS 映射的用户态调度器（替代主线 6.12+ 的 sched\_ext）。标准 Linux 6.6 主线不包含 sched\_ext（6.12 才合入主线），agentrt-linux 采用方案 C-Prime。
- **AIRY\_SCHED\_AGENT 策略**：用户态调度器策略名（AIRY\_SCHED\_AGENT），通过 SCHED\_FIFO/SCHED\_DEADLINE + cgroup cpuset 实现 Agent 感知调度，**禁止**定义 `SCHED_AGENT` 内核调度类宏（避免与内核调度类编号冲突）。
...
- **方案 C-Prime**：基于标准 6.6 用户态调度器（SCHED\_FIFO/SCHED\_DEADLINE/EEVDF），禁止定义 SCHED\_AGENT 内核调度类宏。
...
| 权重范围          | `AIRY_WEIGHT_MIN/MAX`   | 1 / 10000       | 兼容方案 C-Prime 权重模型                                                      |
| 默认时间片         | `AIRY_SLICE_DFL_MS`     | 20              | 20ms                                                                    |
| 方案 C-Prime 约束 | —                       | 标准 6.6 原生 | 禁止定义 SCHED\_AGENT 内核调度类宏                                                     |
...
| ---------- | ---------------------------- | ----------------------------------- | --------------------------------------- |
| 调度语义       | MicroCoreRT 用户态调度器           | 方案 C-Prime 用户态调度器（SCHED\_FIFO/SCHED\_DEADLINE）    | `sched.h`：任务描述符 + vtime + 优先级           |
...
| -------------- | ------------------- | ------------------ | ----------- |
| 调度类            | 用户态协程/线程池           | 方案 C-Prime 用户态调度器注册 | 跨平台 vs 内核专属 |
...
2. **agentrt-linux 子系统初始化**：在 `initcall` 阶段初始化 io\_uring IPC、eBPF struct\_ops、capability 系统、方案 C-Prime 用户态调度器。
3. **Agent 服务启动**：用户态 init 进程启动 Agent 服务管理器（services），注册 AIRY\_SCHED\_AGENT 用户态调度器策略。
4. **Capability 引导**：根 Agent 获得初始 CSpace，开始 capability 分发。
5. **进入 Agent 调度**：AIRY\_SCHED\_AGENT 策略接管 Agent 任务调度。
