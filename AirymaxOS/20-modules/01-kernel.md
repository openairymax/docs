...
> **核心约束**：IRON-9 v3 同源且部分代码共享——与 agentrt 用户态 atoms/corekern 通过 \[SC] 共享契约层（10 头文件）+ \[SS] 语义同源层 + \[DSL] 降级生存层协作，\[IND] 内核态 sched_tac（基于 Linux 6.6 原生调度类组合 SCHED_DEADLINE/SCHED_FIFO/EEVDF + cgroup cpuset 隔离 + seL4 MCS 语义映射的策略框架，非用户态调度器，非 sched_ext）/io\_uring/eBPF/Rust 实现独立。**Syscall 架构**（v1.1 Capability Folding 后）：4 核心 + 20 预留 = 24 槽位（1 Capability Invocation + 3 控制原语，IPC 数据面完全由 io_uring 承载）。**命名统一**：所有 \[SC] 共享标识符使用 `AIRY_*` 前缀，C 函数使用 `airy_sys_*` 前缀。\
> **子仓编号**：01\
