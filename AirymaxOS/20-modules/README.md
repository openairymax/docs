...
> **核心约束**：IRON-9 v2 同源且部分代码共享——[SC] 共享契约层 10 个头文件（syscalls.h/memory_types.h/security_types.h/cognition_types.h/sched.h/ipc.h）落地于 include/airymax/
...
|---|------|------|---------|--------------|---------|---------|
| 1 | kernel | 极境内核 | Linux 6.6 + 微内核化改造 | atoms/corekern（MicroCoreRT） | EEVDF + 方案 C-Prime + io_uring | https://atomgit.com/openairymax/kernel.git |
...
|----------------|------------------|---------|
| patches/sched-cprime-agent | MicroCoreRT 实时调度 | Agent 感知调度策略 |
...
12 daemons 统一归属 `services/daemons/`，由 USV（macro_superv）统一生命周期管理：

| agentrt-linux 模块 | agentrt 同源模块 | 同源语义 |
|----------------|------------------|---------|
| daemons/macro_superv | macro supervisor | 主监管守护进程（USV） |
| daemons/logger_daemon | logger daemon | 日志消费守护进程（ULPS） |
| daemons/config_daemon | config daemon | 配置管理守护进程（UCF） |
| daemons/gateway_d | gateway daemon | 网关守护进程 |
| daemons/sched_d | sched daemon | 调度守护进程 |
| daemons/vfs_d | vfs daemon | VFS 用户态服务守护进程 |
| daemons/net_d | net daemon | 网络策略守护进程 |
| daemons/mem_d | memory daemon | 记忆管理守护进程 |
| daemons/cogn_d | cognition daemon | 认知调度守护进程 |
| daemons/sec_d | security daemon | 安全策略守护进程 |
| daemons/audit_d | audit daemon | 审计守护进程 |
| daemons/dev_d | device daemon | 设备驱动守护进程 |
...
| 子仓 | 设计文档 | 核心内容 |
|------|---------|---------|
| kernel | [01-kernel.md](01-kernel.md) | Linux 6.6 + 方案 C-Prime + io_uring + Rust 实验性 + 微内核化 |
...
| services → cognition | IPC | io_uring | cogn_d/sched_d 与认知循环协作（128B 消息头） |
