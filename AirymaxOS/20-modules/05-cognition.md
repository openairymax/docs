...
| 6  | `macro_d`   | macro\_superv 监管 cogn_d 生命周期（崩溃重启、资源限制违规冻结）                   | `AIRY_IPC_OP_FREEZE` / systemd lifecycle                          |
...
| 11 | `config_d`  | cogn_d 启动配置（kthread 优先级、令牌桶容量）由 config\_daemon 管理              | io\_uring IPC → config\_daemon 配置读写                               |
