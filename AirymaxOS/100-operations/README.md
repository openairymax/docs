...
agentrt 的 12 个 daemons（macro_superv/logger_daemon/config_daemon/gateway_d/sched_d/vfs_d/net_d/mem_d/cogn_d/sec_d/audit_d/dev_d）在 agentrt-linux 中以 systemd 服务运行：
- 进程二进制名与 agentrt 同源（`macro_superv`/`logger_daemon`/`config_daemon` 及 `*_d` 后缀守护进程）
