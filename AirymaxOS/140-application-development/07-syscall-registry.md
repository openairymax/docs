...
|------|--------|--------|------|--------|---------|------|
| 512 | `AIRY_SYS_TASK_SUBMIT` | `airy_sys_task_submit` | 提交 Agent 任务到 AIRY_SCHED_AGENT 调度器 | 2 | 0.1.1 | 已分配 |
...
| 编号 | 宏定义 | C 符号 | 功能 | 参数数 | 引入版本 | 状态 |
|------|--------|--------|------|--------|---------|------|
| 572 | `AIRY_SYS_SCHED_SET_POLICY` | `airy_sys_sched_set_policy` | 设置方案 C-Prime 调度策略 | 2 | 0.1.1 | 已分配 |
...
| 可插拔策略 | K-4 | 调度策略（AIRY_SCHED_AGENT）、遗忘策略（艾宾浩斯/线性/基于访问）、净化策略（正则/类型/语义）均可运行时替换 |
...
/* 任务操作 512-515 */
#define AIRY_SYS_TASK_SUBMIT     512  /* 提交 Agent 任务到 AIRY_SCHED_AGENT */
...
#define AIRY_SYS_SCHED_SET_POLICY    572  /* 设置方案 C-Prime 调度策略 */
