...
| 版本对齐 | agentrt-linux 基线为 vanilla 6.6.149；OLK-6.6 为 6.6.0 分叉 + openEuler 补丁维护（SUBLEVEL=0），硬件适配补丁经 LAYER 正交叠加，不依赖 `6.6.0-148` 段对齐 |
...
[第 6 天] 代码审查
   ...
   └── 双签通过
        ...
        v
[第 7 天] 发布
   ├── 发布 agentrt-linux-6.6.149-airymax.1
...
- CVSS 评分: 8.5（高危）
- 影响版本: agentrt-linux < 6.6.149-airymax.1
- 修复版本: agentrt-linux 6.6.149-airymax.1
...
## 修复方案
升级至 agentrt-linux 6.6.149-airymax.1 或更高版本
...
echo "=== ABI 兼容性测试 ==="
./test_abi_compat --old-version=6.6.147 --new-version=6.6.149
...
示例：
6.6.149-airymax.1          # 基于 Linux 6.6.149 的第 1 次发布
6.6.149-airymax.1.1         # 第 1 次发布的第 1 个补丁
6.6.149-airymax.2           # 基于 Linux 6.6.149 的第 2 次发布
```
...
| 版本段 | 含义 | 变更触发 |
|--------|------|----------|
| `6.6.149` | 上游 Linux stable 版本 | 上游 minor merge |
...
```bash
# 验证回滚机制
./test_rollback --from=6.6.149-airymax.1 --to=6.6.147-airymax.5
# 预期：成功回滚，数据完整
```
