...
**验收标准**: 4 模块设计完善 + 微内核化改造策略（AIRY_SCHED_AGENT / AgentsIPC / capability / MGLRU / CoreLoopThree kthread）明确 + 与 agentrt 同源 API 映射表完成。
...
|---------|-----------|--------------|---------|
| 调度 | MicroCoreRT 调度语义 | AIRY_SCHED_AGENT 策略 | 端到端调度延迟测试 |
