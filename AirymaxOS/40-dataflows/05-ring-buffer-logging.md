...
> **文档定位**：ULPS（统一日志与打印系统）零拷贝 Ring Buffer 日志数据流的唯一权威设计\
> **文档版本**：v1.0\
> **最后更新**：2026-07-17\
...
- **前置知识**：理解 [10-unify-design.md](../10-architecture/10-unify-design.md) ULPS 模块、[09-sc-log-types-contract.md](../30-interfaces/09-sc-log-types-contract.md) 128B 记录格式
- **预计阅读时间**：30 分钟
...
ULPS 采用 `alloc_pages(GFP_KERNEL)` 分配连续物理页 + `remap_pfn_range` 映射到用户态：
...
ULPS 的 Ring Buffer 采用 **reserve/commit 两阶段写入**模型，避免生产者持锁期间被中断/抢占导致死锁：
...
ULPS 的 Fastpath 严格三步，禁止任何格式化操作：
...
`eventfd_signal` 仅增加计数，不唤醒进程（进程由 `io_uring` 的 poll 机制异步唤醒）。Fastpath 路径全程无上下文切换，这是 ULPS 性能优于传统 `printk` 的关键之一。
...
ULPS 严格遵循"内核 Fastpath 禁止格式化"原则。所有耗时的操作（格式化、过滤、落盘）全部下沉到用户态 Logger Daemon：
...
| 过滤 | `services/daemons/logger_daemon/main.c` | 按 level/facility 过滤，受 UCF sysctl 控制 |
...
格式化规则通过 `/etc/airymax/logger_daemon.conf` 配置（受 UCF 管理），支持自定义字段顺序与格式。

### 5.4 Logger Daemon 崩溃恢复

Logger Daemon 是用户态进程，可能崩溃。ULPS 的设计确保 Logger Daemon 崩溃不丢日志：
...
Panic 时 ULPS 回退到 Linux 6.6 原生 `printk_safe` 路径（NMI-safe buffer）：
...
ULPS 的延迟 SLO（受 [170-performance/05-agent-latency-slo.md](../170-performance/05-agent-latency-slo.md) 约束）：
