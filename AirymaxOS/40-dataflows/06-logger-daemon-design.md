...
# Logger Daemon 详细设计
> **文档定位**：ULPS（统一日志与打印系统）用户态日志消费守护进程的唯一权威详细设计\
> **文档版本**：v1.0\
> **最后更新**：2026-07-17\
> **上级文档**：[Airymax Unify Design 总纲](../10-architecture/10-unify-design.md) §5\
> **设计依据**：[15-comprehensive-correction-plan.md](../../docs-closed/agentrt-linux/00-reviews/_review_v2.2/15-comprehensive-correction-plan.md) §4.2.2（ULPS 设计）+ [05-ring-buffer-logging.md](05-ring-buffer-logging.md) §5
...
>
> 技术选型声明：日志内存采用 **alloc_pages(GFP_KERNEL) + mmap**（**不使用 DMA 一致性内存**，x86_64 默认缓存一致）。整体遵循 Unify Design：方案 C-Prime（SCHED_DEADLINE/SCHED_FIFO/EEVDF + seL4 MCS 映射，不使用 sched_ext）+ 纯 C LSM（不使用 BPF LSM）+ IORING_OP_URING_CMD + registered buffer + mmap（不使用 page flipping）。[SC] 共享契约头文件的物理宿主为 `kernel/include/airymax/`。
...
## §1 架构定位：ULPS 模块的用户态日志消费守护进程

### 1.1 在 ULPS 中的角色

Logger Daemon 是 ULPS 模块的用户态消费者，承担所有耗时操作：格式化、过滤、落盘、压缩。内核 Fastpath 仅完成 `reserve → memcpy 128B raw binary → commit` 三步（~50-100ns），**禁止任何格式化操作**。Logger Daemon 与内核 Fastpath 通过 mmap 共享 Ring Buffer + eventfd 通知解耦：
...
| 配置文件 | `/etc/airymax/logger_daemon.conf` | UCF 管理的 JSON 配置 |

### 1.3 与其他模块的协作

| 协作方 | 关系 | 接口 |
|--------|------|------|
| ULPS 内核模块 | 生产者 → 消费者 | mmap Ring Buffer + eventfd |
| UCF 配置管理 | 被管理方 | sysctl/JSON 热重载 |
| USV Macro-Supervisor | 被监管方 | 心跳上报 |
| UIPF io_uring | 复用通道 | io_uring eventfd poll |
...
Logger Daemon 支持三级过滤，由 UCF 配置管理（`/etc/airymax/logger_daemon.conf`）：
...
过滤策略由 UCF 管理，支持 sysctl/JSON 热重载。sysctl 写入触发 `config_reload` 事件，Logger Daemon 通过 RCU 指针原子切换过滤器，无需重启：
...
Logger Daemon 是用户态进程，可能因 OOM、段错误、信号等原因崩溃。ULPS 的设计确保崩溃不丢日志：
...
- [20-modules/11-unified-config.md](../20-modules/11-unified-config.md) —— UCF 配置管理（过滤策略热重载）
