...
> **文档定位**：agentrt-linux（AirymaxOS）调度数据流的详细设计，刻画 EEVDF + 方案 C-Prime + AIRY_SCHED_AGENT 三位一体调度体系\
> **文档版本**：0.1.1\
> **最后更新**：2026-07-07\
> **上级文档**：[agentrt-linux 设计文档](README.md)\
> **核心约束**：IRON-9 v2 同源且部分代码共享——共享契约文件 sched.h（用户态调度器策略契约 + `airy_task_desc` + `airy_sched_ops` 回调表）落地于 include/airymax/，[SS] EEVDF + 方案 C-Prime + AIRY_SCHED_AGENT 策略语义同源，[IND] 用户态调度器 daemon 实现 + cgroup cpuset 隔离配置独立
...
2. **方案 C-Prime**（FR-001, FR-002）：标准 6.6 用户态调度器（替代 sched_ext，6.6 主线不含 sched_ext），基于 SCHED_DEADLINE/SCHED_FIFO/EEVDF + seL4 MCS 映射的可插拔调度方案。agentrt-linux 通过方案 C-Prime 实现 AIRY_SCHED_AGENT 策略，调度策略以用户态 .so 形式加载，支持运行时替换（FR-050）。
3. **AIRY_SCHED_AGENT 策略**（FR-002）：agentrt-linux 自研用户态调度器策略名（非内核调度类编号），专为 Agent 工作负载优化。优先级范围 0-139，支持抢占式调度与策略可插拔。

**核心特征**：

- **EEVDF 算法**：基于 eligible virtual deadline 选择下一个任务，相比 CFS 的红黑树，延迟分布更紧凑（P99 < 100ms，P99.9 < 200ms，NFR-P-001）。
- **方案 C-Prime 用户态策略可插拔**：调度策略以用户态 .so 形式通过 dlopen 加载，运行时替换不影响运行任务（FR-050）。
- **cgroup v2 cpuset + sched_setscheduler**（FR-006，Linux 6.6 原生）：方案 C-Prime 用户态调度器通过 `sched_setscheduler()` 设置原生调度类（SCHED_FIFO/SCHED_DEADLINE/SCHED_NORMAL），通过 cgroup v2 cpuset 实现 CPU 隔离。
- **cgroup cpuset 队列模型**：方案 C-Prime 的核心队列抽象，每个 cgroup 拥有独立 cpuset，支持 per-CPU 本地队列与全局共享队列，提供 FIFO 与 vtime 优先队列两种模式。
...
下图为 Agent 任务从提交到执行的完整调度数据流，包含 AIRY_SCHED_AGENT 策略、EEVDF 选择、策略可插拔、cgroup cpuset 派发：

```mermaid
flowchart TD
    A[Agent 任务提交] --> B[syscall: airy_sys_call<br/>capability invocation]
    B --> C[AIRY_SCHED_AGENT 策略<br/>方案 C-Prime]
...
- 步骤 4-9 在用户态方案 C-Prime 调度器中完成（FR-002），通过 `sched_setscheduler()` 设置原生调度类（FR-006）。
...
cgroup cpuset 队列是方案 C-Prime 的核心队列抽象，是用户态调度器与调度核心之间的解耦层。每个 cgroup 拥有独立 cpuset，可创建用户队列，提供 FIFO 与 vtime 优先队列两种模式。

### 5.1 队列类型与语义

| 队列类型 | 标识 | 语义 | agentrt-linux AIRY_SCHED_AGENT 使用 |
|---------|------|------|---------------------------|
...
```c
/**
 * @brief cgroup cpuset 调度队列（方案 C-Prime 用户态队列，替代内核态 dispatch_q）
 * @since 1.0.1
...
## 6. AIRY_SCHED_AGENT 调度策略

AIRY_SCHED_AGENT 是 agentrt-linux 基于方案 C-Prime 框架实现的**自研用户态调度策略**（FR-002），专为 Agent 工作负载优化。同源 agentrt atoms/corekern 的 MicroCoreRT 调度器。

> **设计约束**（BAN-361 + Linux 6.6 内核基线 验证）：标准 Linux 6.6 主线**不包含** sched_ext（6.12 才合入主线），agentrt-linux 不向前移植 sched_ext，而是采用方案 C-Prime（SCHED_DEADLINE/SCHED_FIFO/EEVDF + seL4 MCS 映射）。**禁止定义 `AIRY_SCHED_AGENT` 内核调度类编号宏**（避免与内核调度类编号冲突）。`AIRY_SCHED_AGENT` 仅作为方案 C-Prime 框架下的**用户态策略名称**（policy name），复用 Linux 6.6 原生 SCHED_DEADLINE/SCHED_FIFO/EEVDF 调度类。

### 6.1 原生调度类与 AIRY_SCHED_AGENT 策略定义
 ...
 * @note 禁止定义 AIRY_SCHED_AGENT 策略编号宏（BAN-361 + IRON-1）
...
/**
 * @brief AIRY_SCHED_AGENT 策略名称（方案 C-Prime 用户态策略标识，非调度类编号）
 * @since 1.0.1
 * @note AIRY_SCHED_AGENT 是用户态策略名称，调度类编号复用内核原生 SCHED_DEADLINE/SCHED_FIFO
 */
#define AIRY_SCHED_AGENT_NAME  "airy_sched_agent"  /* 用户态策略名称，非调度类编号 */
...
/* AIRY_SLICE_DFL 默认时间片 20ms（方案 C-Prime 默认值） */
#define AIRY_SLICE_DFL  (20 * 1000000ULL)
```

### 6.2 AIRY_SCHED_AGENT 用户态调度器程序

AIRY_SCHED_AGENT 用户态调度器实现 `struct airy_sched_ops` 的关键回调（非 BPF struct_ops，纯用户态函数表），定义调度策略。以下为 `default_eevdf` 策略的参考实现：

```c
/**
 * @brief AIRY_SCHED_AGENT default_eevdf 策略用户态调度器程序
 * @since 1.0.1
 * @see 方案 C-Prime 用户态调度器
 * @note IRON-9 v2 [SS] 语义同源层：语义同源 airy_sched_ops，实现独立
...
/**
 * @brief enable: 任务进入 AIRY_SCHED_AGENT 时初始化 vtime
 */
void agent_enable(struct task_struct *p) {
    struct airy_task_state *st = &task_table[p->pid % MAX_AGENTS];
    st->vtime = vtime_now;
    /* 方案 C-Prime：通过 sched_setscheduler 设置原生调度类 */
...
### 6.4 AIRY_SCHED_AGENT 优先级到权重转换

```c
/**
 * @brief 优先级到权重映射（方案 C-Prime set_weight）
 * @since 1.0.1
...
## 7. 方案 C-Prime 用户态调度器状态机

方案 C-Prime 用户态调度器通过两级状态机管理调度器与任务的生命周期，确保用户态调度器加载/卸载/失败时的系统完整性。
...
stateDiagram-v2
    ...
    INIT --> READY: 完全初始化
    READY --> ENABLED: 进入 AIRY_SCHED_AGENT
    ENABLED --> READY: 退出 AIRY_SCHED_AGENT
    INIT --> [*]: init_task 取消
    READY --> [*]: task exit
...
`atomic_long_t ops_state` 让调度核心随时可靠判定任务是否可被派发，从而放宽对用户态调度器的要求（用户态可尝试派发任意状态的任务，调度核心拒绝无效派发）。agentrt-linux AIRY_SCHED_AGENT 同源采用此机制实现**软可靠性**。

---

## 8. fallback 自动回退机制

agentrt-linux AIRY_SCHED_AGENT 的核心安全网，确保用户态调度器失败时系统完整性不破坏。
...
agentrt-linux AIRY_SCHED_AGENT 增强方案 C-Prime 的单一 30s 超时为分级超时：
...
调度策略可插拔（FR-050）是 agentrt-linux 的核心特征，基于方案 C-Prime 实现运行时策略替换，不影响运行任务。同源 agentrt coreloopthree 的策略可插拔机制。
...
方案 C-Prime 通过 `kf_mask` 限制不同回调上下文可调用的 API 集合，避免用户态调度器在不安全上下文中调用需锁的 API：

| 掩码 | 上下文 | agentrt-linux AIRY_SCHED_AGENT 使用 |
|------|-------|---------------------------|
...
## 11. 方案 C-Prime 与 EEVDF 协同机制

### 11.1 调度类优先级

方案 C-Prime 用户态调度器位于 EEVDF（SCHED_NORMAL）之上、原生实时类（SCHED_FIFO/SCHED_DEADLINE）之下。当用户态调度器加载且未设置部分切换标志时，所有 SCHED_NORMAL/BATCH/IDLE 任务走方案 C-Prime 策略，EEVDF 退化为 fallback。

### 11.2 策略标志位

| 标志位 | 值 | 语义 | agentrt-linux AIRY_SCHED_AGENT 使用 |
|--------|-----|------|---------------------------|
...
**关键决策**：agentrt-linux AIRY_SCHED_AGENT 默认**不设置** `AIRY_OPS_SWITCH_PARTIAL`，即策略加载后所有 SCHED_NORMAL/SCHED_BATCH/SCHED_IDLE 任务均走 AIRY_SCHED_AGENT，最大化 Agent 调度优先级。

### 11.3 fallback 回退机制

方案 C-Prime 内置 `AIRY_Q_BYPASS` 队列，当用户态调度器卸载或失败时：

1. 所有任务迁入 `AIRY_Q_BYPASS`（FIFO）
2. CFS（EEVDF）重新接管调度
3. 触发 `AIRY_OPS_DISABLING` → `AIRY_OPS_DISABLED` 状态迁移

agentrt-linux AIRY_SCHED_AGENT 同源实现 fallback 机制，确保用户态调度器失败时回退到 EEVDF 默认调度。
...
| `agent_priority_threshold` | `/sys/fs/cgroup/agentrt/sched/` | 100 | AIRY_SCHED_AGENT 实时优先级阈值 |
| `agent_default_timeslice` | `/sys/fs/cgroup/agentrt/sched/` | 100ms | AIRY_SCHED_AGENT 默认时间片 |
| `max_queue_depth` | `/sys/fs/cgroup/agentrt/sched/` | 1024 | 方案 C-Prime 队列深度 |
...
perf/ftrace 命令详见 §12.2 性能验证方法。方案 C-Prime 用户态调度器追踪：`cat /sys/kernel/debug/tracing/airy_sched/trace`。
...
```json
{
  ...
  "context": { "task_id": "task_xyz789", "priority": 50,
    "sched_class": "AIRY_SCHED_AGENT", "policy": "agent_realtime",
...
| 策略名称 | `AIRY_SCHED_AGENT_NAME "airy_sched_agent"` | `include/airymax/sched.h` |
...
agentrt-linux AIRY_SCHED_AGENT 用户态调度器实现以下回调（签名与 `struct airy_sched_ops` 同源，但实现独立于 agentrt）：
...
agentrt 用户态 MicroCoreRT 调度语义与 agentrt-linux AIRY_SCHED_AGENT 策略、AgentsIPC 与内核原生 IPC 均共享 [SC] 契约层，运行于 agentrt-linux 上天然契合无需适配层：

| 维度 | agentrt（用户态） | agentrt-linux（内核态） | 同源标注 |
|------|------------------|----------------------|----------|
| 优先级范围 | 0-139 | AIRY_SCHED_AGENT 0-139 | [SC] |
...
**同源红利**：agentrt 作为普通用户态应用（libc/POSIX）在 agentrt-linux 上天然更稳健；可选 `sched_setscheduler(p, SCHED_FIFO, &param)` 享受内核原生调度优先级（方案 C-Prime 复用 Linux 6.6 原生 SCHED_FIFO/SCHED_DEADLINE），无需任何转换/适配层。
...
|---|--------|------------------|---------------------|-----------|
| 1 | [SC] 调度类 | 不涉及（用户态无调度类编号） | 复用 Linux 6.6 原生 SCHED_DEADLINE/SCHED_FIFO/EEVDF | PASS 完全共享（复用内核原生调度类，禁止 AIRY_SCHED_AGENT 宏） |
...
| 10 | [SS] 调度语义 | MicroCoreRT 调度语义一致 | AIRY_SCHED_AGENT 调度语义同源 | PASS 语义同源 |
...
**结论**：15 项检查全部 PASS。调度数据流与 agentrt atoms/corekern 在 [SC] 共享契约层完全一致（调度类/magic/描述符/时间片/权重/vtime 7 项），[SS] 语义同源层语义等价，[IND] 独立层正确分离（用户态专属机制不污染 agentrt）。agentrt 设计无需修改，保持跨平台用户态；agentrt-linux 在 [IND] 独立层正确引入方案 C-Prime 用户态调度器路径，遵循 IRON-9 v2 同源且部分代码共享原则。
...
- [内核模块设计](../20-modules/01-kernel.md)：EEVDF + 方案 C-Prime + AIRY_SCHED_AGENT
...
| 版本 | 日期 | 变更内容 | 变更人 |
|---|---|---|---|
| 0.1.1 | 2026-07-06 | 初始版本，定义 EEVDF + 方案 C-Prime + AIRY_SCHED_AGENT 调度数据流 | 工程规范委员会 |
| 0.1.1 | 2026-07-07 | 增强：补充 cgroup cpuset 队列模型、状态机、fallback、kf_mask、IRON-9 v2 三层共享模型落地；修复 agentos→agentrt 命名 | 工程规范委员会 |
| 0.1.1 | 2026-07-07 | 新增 Copyright 头 + §16 agentrt 一致性检查（15 项全 PASS） | 工程规范委员会 |
| 0.1.1 | 2026-07-17 | 修正：基于 Linux 6.6 内核基线采用方案 C-Prime（SCHED_DEADLINE/SCHED_FIFO/EEVDF + seL4 MCS 映射）替代 sched_ext，SCHED_AGENT → AIRY_SCHED_AGENT，BPF 代码重写为用户态 C 代码，DSQ → cgroup cpuset 队列模型 | 工程规范委员会 |
