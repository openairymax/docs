...
agentrt-linux 需要在内核层面感知 Token 消耗与资源消耗的关联，通过 AIRY_SCHED_AGENT 用户态调度器策略优化能效。本方案参考 Linux 6.6 perf 功耗剖析（`tools/perf/`）与 seL4 MCS（Mixed-Criticality System）时间隔离思想，建立 Token 能效工程体系。
...
```mermaid
graph TD
    A[Agent 推理请求] --> B[AIRY_SCHED_AGENT 调度]
    B --> C{推理阶段?}
