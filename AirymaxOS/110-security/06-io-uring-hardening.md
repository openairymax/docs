...
# io_uring 安全加固设计
> **文档定位**：UIPF（统一进程间通信体系）io_uring 在 Agent 环境安全加固的唯一权威设计\
> **文档版本**：v1.0\
> **最后更新**：2026-07-17\
> **上级文档**：[Airymax Unify Design 总纲](../10-architecture/10-unify-design.md) §8\
> **设计依据**：[15-comprehensive-correction-plan.md](../../docs-closed/agentrt-linux/00-reviews/_review_v2.2/15-comprehensive-correction-plan.md) §4.2.5（UIPF 设计）+ §6.2.1 C-02（page flipping 修正）
...
- **目标读者**：安全模块开发者、UIPF 模块开发者、内核开发者
- **前置知识**：理解 [10-unify-design.md](../10-architecture/10-unify-design.md) §8 UIPF 模块、[03-capability-model.md](03-capability-model.md) Capability 模型、Linux 6.6 io_uring 机制与 `io_uring_disabled` sysctl
- **预计阅读时间**：25 分钟
...
io_uring 安全加固是 UIPF 模块的安全基础（详见 [10-unify-design.md](../10-architecture/10-unify-design.md) §8）。UIPF 的 IPC 语义映射依赖 io_uring 的 `IORING_OP_URING_CMD`，加固确保此通道不被滥用：
...
| opcode | 是否允许 | 用途 |
|--------|---------|------|
| `IORING_OP_URING_CMD` | ✅ 允许 | IPC 语义映射（UIPF 核心） |
...
## §6 与 UIPF 模块的关系：io_uring 安全是 UIPF 的安全基础

### 6.1 在 UIPF 中的定位

io_uring 安全加固是 UIPF 模块的安全基础，确保 IPC 通道不被滥用：
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v1.0 | 2026-07-17 | 初始版本：io_uring 安全加固设计；io_uring_disabled sysctl（0/1/2 三级控制）；opcode 白名单（仅 IORING_OP_URING_CMD + NOP）；纯 C LSM 双重校验（opcode + Capability，不使用 BPF LSM）；registered buffer 安全（注册前 Capability 校验 + 地址 + 大小限制）；与 UIPF/纯 C LSM/seL4 Capability 模型关系 |
