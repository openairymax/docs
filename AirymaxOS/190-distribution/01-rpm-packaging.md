...
# agentrt-liunx（AirymaxOS）RPM 打包设计详细规范

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）发行版 RPM 包结构与打包规范详细设计
...
agentrt-liunx（AirymaxOS）RPM 打包设计旨在为发行版构建提供可重现、可追溯、可验证的包管理标准。本设计聚焦三大目标：
...
本设计严格遵守 agentrt-liunx 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-liunx（OS 发行版）称为**微内核**（micro-kernel）。所有外部 Linux 发行版统一表述为"主流 Linux 发行版"，禁止使用 openEuler/Euler 字样。RPM 包与 agentrt 之间属于 IRON-9 v2 [IND] 完全独立层。
...
agentrt-liunx 发行版顶层分为"内核"、"用户态"、"开发"三大组：
...
```spec
# airymaxos-kernel.spec —— agentrt-liunx 内核包 spec 文件
# SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0
# Copyright (c) 2025-2026 SPHARX Ltd. All Rights Reserved.
...
Summary:        agentrt-liunx (AirymaxOS) Linux 6.6 kernel with sched_ext
...
%description
The agentrt-liunx (AirymaxOS) kernel package contains the Linux 6.6 kernel
...
%package core
Summary:        agentrt-liunx kernel core (vmlinuz + initramfs)
Requires:       airymaxos-kernel-modules = %{version}-%{release}

%package modules
Summary:        agentrt-liunx kernel modules

%package headers
Summary:        agentrt-liunx kernel headers for development
BuildArch:      noarch

%package devel
Summary:        agentrt-liunx kernel development package
Requires:       airymaxos-kernel-headers = %{version}-%{release}

%package tools
Summary:        agentrt-liunx kernel tools (perf, bpftool, etc.)
...
- Initial agentrt-liunx (AirymaxOS) 1.0.1 release
...
Version:        %{agentrt_version}
Release:        %{agentrt_release}%{?dist}
Summary:        agentrt-liunx LLM Inference Daemon (llm_d)
...
%description
The llm_d daemon provides LLM inference proxy for agentrt-liunx (AirymaxOS).
...
```ini
# /etc/yum.repos.d/airymaxos.repo —— agentrt-liunx dnf 仓库配置

[airymaxos-base]
name=agentrt-liunx (AirymaxOS) BaseOS $releasever - $basearch
...
[airymaxos-appstream]
name=agentrt-liunx (AirymaxOS) AppStream $releasever - $basearch
...
[airymaxos-updates]
name=agentrt-liunx (AirymaxOS) Updates $releasever - $basearch
...
[airymaxos-testing]
name=agentrt-liunx (AirymaxOS) Testing $releasever - $basearch
...
发行版管理错误码纳入 agentrt-liunx 统一错误码体系：
...
agentrt-liunx 的 RPM 包与 agentrt 完全独立（[IND] 完全独立层），各自演进。
...
> **文档结束** | agentrt-liunx（AirymaxOS）RPM 打包设计详细规范 | 1.0.1 开发版本
