...
# agentrt-liunx（AirymaxOS）编码规范详细设计

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）字符编码规范与 UTF-8 处理详细设计
...
agentrt-liunx（AirymaxOS）编码规范旨在为内核态与用户态代码统一字符编码处理标准。本设计聚焦三大目标：
...
本设计严格遵守 agentrt-liunx 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-liunx（OS 发行版）称为**微内核**（micro-kernel）。编码契约在 agentrt 与 agentrt-liunx 之间属于 IRON-9 v2 [SC] 共享契约层。
...
agentrt-liunx 内核态严格禁止使用 `wchar_t` 类型，原因有三：
...
agentrt-liunx 内核态严格禁止 `strcpy`、`strcat`、`sprintf`、`gets` 等不安全函数，统一使用 `strscpy`、`strscpy_pad`、`scnprintf`：
...
/* int bad_example(char *dst) { strcpy(dst, "agentrt-liunx"); } */
...
int good_example(char *dst, size_t dst_size)
{
	...
	ret = strscpy(dst, "agentrt-liunx", dst_size);
	if (ret < 0)
		return -E2BIG;
	return 0;
}
...
agentrt-liunx 的 Token 计数遵循"按 Unicode 码点计数"原则：
...
> **文档结束** | agentrt-liunx（AirymaxOS）编码规范详细设计 | 1.0.1 开发版本
