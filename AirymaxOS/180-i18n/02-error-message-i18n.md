...
# agentrt-liunx（AirymaxOS）错误消息国际化设计

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）错误消息国际化工程设计文档，覆盖错误码（agentrt_error_t）与本地化消息分离设计、错误码注册表 SSoT、多语言消息映射
...
agentrt-liunx（AirymaxOS）作为面向全球开发者和用户的智能体操作系统发行版，其错误消息体系是开发者体验与可追溯性的关键。本文档定义错误消息国际化工程设计，目标是：
...
4. **跨端错误码同源**：agentrt-liunx 内核态与 agentrt 用户态运行时错误码完全一致，实现跨端错误可追溯
...
本文档严格遵循术语规范：agentrt（用户态运行时）= 微核心（micro-core）；agentrt-liunx（OS 发行版）= 微内核（micro-kernel）。错误码定义共享于 [SC] 共享契约层（两端完全共享代码），错误消息呈现独立于两端实现。文中不使用 openEuler/Euler 字样，错误码体系对照一律表述为"主流 Linux 发行版"。
...
agentrt-liunx 采用双错误码体系，分别适用于不同场景：
...
agentrt-liunx 提供 `airymaxos-error-gen` 工具，从 `include/airymax/error.h` 自动生成各语言的错误码绑定：
...
```po
# agentrt-liunx error messages English translation
# Copyright (c) 2025-2026 SPHARX Ltd.
#
msgid ""
msgstr ""
...
错误码定义位于 IRON-9 v2 [SC] 共享契约层，agentrt-liunx 内核态与 agentrt 用户态运行时完全共享同一份 `include/airymax/error.h`：

```
agentrt 仓库:
  include/airymax/error.h  ←──┐
                               │ 同一份代码
agentrt-liunx 仓库:           │ （symlink 或 git submodule）
  include/airymax/error.h  ←──┘
```
...
# 3. agentrt-liunx 仓库 CI 校验
cd /path/to/agentrt-liunx && make ci-check-error-codes
...
| `test_error_cross_validate` | 跨端校验 | agentrt 与 agentrt-liunx 错误码一致 |
...
int agentrt_error_registry_init(struct error_registry_cfg *cfg)
{
    ...
    pr_info("agentrt-liunx: 错误码注册表初始化完成\n");
    return 0;
...
> **文档结束** | agentrt-liunx（AirymaxOS）错误消息国际化设计 v1.0.1
