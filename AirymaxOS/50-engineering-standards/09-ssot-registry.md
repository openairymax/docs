...
| **IRON-9**  | agentrt 和 AirymaxOS 同源且部分代码共享（IRON-9 v3）：四层共享模型 \[SC]+\[SS]+\[IND]+\[DSL]，agentrt 保持跨平台用户态纯净，AirymaxOS 是完整发行版 | §38.1  |
