...
当前版本为 **v1.0.1**，定义文档体系结构与三层架构边界，核心接口规范由本目录 L1/L2/L3 文档完整描述。
...
- 12 个 [SC] 头文件（10 核心 + 2 补充 `syscall.h`/`bpf_struct_ops.h`；核心：syscalls.h / memory_types.h / security_types.h / cognition_types.h / sched.h / ipc.h / error.h / log_types.h / uapi_compat.h / lsm_types.h）

共享方式：12 个 [SC] 头文件唯一物理宿主于 `kernel/include/uapi/linux/airymax/`，其他子仓通过 `-I../kernel/include` 引用，禁止物理副本（OS-IRON-014）。任何修改必须首先在物理宿主提交，两端逐字节相同。
