...
| sched_tac（SCHED_DEADLINE/SCHED_FIFO/EEVDF） | agentrt-linux（AirymaxOS）增强 | 基于主线 6.6 调度策略 | GPL-2.0-only | SPDXRef-sched-tac |
...
| 子仓 / 组件 | 错误码空间 | 说明 |
|-------------|------------|------|
| kernel | 内核态负整数 (-1 ~ -300) | 内核模块、系统调用返回值 |
...
| **编码空间** | 紧凑（-1 ~ -300，共 300 个有效值，对齐 `error.h` SSoT） | 宽松（0x00010000 ~ 0xFFFF0000，共 65535 个有效值） |
| **扩展性** | 有限（每个分段 20 个值，10 个子空间 + 预留段） | 充裕（每个分段 4096 个值） |
...
各子空间的具体宏定义、典型场景、C-S 检查链对齐关系、[DSL] 降级语义等内容，**不在本项目管理规范中重复定义**（避免与 `error.h` SSoT 产生二次漂移），统一查询以下权威源：

| 查询内容 | 权威源 |
|---------|--------|
| 宏定义（C 代码） | [SC] `include/uapi/linux/airymax/error.h` |
| 子空间分配 + IPC/Capability 详解 + C-S 检查链 | [30-interfaces/08-sc-error-contract.md](../../30-interfaces/08-sc-error-contract.md) |
| 跨项目前缀规范（`AIRY_E*` / `AIRY_ERROR_*`） | [50-engineering-standards/120-cross-project-code-sharing.md](../120-cross-project-code-sharing.md) §2.5 |
| Fault 码（0x1001+，不可恢复） | [30-interfaces/08-sc-error-contract.md](../../30-interfaces/08-sc-error-contract.md) §4 |
| [DSL] 降级块映射 | [10-architecture/11-degraded-survival-layer.md](../../10-architecture/11-degraded-survival-layer.md) §2.2 |
...
##### 3.1.1 [SC] 共享契约层 — 错误码 SSoT 定义

错误码定义统一收纳在 [SC] `include/uapi/linux/airymax/error.h` 中（10 子空间，[-1,-300]），agentrt 和 agentrt-linux 使用相同的错误码数值和语义。`error.h` 是错误码的唯一 SSoT 物理宿主，其余 9 个 [SC] 头文件（`syscalls.h` / `memory_types.h` / `security_types.h` / `cognition_types.h` / `sched.h` / `ipc.h` / `log_types.h` / `uapi_compat.h` / `lsm_types.h`）通过 `#include <uapi/linux/airymax/error.h>` 引用错误码，不重复定义：

| [SC] 头文件 | 错误码分类 | 说明 |
|------------|-----------|------|
| `error.h` | **全部分类（SSoT）** | 唯一定义源：POSIX / IPC / Capability / Config / A-ULS / MemoryRoVol / Cognition / Log / Object / Syscall 共 10 子空间（[-1,-300]） |
| `ipc.h` | IPC 错误码（引用） | 通过 `#include error.h` 引用 `AIRY_EIPC_*` |
| `security_types.h` | capability 错误码（引用） | 通过 `#include error.h` 引用 `AIRY_ECAP_*` / `AIRY_ESEC_*` |
| `memory_types.h` | 记忆错误码（引用） | 通过 `#include error.h` 引用 `AIRY_EMEM_*` |
| `sched.h` | 调度错误码（引用） | 通过 `#include error.h` 引用 `AIRY_ESCHED_*` / `AIRY_ELIFECYCLE_*` |
| `cognition_types.h` | 认知错误码（引用） | 通过 `#include error.h` 引用 `AIRY_ECOG_*` |

##### 3.1.2 [SS] 语义同源层 — 语义一致，数值独立

以下错误码语义与 agentrt 一致，但具体数值可能不同（agentrt-linux 侧数值对齐 `error.h` SSoT）：

| 语义域 | agentrt 实现 | agentrt-linux 实现 | 说明 |
|--------|--------------|---------------------|------|
| 调度错误 | atoms/corekern 调度错误码 | kernel 调度错误码（A-ULS 码 `[-121,-140]`，对齐 `error.h`） | 调度语义一致，数值独立 |
| 安全错误 | cupolas 安全错误码 | security 安全错误码（Capability 码 `[-71,-100]`，对齐 `error.h`） | 安全模型一致，数值独立 |
| 认知错误 | coreloopthree 认知错误码 | cognition 认知错误码（Cognition 码 `[-161,-180]`，对齐 `error.h`） | 认知循环一致，数值独立 |
...
/* === IPC 错误码（对齐 [SC] error.h [-41,-70]，禁止 E_IPC_* 废弃前缀） === */
...
/* 更多错误码定义见 [SC] error.h SSoT 与 08-sc-error-contract.md */
```
...
| 分段 | 维护主体 | 审查主体 |
|------|----------|----------|
| POSIX 码（[-1,-40]） | 全部子仓（复用 Linux errno） | 工程规范委员会 |
| IPC 码（[-41,-70]） | kernel + services 治理组 | Kernel + Base Systems 治理组 |
| Capability 码（[-71,-100]） | security 治理组 | Security 治理组 |
| Config 码（[-101,-120]） | system 治理组 | Base Systems 治理组 |
| A-ULS 码（[-121,-140]） | kernel 治理组 | Kernel 治理组 |
| MemoryRoVol 码（[-141,-160]） | memory 治理组 | Memory 治理组 |
| Cognition 码（[-161,-180]） | cognition 治理组 | Cognition 治理组 |
| Log 码（[-181,-200]） | services 治理组 | Base Systems 治理组 |
| Object 码（[-201,-220]） | kernel 治理组 | Kernel 治理组 |
| Syscall 码（[-221,-240]） | kernel 治理组 | Kernel 治理组 |
| 预留（[-241,-300]） | — | 工程规范委员会 |
...
|----------|----------|----------|----------|------|
| agentrt | kernel, services, cognition | 运行时平台 | agentrt-linux >= 1.0.1 | 稳定 |
| agentrt SDK | services (API), system (CLI) | API 绑定 | agentrt-linux >= 1.0.1 | 稳定 |
| agentctl | system (agentctl socket) | IPC 协议 | agentrt-linux >= 1.0.1 | 稳定 |
| Airymax Desktop | cloudnative, cognition | 完整 OS 镜像 | agentrt-linux >= 1.0.1 | Beta |
| Airymax Edge | kernel (minimal), memory | 裁剪镜像 | agentrt-linux >= 1.0.1 | 稳定 |

#### 4.2 生态下游

| 下游项目 | 依赖组件 | 依赖类型 | 兼容版本 | 状态 |
|----------|----------|----------|----------|------|
| 主流 Linux 发行版标准兼容层 | kernel (ABI) | ABI 兼容 | agentrt-linux >= 1.0.1 | 已规划 |
| K8s Airymax Node | cloudnative (CRI) | CRI 插件 | agentrt-linux >= 1.0.1 | Alpha |
| SPHARX AI Platform | cognition | 推理运行时 | agentrt-linux >= 1.0.1 | 稳定 |
...
> **IRON-7 合规说明**：agentrt-linux 唯一基线版本为 v1.0.1，禁止 v1.0/v1.1/v1.1.1/v1.2/v2.0 等中间过渡版本。后续版本规划遵循 IRON-7 铁律另行确定。
