...
错误码定义内嵌在 `include/airymax/` 的 6 个 [SC] 共享契约层头文件中，agentrt 和 agentrt-liunx 使用相同的错误码数值和语义：
...
通用错误码（`AGENTRT_E*`）属于 [SS] 语义同源层，agentrt 和 agentrt-liunx 使用相同的错误码前缀和语义，但具体数值实现各自独立（详见 3.1.2 节）。
