...
| **OS-IRON-008** | 共享契约层 10 个 [SC] 核心头文件（bpf_struct_ops.h + error.h）双向 CI 验证——[SC] 头文件变更必须通过 agentrt + AirymaxOS CI 同时通过 | E-6 / E-8 | 120 跨项目代码共享 |
...
| **OS-IRON-014** | [SC] 共享契约层 10 个 [SC] 核心头文件（bpf_struct_ops.h + error.h）单一数据源（禁止物理副本）——10 个 [SC] 头文件物理宿主在 kernel/include/airymax/，其他子仓通过 -I 引用 | E-7 | 120 跨项目代码共享 |
...
| OS-IRON-003 | 可插拔策略审查（方案 C-Prime / LSM 钩子 / ops 注入） | 策略硬编码入内核被拒绝 |
...
| `include/airymax/sched.h` | 方案 C-Prime 调度类约束（使用 SCHED_DEADLINE/SCHED_FIFO/EEVDF 原生调度类，禁止定义 SCHED_AGENT 宏）+ 任务描述符（magic 0x41475453 'AGTS'）+ vtime 类型与衰减公式 + 优先级范围 0-139 + AIRY_SLICE_DFL（20ms） | [SC] |
...
| `sched_ext（误标为 Linux 6.6 原生能力）` | `方案 C-Prime（SCHED_DEADLINE/SCHED_FIFO/EEVDF + seL4 MCS 映射）` | 标准 Linux 6.6 主线不包含 sched_ext（6.12 才合入主线）；agentrt-linux 采用方案 C-Prime 替代 sched_ext |
...
| 微内核化改造策略（方案 C-Prime + seL4 MCS 映射 + eBPF + io_uring） | ~~真正的微内核~~ | 是改造，不是原生微内核 |
...
| 5 | IRON-9 v2 三层模型落地 | 验证通过 | 10 个 [SC] 共享头文件已定义 |
