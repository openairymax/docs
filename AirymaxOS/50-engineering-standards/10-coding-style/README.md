...
| **[SS] 语义同源层** | 高层 API 语义同源，签名独立演进 | 共享命名约定（`agentrt_` 前缀）、错误码体系、注释风格；但 C 内核态用 kernel-doc，Rust 用户态用 rustdoc | SDK 层 `agentrt_ipc_send()` 签名同源（同一份源码两端编译），其他层语义同源 |
...
|----------|-------------------|-------------|---------|
| `agentrt_ipc_send()` | io_uring + 内核固定 OP | 用户态消息队列 | 签名一致（SDK 层），协议兼容 |
| `agentrt_task_submit()` | kthread + sched_ext | pthread + 用户态调度器 | 语义等价，调度模型同源 |
| `agentrt_capability_check()` | LSM hook + kernel capability | 用户态 ACL | 语义等价，判定逻辑同源 |
