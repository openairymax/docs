...
# agentrt-liunx（AirymaxOS）日志格式契约
...
本文档定义了 agentrt-liunx（AirymaxOS）的日志格式契约。日志是所有可观测性能力的基石，也是 agentrt-liunx（AirymaxOS）五维正交可观测性架构中"可观测性维度"的核心载体。本契约覆盖内核态日志、用户态日志、审计日志三个层面，规定了日志的格式、字段、传输管道、安全策略与保留策略，确保 agentrt-liunx 的日志体系在分布式部署、多租户隔离、安全审计等场景下保持一致性。

agentrt-liunx 的日志系统遵循以下五维正交原则：
...
| 术语 | 含义 |
|------|------|
| agentrt-liunx | agentrt-liunx（AirymaxOS）的操作系统内核与运行时环境 |
...
agentrt-liunx（AirymaxOS）定义了六个标准日志等级，与 syslog 标准兼容，同时扩展了 OpenTelemetry 的 SeverityNumber 映射：
...
所有 agentrt-liunx 日志均以 JSON 格式输出，确保与 OpenTelemetry 收集器（otelcol）原生兼容。每条日志为一行 JSON（NDJSON），不含换行符嵌套。
...
所有时间戳均通过 `clock_gettime(CLOCK_REALTIME, &ts)` 获取，确保跨节点的事件时序一致性。在 agentrt-liunx 内核中，日志时间戳在内核日志环形缓冲区写入时一次性生成，后续传输不再修改，避免时间戳失真。
...
agentrt-liunx 内核日志通过 `log_write()` 和 `log_write_va()` 两个核心函数输出。这两个函数是 agentrt-liunx 对 Linux printk 子系统的扩展封装，实现了以下增强：
...
内核日志通过 `/dev/kmsg` 或 `printk` 环形缓冲区转发至 systemd-journald。agentrt-liunx 在 journald 侧配置了专门的转发规则：
...
审计日志是 agentrt-liunx 安全体系的核心组成部分，用于记录所有安全相关操作（认证、授权、配置变更、数据访问）。审计日志与普通日志隔离，具有以下特征：
...
以下 Mermaid 图展示了 agentrt-liunx 日志从产生到最终存储的完整数据流：
...
以下 Mermaid 图展示了 agentrt-liunx 的日志等级动态升级（escalation）流程：
...
agentrt-liunx 日志系统在写入路径上集成了敏感数据掩码引擎，在 `log_write_va()` 内部完成格式化之后、写入环形缓冲区之前执行掩码。掩码规则如下：
...
agentrt-liunx 日志轮转由 `agentrt-log-rotator` 守护进程管理，基于 `logrotate` 扩展：
...
agentrt-liunx 日志系统采用全异步架构，确保日志 I/O 不会阻塞业务逻辑：
...
agentrt-liunx 的日志系统通过 OTLP 协议（gRPC）与 OpenTelemetry Collector 集成。日志在 agentrt-log-collector 中转换为 OpenTelemetry LogRecord 格式后发送。
...
- [agentrt-liunx（AirymaxOS）IRON-9 v2 通道规范](iron9_channel_contract.md)
- [agentrt-liunx（AirymaxOS）安全策略规范](security_policy.md)
- [agentrt-liunx（AirymaxOS）OpenTelemetry 集成指南](otel_integration.md)
