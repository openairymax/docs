...
| AIRY_IPC_F_ZEROCOPY | 0x0004 | [IND] 扩展 | 使用 io_uring 零拷贝路径（registered buffers + page flipping） | 发送方必须注册 buffer |
...
| AIRY_IPC_F_URGENT | 0x0040 | [IND] 扩展 | 紧急消息，优先投递 | 仅 SCHED_EXT 实时类可用 |
...
agentrt-linux 系统调用错误码对齐 `include/airymax/error.h`（[SC] SSoT，权威定义见 `180-i18n/03-error-message-i18n.md` §2.2），与 agentrt 同源且部分代码共享（IRON-9 v2 [SC] 层）。错误码统一使用 `AIRY_E*` 前缀，负值返回：
