...
# agentrt-liunx（AirymaxOS）维护者制度与治理

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）工程标准规范第 7 篇——治理层规范
...
> **替代关系**: 本文档替代 Linux `Documentation/process/management-style.rst`、`submitting-patches.rst`（DCO 章节）、`contribution-maturity-model.rst`、`6.Followthrough.rst` 在 agentrt-liunx 场景下的适用
...
agentrt-liunx 是一个跨内核态与用户态、跨 8 个子仓、跨多语言栈（C/Rust/Python/TypeScript）的智能体操作系统发行版。当一个项目的代码量、贡献者数量、子系统边界同时增长时，"谁有权接受补丁""谁对某段代码的稳定性负责""补丁如何从作者流到主线"这三个问题若没有明确答案，项目会迅速退化为"无人负责的代码海洋"。
...
这套机制的关键数据：Linus 直接处理的补丁长期维持在约 1.3% 的比例——这不是因为 Linus 偷懒，而是因为信任链已经把 98.7% 的补丁在到达他之前过滤、审查、稳定过一遍。agentrt-liunx 直接继承这套经过验证的治理模型。

### 1.3 agentrt-liunx 维护者制度的三大支柱

agentrt-liunx 维护者制度建立在三个不可拆分的支柱之上：
...
agentrt-liunx 工程标准与 agentrt 工程标准是 **IRON-9 v2 同源且部分代码共享** 的关系：

- **同源**：两端共享五维正交 24 原则作为顶层设计哲学；共享 17 类规则编号体系骨架（IRON/BAN/STD/ACC）；共享 MicroCoreRT 微核心运行时理念（agentrt-liunx 在内核态实现 MicroCoreRT，agentrt 在用户态实现同源语义）。
- **独立**：agentrt-liunx 承担内核态严肃性责任——内核 ABI 稳定性、内核内部 API 不稳定性、补丁生命周期、维护者层级制度是 agentrt 不涉及的领域；agentrt-liunx 的治理规则（OS-IRON-5~10、OS-STD-GOV-*）独立于 agentrt 的 IRON 规则。
- **互操作**：agentrt 在 agentrt-liunx 上运行时，两端的治理边界清晰——agentrt 遵循用户态运行时治理（其自身的 IRON-1~10），agentrt-liunx 遵循内核发行版治理（本文档的 OS-IRON-5~10）。两端通过同源语义（AgentsIPC 128B 消息头）实现无适配层互操作，但治理上互不越权。

> **OS-IRON-9**：agentrt-liunx 治理与 agentrt 治理 IRON-9 v2 同源且部分代码共享——共享原则骨架，独立行使内核态/用户态各自的治理权责，互不否决对方的工程决策。
...
agentrt-liunx 沿用 Linux 6.6 内核基线 `MAINTAINERS` 文件的 14 字段，并按现代 SCM（git + GitHub）场景做适配：
...
### 2.2 agentrt-liunx 新增字段

在 14 字段之外，agentrt-liunx 新增 3 个字段以承载同源治理与代码所有权信息：
...
| **V:** | 版本兼容性声明（标注该子系统对 agentrt-liunx 主版本/内核基线的要求） | V: agentrt-liunx 1.x / Linux 6.6 |

`O:` 字段是 `M:`/`R:` 的 CODEOWNERS 投影——`M:`/`R:` 给人看，`O:` 给机器看。两者必须保持一致，CI 会校验。`H:` 字段是 IRON-9 v2 同源且部分代码共享原则的落地：每个 agentrt-liunx 子系统须声明其 agentrt 同源模块，以便在两端的语义漂移检测中自动对齐。

### 2.3 Status 状态定义

agentrt-liunx 在 Linux 原有 5 种状态基础上，新增 4 种面向发行版生命周期的状态：
...
| **Experimental** | 实验性，API/ABI 可能剧烈变动 | agentrt-liunx 0.1.x 阶段全部子仓 |
...
graph TD
    ...
    C -->|pull 请求| D["agentrt-liunx 总维护者<br/>(BDFL 角色)<br/>最终合并权"]
...
2. **链可任意长但实务不超过 2-3 级**：理论上信任链可无限延伸，但超过 3 级会显著增加补丁延迟与合并冲突风险。agentrt-liunx 实务上限为 3 级（贡献者 → 子系统维护者 → 总维护者）。
3. **总维护者信任子系统维护者不送坏补丁上来**：这是信任链的根基。若子系统维护者反复送上未充分审查的补丁，总维护者会逐步降低对该子系统的信任权重，乃至收回合并授权。
4. **直接处理比例可作为信任链健康度指标**：参考 Linus 约 1.3% 直接处理率，agentrt-liunx 总维护者的直接处理比例应维持在 5% 以下（项目规模较小时上限可放宽）。若该比例持续升高，意味着信任链在某一层断裂——下层维护者要么缺位、要么不作为。
...
agentrt-liunx 由 8 个子仓构成，每个子仓对应一个 agentrt 同源模块域。0.1.1 版本下维护者席位待定（TBD），但归属关系与同源映射须先行固定：
...
- **airymaxos-kernel**：对应 agentrt `atoms/corekern`，承载 MicroCoreRT 微核心运行时在内核态的实现（IPC、内存、任务、时间四大子系统）。这是整个 agentrt-liunx 的内核基座，主维护者须具备 Linux 6.6 内核基线的深度经验。
...
agentrt-liunx 沿用 Linux 内核的 Developer's Certificate of Origin 1.1。每一份贡献在提交时，作者须能自我认证以下声明：
...
agentrt-liunx 使用 DCO bot 在 PR 层自动验证 SoB 链完整性：缺少 SoB 的 PR 无法合并。

### 5.3 审查标签体系

agentrt-liunx 保留 Linux 内核的全部审查标签，语义不变：
...
`Reviewed-by:` 标签不是礼貌性点赞，而是审查者签署的一份正式声明。agentrt-liunx 沿用 Linux 内核的 Reviewer's Statement of Oversight 语义：
...
### 6.4 agentrt-liunx 等价物

agentrt-liunx 的 `Reviewed-by:` 标签语义与 Linux 完全一致。在 PR 场景下：
...
agentrt-liunx 采纳 Linux Foundation TAB 提出的贡献者成熟度模型，作为组织层面人才梯队建设的标准。该模型定义"组织允许/期望/鼓励工程师投入多少上游工作"的六级阶梯：

| Level | 标准 | agentrt-liunx 期望 |
|-------|------|----------------|
| 0 | 工程师不允许贡献内核 | 不适用（agentrt-liunx 拒绝与此类组织合作） |
| 1 | 允许贡献（业余或工作时间内） | 起步阶段——新加入的贡献者 |
| 2 | 期望贡献作为工作职责；支持参加 Linux 会议；贡献计入晋升评审 | **目标 Level**——所有 agentrt-liunx 工程师底线 |
...
- 团队/组织的上游贡献数量
- 上游贡献者占内核开发者总数的比例
- 组织使用的 agentrt-liunx 版本与上游基线的时间间隔（落后天数）
- 内核中的 out-of-tree commit 数量

### 7.2 agentrt-liunx 适配

agentrt-liunx 在 Linux 模型基础上增加两条适配：

1. **"上游"在 agentrt-liunx 语境下双指**：既指 agentrt-liunx 自身的 main 主线，也指 Linux 6.6 内核基线上游。工程师投入 Linux 上游的贡献（如 sched_ext、MGLRU 改进）同样计入 agentrt-liunx 成熟度评估。
2. **Level 2 为组织合作底线**：与 agentrt-liunx 建立商业合作或深度技术合作的组织，其工程团队须达到 Level 2。这是 IRON-9 v2 同源且部分代码共享原则在组织治理上的延伸——agentrt-liunx 不与压榨工程师上游贡献时间的组织深度合作。

> **OS-IRON-10**：组织 6 级成熟度 Level 2 为底线目标——与 agentrt-liunx 深度合作的组织工程团队须达到 Level 2（期望贡献作为工作职责）。
...
agentrt-liunx 总维护者承担 BDFL（Benevolent Dictator For Life）角色。该角色的管理哲学直接继承 Linux 内核 `management-style.rst`，并提炼为四条可操作原则。
...
本章汇总 agentrt-liunx 工程标准体系中所有规则编号，作为规则编号的**单一事实源（Single Source of Truth, SSoT）**。所有工程标准文档中的规则编号必须与本章注册表一致；任何编号冲突以本章为准。新规则须按 10.7 流程申请注册。
...
五维原则在治理层的映射具有正交性：S-3 关注"谁有权决策"（层级结构），A-3 关注"决策如何传达"（人际礼仪），A-4 关注"决策者如何培养"（成熟度梯队），E-6 关注"决策如何追溯"（DCO 链），IRON-9 关注"决策边界在哪"（同源独立）。五者互不重叠，共同构成 agentrt-liunx 治理的完备规则集。
...
- **维护者**: agentrt-liunx 工程标准委员会（待成立，本文档定义其任命流程）
- **变更流程**: 任何治理规则变更必须经过 RFC → 评审（14 天公示）→ 注册流程（第 10.6 节）
- **回顾周期**: 季度回顾 + 年度大版本

---

> **文档结束** | agentrt-liunx 工程标准第 7 篇 | 治理层规范 | 0.1.1 版本 P0 优先完成
