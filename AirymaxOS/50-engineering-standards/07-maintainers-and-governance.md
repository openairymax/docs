...
> **OS-STD-GOV-004**：`Co-developed-by:` 必须紧跟对应共同作者的 `Signed-off-by:`，否则视为格式错误。
...
> **OS-STD-GOV-005**：审查意见必须逐条响应——同意则改代码，不同意则给技术理由，未导致代码改动则转化为代码注释。

> **OS-STD-GOV-006**：不响应审查是致命错误——未逐条响应前轮审查意见就重发的补丁，维护者应直接拒绝。

> **OS-STD-GOV-007**：Andrew Morton 规则——未导致代码改动的审查意见应转化为代码注释，沉淀为代码自文档。
...
- **变更流程**: 任何治理规则变更必须经过 RFC → 评审（14 天公示）→ 注册流程（第 10.1 节）
- **回顾周期**: 季度回顾 + 年度大版本
