...
agentrt daemons 的 `agentrt_agent_module_register()` 与内核 `driver_register()` 同源——两者都遵循"注册 → bus 匹配 → probe 探测 → remove 卸载"生命周期。[SS] 语义同源在此体现为：操作模式同源（注册/匹配/生命周期四段式概念一致），函数签名因抽象层级不同而独立（用户态 daemon 服务 vs 内核 device core）。
