...
**v1.0.1 变更说明**：v1.0 的 IPC 码（`RING_FULL`/`RING_EMPTY`/`REGISTERED`/`URING_CMD` 等）已废弃，替换为与 C-S 检查链精确对齐的命名。由于 0.1.1 阶段 0 行内核代码，此替换无向后兼容负担。
