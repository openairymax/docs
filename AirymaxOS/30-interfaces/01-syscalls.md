...
错误码对齐 `include/airymax/error.h`（[SC] 补充共享头文件，SSoT 权威定义见 `30-interfaces/08-sc-error-contract.md` §2.4），与 agentrt 同源且部分代码共享（IRON-9 v3）。错误码统一使用 `AIRY_E*` 前缀，负值返回。以下为 SSoT 引用，权威定义见 `include/airymax/error.h` 与 [08-sc-error-contract.md §2](08-sc-error-contract.md)，不得另起定义。
