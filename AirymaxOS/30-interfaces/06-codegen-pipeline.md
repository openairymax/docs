...
LSM 钩子 ID（252 个，对齐 Linux 6.6 `security_hook_heads` 钩子数量，详见 [10-architecture/07-directory-structure.md](../10-architecture/07-directory-structure.md) §include/uapi/linux/airymax/security_types.h）由 `cap_gen.py` 的第二阶段生成。输入为 `security/include/airy_lsm_hooks.def`（X-macro 风格，每个条目声明一个 LSM 钩子），生成 `enum airy_lsm_hook_id`（0-251）与钩子分派表索引。
