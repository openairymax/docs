...
-- Capability Badge 位布局（[SC] ipc.h：Epoch<<48 | RandTag<<16 | Perms）
-- 64 位 Native Word Badge（Capability Folding，fastpath C-S9 内联校验）
block cap_badge(64) {
    field perms          15..0       -- 权限位（AIRY_CAP_PERM_*）
    field randtag        47..16      -- 随机标签（32 位）
    field epoch          63..48      -- 时代号（16 位）
}

-- 任务描述符位域（magic 0x41475453 'AGTS'）
-- 11 字段，对齐 [SC] sched.h struct airy_task_desc（64B，v1.0.1）
block task_desc(512) {
    field magic          31..0       -- 'AGTS' 0x41475453
    field prio           47..32      -- 优先级 0-139
    field _pad           63..48      -- 对齐填充
    field runtime_ns     127..64     -- runtime 预算（ns）
    field deadline_ns    191..128    -- 截止时间（ns）
    field period_ns      255..192    -- 周期（ns）
    field vtime          287..256    -- Q16.16 定点虚拟时间
    field agent_id       319..288    -- agent 标识 [0,1023]
    field sched_policy   351..320    -- AIRY_SCHED_POLICY_DEADLINE/FIFO/EEVDF/BESTEFFORT
    field weight         383..352    -- EEVDF 权重
    field state          415..384    -- 生命周期状态
    padding _reserved    511..416    -- 保留 12B（offset 52）
}
```
...
struct airy_ipc_msg_hdr {
	__u32 magic;          /* 31..0     offset 0  */
	__u16 opcode;         /* 47..32    offset 4  */
	__u16 flags;          /* 63..48    offset 6  */
	__u64 trace_id;       /* 127..64   offset 8  */
	__u64 timestamp_ns;   /* 191..128  offset 16 */
	__u64 src_task;       /* 255..192  offset 24 */
	__u64 dst_task;       /* 319..256  offset 32 */
	__u64 capability_badge; /* 383..320 offset 40 (D-9: 8-byte aligned) */
	__u32 payload_len;    /* 415..384  offset 48 */
	__u32 crc32;          /* 447..416  offset 52 */
	__u8  reserved[72];   /* 1023..448 offset 56 (CL1 [56:64) + CL2 [64:128)) */
} AIRY_ALIGNED(64);
...
/* 无 AIRY_IPC_TYPE_* 宏——payload 协议类型（REQUEST/RESPONSE/EVENT/STREAM/
 * CONTROL）由 payload 体首字段携带，不下沉为 [SC] 宏 */

#define AIRY_IPC_FLAG_ZEROCOPY	0x0001
#define AIRY_IPC_FLAG_CAP_CARRY	0x0002
#define AIRY_IPC_FLAG_ENCRYPT	0x0004
#define AIRY_IPC_FLAG_COMPRESS	0x0008
#define AIRY_IPC_FLAG_BATCH_TAIL	0x0010
#define AIRY_IPC_FLAG_RESERVED	0xFFE0
...
AIRY_IPC_FLAG_BATCH_TAIL = 0x0010
AIRY_IPC_FLAG_RESERVED   = 0xFFE0


class AiryIpcMsgHdr(ctypes.LittleEndianStructure):
    """IPC 128 字节定长消息头（@generated — 禁止手工修改）。
    自然对齐（Layout C v4，无 _pack_=1）。"""
    _fields_ = [
        ...
        ("capability_badge", ctypes.c_uint64),  # offset 40
        ("payload_len",      ctypes.c_uint32),  # offset 48
        ("crc32",            ctypes.c_uint32),  # offset 52
        ("reserved",         ctypes.c_uint8 * 72),  # offset 56
    ]
...
AIRY_CAP_ID(AIRY_CAP_CHECKPOINT, 40, "checkpoint/restore")
...
> **SSoT 声明**：LSM 钩子**不通过数字 ID 枚举注册**。Airy 实际注册 7 个 LSM 钩子（对齐 [SC] `lsm_types.h` 的 `AIRY_LSM_HOOK_IMPLEMENTED = 7`），通过 Linux 6.6 LSM 框架原生的 `LSM_HOOK_INIT` 宏名注册（`uring_cmd/task_alloc/task_free/task_kill/file_open/inode_alloc_security/inode_free_security`），不存在虚构的 `airy_lsm_hook_id` 250 ID 枚举。详见 [110-security/01-lsm-framework.md](../110-security/01-lsm-framework.md) §11.2.1。

### 5.3 LSM 钩子注册（非 codegen 生成）

LSM 钩子注册由 `kernel/security/airy/airy_lsm.c` 通过 `LSM_HOOK_INIT` 宏静态注册，**不经过 codegen 管道**。`DEFINE_LSM(airy)` 通过 `security_add_hooks()` 将 7 个钩子挂接到 Linux 6.6 LSM 框架的 `security_hook_heads` 链表。钩子标识通过钩子名（字符串）在内核内部传递，不通过数字 ID。
...
GENERATORS = [
    # (模块, 入口函数, 说明)
    ("cap_gen",      "generate", "Capability ID 生成（LSM 钩子静态注册，非 codegen）"),
...
{{ n + linux_base }}	common	airy_reserved_{{ n }}	sys_ni_syscall
{% endfor %}
{% endfor %}
```
