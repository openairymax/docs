...
| 失败条件 | 降级目标 | 原因 |
|---------|---------|------|
| `payload_len > 0` | SLOW_SEND | payload 需 page flipping，不可内联 |
...
- 接收方同理注册接收缓冲池。
- fastpath 中 `sqe.flags |= IOSQE_FIXED_FILE | IOSQE_BUFFER_SELECT`，从固定池选取 buffer id，零拷贝 page flipping（OS-IFACE-003 零拷贝优先）。
