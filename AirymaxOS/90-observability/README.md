...
# agentrt-liunx（AirymaxOS）可观测性设计

> **文档定位**: agentrt-liunx（AirymaxOS）可观测性工程体系主索引
...
agentrt-liunx 可观测性体系是系统运行状态可见性的核心保障。它继承 Linux 内核 30+ 年沉淀的多层可观测性哲学（ftrace + eBPF + perf + 4 层文件系统接口），并在其上扩展智能体操作系统专属的 Token 能效可观测性、Agent 行为追踪、记忆卷载监控等。
...
| **L6** | **Token 能效监控** | **agentrt-liunx 专属** | **Agent Token 消耗追踪** |
| **L7** | **Agent 行为追踪** | **agentrt-liunx 专属** | **Agent 决策路径审计** |
| **L8** | **记忆卷载监控** | **agentrt-liunx 专属** | **L1→L2→L3→L4 记忆演化** |
...
├── 07-token-efficiency.md          # agentrt-liunx 专属：Token 能效监控
├── 08-agent-tracing.md             # agentrt-liunx 专属：Agent 行为追踪
└── 09-memory-monitoring.md         # agentrt-liunx 专属：记忆卷载监控
```
...
agentrt 的 `commons/` 模块（error/logger/metrics/trace/cost）与 agentrt-liunx 可观测性同源：
- agentrt 用户态：`agentrt_log_write()` / `agentrt_metrics_record()`
- agentrt-liunx 内核态：`printk` / `trace_printk` / `perf_event`
- 两端通过 user_events 桥接
