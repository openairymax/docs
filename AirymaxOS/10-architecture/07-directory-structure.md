...
| **P3 单一宿主** | "每个对象有唯一目的与位置" | [SC] 头文件唯一物理宿主 kernel/include/airymax/ | 禁止物理副本，-I 引用 |
...
**备选方案**：
- E1（全量创建 10 头文件）：在 kernel/include/airymax/ 下创建全部 10 个 [SC] 头文件，0.1.1 内完成
...
**落地约束**：10 头文件在 0.1.1 内全部创建于 `kernel/include/airymax/`（详见 §5）：
...
**备选方案**：
- G1（-I 直接引用，禁止物理副本）：所有子仓通过 `-I../kernel/include` 引用 kernel/include/airymax/
- G2（git submodule 嵌套）：将 kernel/include/airymax/ 作为独立 submodule 嵌套
- G3（构建时复制）：构建脚本将头文件复制到各子仓

**决策依据**：
1. P3 单一宿主：[SC] 头文件唯一物理宿主 kernel/include/airymax/
...
- Kbuild 子仓（security/memory/cognition 内核模块部分）：
  ...
  ```
- 禁止 `cp kernel/include/airymax/*.h <submodule>/include/`
- sc-dual-ci.yml 校验：`diff -r kernel/include/airymax/ agentrt/../kernel/include/airymax/`
...
**落地约束**：
1. **[SC] 共享层**：10 头文件物理宿主 kernel/include/airymax/，各子仓 -I 引用
...
│   ├── kernel/                       # 内核内部头（原生）
│   └── airymax/                      # ★ [SC] 10 头文件唯一物理宿主（OS-IRON-014）
...
│   ├── airy_superv_lsm.c            # DEFINE_LSM(airy) 注册 + LSM_ORDER_FIRST
│   ├── airy_cap_check.c             # 纯 C LSM 钩子（radix_tree_lookup）
...
│   ├── Kbuild
│   ├── airy_lsm.c                   # LSM 注册（DEFINE_LSM(airy) + LSM_ORDER_FIRST）
...
**唯一物理宿主**：`kernel/include/airymax/`（10 个头文件全部在此目录）
...
sc-dual-ci:
  steps:
    - name: verify-sc-byte-identity
      run: |
        diff -r kernel/include/airymax/ agentrt/../kernel/include/airymax/
        if [ $? -ne 0 ]; then
...
|---|--------|---------|-------------|---------|------|
| 1 | `error.h` | A-UEF | — | `kernel/include/airymax/error.h` | 统一错误码（5 子空间：[-1,-40] POSIX / [-41,-70] IPC / [-71,-100] Capability / [-101,-200] [SC] / [-201,-300] [DSL]）+ Fault 码（0x1000+）+ [DSL] 降级块（38 POSIX 码） |
| 2 | `log_types.h` | A-ULP | — | `kernel/include/airymax/log_types.h` | 128B 固定日志记录 + 5 级日志枚举 + printk 8 级映射 |
| 3 | `ipc.h` | A-IPC | `0x41524531` ('ARE1') | `kernel/include/airymax/ipc.h` | IPC 消息头（128B `struct airy_ipc_msg_hdr`，9 字段，`__attribute__((packed))`，`_Static_assert(sizeof==128)`）+ SQE/CQE 操作码与标志位 + Ring 常量（DEF=256, MAX=32768） |
| 4 | `sched.h` | A-ULS | `0x41475453` ('AGTS') | `kernel/include/airymax/sched.h` | 任务描述符（magic/prio/_pad/vtime）+ `airy_vtime_decay()` inline + MAC_MAX_AGENTS=1024 + AIRY_SLICE_DFL=20 + 优先级 0-139 + 权重 1-10000 |
| 5 | `memory_types.h` | MemoryRovol | — | `kernel/include/airymax/memory_types.h` | L1-L4 enum（HOT/WARM/COLD/PMEM）+ GFP 掩码语义（AIRY_GFP_HOT/WARM/COLD/PMEM） |
| 6 | `security_types.h` | 安全 | — | `kernel/include/airymax/security_types.h` | POSIX capability 41 ID（CAP_AGENT_SPAWN=41 起）+ LSM 钩子 252 ID + `enum airy_verdict`（ALLOW/DENY/AUDIT/COMPLAIN）+ `enum airy_cap_op`（7 操作：Copy/Mint/Move/Mutate/Revoke/Delete/Rotate）+ `cap_t` = `uint64_t` |
| 7 | `cognition_types.h` | A-UCS | — | `kernel/include/airymax/cognition_types.h` | `airy_q16_t`（= `__s32`，Q16.16 定点数，因 -mno-80387 禁 FPU）+ `enum airy_cog_phase`（PERCEPT/THINK/ACT）+ `enum airy_think_mode`（FAST/SLOW） |
| 8 | `syscalls.h` | 系统调用 | — | `kernel/include/airymax/syscalls.h` | v1.1: 4 核心系统调用（AIRY_SYS_CALL=0 ... AIRY_SYS_CLT_NOTIFY=3）+ 20 预留（4-23）= 24 槽位 |
| 9 | `uapi_compat.h` | 桥接 | — | `kernel/include/airymax/uapi_compat.h` | 三路类型桥接（`#ifdef __KERNEL__` / `#elif defined(__linux__)` / `#else`），确保 [SC] 头文件跨平台逐字节相同编译 |
| 10 | `lsm_types.h` | 安全 | — | `kernel/include/airymax/lsm_types.h` | `struct airy_lsm_blob` + `airy_capability_check()` 回调原型 + Capability 缓存结构 |

### 5.3 补充共享文件（非 [SC] 核心）

| 文件 | 路径 | 说明 |
|------|------|------|
| `bpf_struct_ops.h` | `kernel/include/airymax/bpf_struct_ops.h` | BPF struct_ops 状态管理共享结构（`struct airy_struct_ops_value` + `enum airy_struct_ops_state`），两端共享但**不属于** 10 个 [SC] 核心头文件 |
...
|------|------|---------|------------|------|
| **共享契约层** | [SC] | 完全共享代码 | `kernel/include/airymax/`（10 头文件） | 10 个头文件，逐字节相同，两端共同依赖 |
...
**物理宿主**：`kernel/include/airymax/`（10 头文件，详见 §5）
...
- `kernel/`（除 include/airymax/ 外）：内核驱动、Kbuild、内核 API、systemd、纯 C LSM、eBPF struct_ops、IORING_OP_URING_CMD、alloc_pages+mmap
...
graph TD
    ...
    subgraph "[SC] 共享层"
        SC[kernel/include/airymax/<br/>10 头文件]
    end
...
| `include/airymax/` | ★ Airymax 新增 | [SC] 10 头文件唯一物理宿主 |
...
**文件**：
1. `airy_superv_lsm.c`：`DEFINE_LSM(airy)` 注册 + `LSM_ORDER_FIRST`
2. `airy_cap_check.c`：纯 C LSM 钩子（`radix_tree_lookup`）
...
| IRON-9 [SC] | 引用方（-I 引用） | 物理宿主（kernel/include/airymax/） |
| IRON-9 [IND] | 跨平台用户态 + SDK 4 语言 | 内核驱动/Kbuild/内核 API |

### 10.2 [SC] 头文件宿主关系

```
agentrt-linux/kernel/include/airymax/    ← 唯一物理宿主（10 头文件）
            ↑
            | -I 引用（禁止物理副本）
            |
   +--------+--------+
...
- 字节级 diff：`diff -r kernel/include/airymax/ agentrt/../kernel/include/airymax/`
...
| **P3 单一宿主** | ★★★★★ | [SC] 10 头文件唯一物理宿主 kernel/include/airymax/，禁止物理副本 | 无 |
...
| [SC] 头文件宿主 | 未明确 | kernel/include/airymax/ 唯一宿主 | P3 提升 |
...
| 4 | 8 子仓结构 | §2、§4 | ✅ |
| 5 | [SC] 头文件单一物理宿主 kernel/include/airymax/ | §5 | ✅ |
