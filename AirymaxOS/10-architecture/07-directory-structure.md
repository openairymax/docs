...
- **OS-IRON-014（\[SC] 单一数据源）**：\[SC] 10 个头文件**唯一物理宿主**于 `kernel/include/airymax/`，其他子仓通过 `-I../kernel/include` 引用，**禁止物理副本**。
...
`kernel/include/airymax/` 目录是 \[SC] 共享契约层的**唯一物理宿主**，10 个 [SC] 头文件 + 2 个补充共享文件：
