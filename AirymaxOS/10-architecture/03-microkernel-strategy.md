...
> **VFS 用户态化决策（USV 统一）**：内核保留 VFS 框架（路径解析、dentry cache），具体文件系统实现通过 FUSE 模型用户态化。此决策由 Airymax Unify Design 的 USV 模块统一管理。
...
> **升级背景**：本次 v1.0 升级将本文档对齐 Airymax Unify Design（见 [10-unify-design.md](10-unify-design.md)）5 模块设计与方案 C-Prime 技术选型，消解 v0.1.1 遗留的架构冲突点。升级内容如下。

### A.1 已融入 Airymax Unify Design 5 模块设计

本文档 §1.4 / §2.2 / §4 等章节描述的机制-策略分离、服务用户态化、IPC 消息传递等微内核思想，已纳入 Airymax Unify Design 5 模块统一架构：

- **UEF（统一错误码与故障定义）**：§3 capability 操作返回值统一为 `AIRY_E*` 负数错误码（[SC] `error.h`）
- **ULPS（统一日志与打印系统）**：内核态日志统一为 `LOG_*` + 128B 记录（[SC] `log_types.h`），见 [05-ring-buffer-logging.md](../40-dataflows/05-ring-buffer-logging.md)
- **UCF（统一配置管理）**：Kconfig/sysctl 与用户态 JSON/TOML 语义映射，见 [11-unified-config.md](../20-modules/11-unified-config.md)
- **USV（统一 Supervisor）**：§2.2 VFS 用户态化决策、调度策略由 USV 模块统一管理
- **UIPF（统一 IPC Fastpath）**：§4 IPC 消息传递对齐 UIPF 模块，见 [07-ipc-fastpath.md](../30-interfaces/07-ipc-fastpath.md)

### A.2 已删除 sched_ext 依赖，采用方案 C-Prime

v0.1.1 中 §1.4 / §2.1 / §4.5 / §9.1 多处将 `sched_ext` BPF 框架作为调度原语承载。鉴于 Linux 6.6 基线下 sched_ext 不可用（见 [15-comprehensive-correction-plan.md](../../docs-closed/agentrt-linux/00-reviews/_review_v2.2/15-comprehensive-correction-plan.md) §1 C-01 冲突点），v1.0 删除 sched_ext 依赖，采用**方案 C-Prime**：

- 调度机制改为 `SCHED_DEADLINE` / `SCHED_FIFO` / `EEVDF` 三层组合，零内核调度器修改
- seL4 MCS 语义映射（SchedContext 捐赠）通过方案 C-Prime 的优先级继承实现，不再依赖 sched_ext
- Agent 8 态生命周期调度承载由 USV 模块管理，权威定义见 [10-sc-sched-extension.md](../30-interfaces/10-sc-sched-extension.md)

### A.3 已新增 [DSL] 降级生存层

v0.1.1 描述的 IRON-9 v2 三层共享模型（[SC] / [SS] / [IND]）已升级为 IRON-9 v3 四层模型，新增 **[DSL]（Degraded Survival Layer，降级生存层）** 作为第四层：
...
### A.4 VFS 用户态化决策已统一（USV 模块管理）

v0.1.1 §2.2 已提及"VFS 用户态化决策（USV 统一）"，v1.0 进一步明确该决策由 Airymax Unify Design 的 **USV 模块**统一管理，消解 C-03 冲突点（VFS 是否完全下沉用户态的矛盾结论）：

- **统一决策**：内核保留 VFS 框架（路径解析、dentry cache），具体文件系统实现通过 FUSE 模型用户态化
- **权威源**：USV 模块（[10-unify-design.md](10-unify-design.md) §5）是 VFS 用户态化决策的唯一权威源
- **禁止重复定义**：本文档及其他文档仅引用 USV 决策，不得重新定义 VFS 下沉边界
