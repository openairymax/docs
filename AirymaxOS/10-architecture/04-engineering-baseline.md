...
> **基线声明（C-08 修正，对齐方案 C-Prime）**：agentrt-linux 1.x.x 采用**标准 Linux 6.6 LTS 主线**作为内核基线，**不含 sched_ext**（sched_ext 于主线 6.12 才合入，6.6 原生不具备）。AI 原生调度能力由**方案 C-Prime**（SCHED_DEADLINE + SCHED_FIFO + EEVDF + seL4 MCS 语义映射）提供，该方案为纯用户态调度策略，零内核调度器修改，性能优于 sched_ext（调度延迟 ~60-150ns vs sched_ext ~200-400ns）。禁止 backport sched_ext，禁止升级到 6.12+ 以获取 sched_ext。
...
| 增强基线（1.x.x）  | Linux 6.6 LTS + 方案 C-Prime（SCHED\_DEADLINE/SCHED\_FIFO/EEVDF + seL4 MCS 映射） | AI 原生能力增强（用户态调度策略，零内核调度器修改，不含 sched\_ext）                    |
...
| 方案 C-Prime 三层调度             | Linux 6.6 原生（SCHED\_DEADLINE/SCHED\_FIFO/EEVDF） | 主线 6.6 原生 | kernel 用户态调度（不含 sched\_ext） |

### 2.3 调度策略增强（方案 C-Prime，不含 sched\_ext）

agentrt-linux 1.0.1 的 AI 原生调度能力通过**方案 C-Prime**（三层调度 + seL4 MCS 语义映射）实现，基于 Linux 6.6 原生调度器（SCHED_DEADLINE / SCHED_FIFO / EEVDF），**不 backport sched\_ext，不升级到 6.12+**：

```
Linux 6.6 原生特性（EEVDF / Rust / MGLRU / XFS fsck / eBPF kfunc / io_uring）
    +
方案 C-Prime（SCHED_DEADLINE + SCHED_FIFO + EEVDF + seL4 MCS 映射，纯用户态调度策略）
    =
...
| ---------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 1.x.x            | **Linux 6.6 LTS 长期锁定**                 | 方案 C-Prime（SCHED\_DEADLINE/SCHED\_FIFO/EEVDF）+ io\_uring + 微内核化改造阶段 1-2（VFS / 网络栈 / 驱动用户态化）                                  |
...
在调度接口契约中，将「调度策略」抽象为 BPF struct\_ops 接口。当 7.1 cgroup 子调度器可用时启用 per-cgroup 策略，否则降级为单 sched\_ext 实例。预留 `airy_sched_ext_ops` 结构体，但 2.x.x 不默认启用 cgroup 子调度器（避免依赖未稳定 ABI）。
...
| -------- | --------------------- | ----------- | ------------ | ------------------------ |
| 调度性能     | SCHED\_AGENT 调度延迟 P99 | 6.6 基线值     | ≤ 基准 110%    | perf + sched\_stat       |
| IPC 性能   | AgentsIPC 小消息延迟       | 6.6 基线值     | ≤ 200 cycles | 微基准测试                    |
| 内存性能     | MGLRU 回收效率            | 6.6 基线值     | ≥ 基准 100%    | stress-ng + /proc/vmstat |
| I/O 性能   | io\_uring 吞吐量         | 6.6 基线值     | ≥ 基准 105%    | fio + io\_uring          |
| 实时性      | PREEMPT\_RT 调度延迟      | 6.6 基线值     | ≤ 10ms       | cyclictest               |
...
2. **syscall 编号独立**——agentrt-linux 拥有独立的 syscall 编号体系（AIRY\_SYS\_0\~11 核心 + AIRY\_SYS\_12\~23 预留），不直接复用 Euler API 的 syscall 编号。这是 IRON-9 v2 "同源且独立"原则的体现：语义同源，编号独立。
