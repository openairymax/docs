...
| 2.x.x | 规划中 | **Linux 7.1 长期锁定**（2026-06-14 发布） | 下一代基线，升级至 Linux 7.1（ADR-013），前瞻性预留见 §8.4 |
...
| 2.x.x | **Linux 7.1 长期锁定** | 完整微内核化 + 超节点沙箱 + 具身智能 + sched_ext cgroup 子调度器 + BPF-powered io_uring + ublk zero-copy + clone3 新 flag |

> **版本基线锁定战略决策**（ADR-013）：1.x.x 全系列长期锁定 Linux 6.6 LTS（维护至 2027-12），2.x.x 全系列升级至 Linux 7.1。迁移遵循「兼容层先行 → 渐进式迁移 → 旧基线 EOL」三阶段路径。详见 §8.4 Linux 7.1 前瞻性预留设计。
...
| 阶段 | 时间 | 策略 |
|------|------|------|
| 短期（2.x.x） | 2027+ | 保持 6.6 LTS 基线，将 7.1 关键 ABI 作为「前瞻性接口」在接口契约中预留（运行时探测 + 条件降级） |
| 中期（3.x.x） | 待定 | 评估升级至 6.18 LTS（PREEMPT_RT/sched_ext 主线化，2028-12 EOL）或 7.4/7.5 LTS（待 2026 末确认） |
| 长期（4.x.x） | 待定 | 全面拥抱 7.x，启用 sched_ext cgroup 子调度器、BPF-powered io_uring、ublk zero-copy 等作为一等公民 |
