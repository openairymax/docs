...
本文档覆盖 device/driver/bus/class 四元关系、probe/remove 生命周期、name/OF/ACPI 三源匹配、sysfs 导出、kref/kobject 引用计数五大主题。IRON-9 v2 同源且部分代码共享体现为：内核态结构体与上游保持语义等价（确保可直接复用上游驱动代码）；用户态扩展（`agent_bus_type`、`devm_agentrt_*` 资源族）不进入上游内核，而是作为 agentrt-liunx 专属扩展通过 `agentos/daemon/` 适配器接入。
