...
- [第 11 章 AirymaxOS Token 能效监控与 Agent 行为追踪](#第-11-章-airymaxos-token-能效监控与-agent-行为追踪)
...
Linux 6.6 内核基线在 `include/uapi/linux/bpf.h` 定义了 30+ 种 BPF 程序类型。AirymaxOS 主要使用以下七种：
...
`BPF_PROG_TYPE_STRUCT_OPS` 程序作为子系统的操作表成员，通过 `BPF_MAP_TYPE_STRUCT_OPS` map 注册。AirymaxOS SCHED_AGENT 调度器通过此类型集成（详见第 10 章）。`BPF_PROG_TYPE_TRACING` 程序通过 fentry/fexit 挂载到内核函数入口/出口，相比 kprobe 性能更高（无 int3 陷阱），但需 BTF 支持。
...
Linux 6.6 内核基线提供 28+ 种 BPF map 类型。AirymaxOS 常用以下几种：
...
eBPF 程序从源码到运行经历五步：编译（clang -target bpf）、加载（bpf(BPF_PROG_LOAD)）、attach（kprobe/tracepoint/perf_event/struct_ops 挂载）、运行、卸载。AirymaxOS 推荐使用 libbpf 框架管理生命周期：
...
struct_ops 是 Linux 6.6 内核基线提供的机制，允许 BPF 程序作为内核子系统的操作表（struct of function pointers）。AirymaxOS 通过此机制实现 SCHED_AGENT 可插拔调度器和 Cupolas 动态安全策略。
...
**OS-OBS-021: AirymaxOS 可插拔调度策略必须通过 struct_ops 机制集成，不得自行编写调度类内核模块；SCHED_AGENT 注册名为 `"sched_agent_ops"`。**
...
**OS-OBS-022: AirymaxOS BPF 调度器加载后，策略守护进程必须通过 [SC] `airymax_struct_ops_state` 读取 struct_ops map 的 state 字段，确认 state==INUSE 后方可标记调度器在线。**
...
## 第 11 章 AirymaxOS Token 能效监控与 Agent 行为追踪
...
MicroCoreRT 极简内核契约要求所有 BPF 程序代码体积最小化。AirymaxOS BPF 程序单文件指令数不得超过 1 万，编译后 .o 体积不得超过 64 KB。该约束体现 IRON-9 v2 同源且部分代码共享原则——与 agentrt 用户态观测通过 [SC] 共享 trace_event 头契约 + [SS] 语义同源（BPF map/ringbuf 模型），但 [IND] 内核态实现独立（BPF 字节码 vs 用户态进程）。
...
AirymaxOS 在 Linux 6.6 内核基线上严格遵循五维正交 24 原则——每条 OS-OBS 规则都可追溯至至少一条五维原则。IRON-9 v2 同源且部分代码共享原则要求 eBPF 的内核态实现与 agentrt 用户态可观测性保持 [SC] 共享契约 + [SS] 语义同源 + [IND] 内核态实现独立。两端通过 MicroCoreRT 极简内核契约与 AgentsIPC 消息协议实现无适配层互操作。
...
IRON-9 v2 将 agentrt 与 AirymaxOS 的 eBPF 协作划分为三层：
...
**OS-STD-015: AirymaxOS eBPF 与 agentrt 共享 `include/airymax/bpf_struct_ops.h` 头文件，struct_ops state 枚举值（0/1/2/3）与字段语义两端必须一致；两端事件格式必须一致，便于跨态聚合分析。**

### 13.3 [SS] 语义同源层

| 维度 | agentrt 用户态 | AirymaxOS 内核态 | 同源点 |
|------|----------------|------------------|--------|
...
graph LR
    ...
    F[agentrt 策略引擎] -->|读取 [SC] state 字段| G[AirymaxOS struct_ops map]
