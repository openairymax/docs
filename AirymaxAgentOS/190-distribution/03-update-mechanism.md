...
# agentrt-liunx（AirymaxOS）更新机制设计详细规范

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）内核热更新、原子更新与回滚机制详细设计
...
agentrt-liunx（AirymaxOS）更新机制设计旨在为运行中的系统提供安全、原子、可回滚的更新能力。本设计聚焦三大目标：
...
本设计严格遵守 agentrt-liunx 术语规范：agentrt（用户态）称为**微核心**（micro-core），agentrt-liunx（OS 发行版）称为**微内核**（micro-kernel）。所有外部 Linux 发行版统一表述为"主流 Linux 发行版"，禁止使用 openEuler/Euler 字样。更新机制与 agentrt 之间属于 IRON-9 v2 [IND] 完全独立层。
...
MODULE_DESCRIPTION("agentrt-liunx SCHED_AGENT vtime fix");
```
...
#     Subject: agentrt-liunx 1.0.1 release
...
#     Version: 1.0.2
#     Commit: 1a2b3c4d5e6f...
#     Subject: agentrt-liunx 1.0.2 release
#   airymaxos 1.0.1
#     Version: 1.0.1
#     Commit: 7a8b9c0d1e2f...
#     Subject: agentrt-liunx 1.0.1 release
...
#     Version: 1.0.0
#     Commit: 0a1b2c3d4e5f...
#     Subject: agentrt-liunx 1.0.0 release
...
[Unit]
Description=agentrt-liunx Health Check after Boot
...
更新机制错误码纳入 agentrt-liunx 统一错误码体系：
...
agentrt-liunx 的更新机制与 agentrt 完全独立（[IND] 完全独立层），各自演进。
...
> **文档结束** | agentrt-liunx（AirymaxOS）更新机制设计详细规范 | 1.0.1 开发版本
