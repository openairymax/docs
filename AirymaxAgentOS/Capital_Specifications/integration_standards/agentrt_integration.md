...
graph TB
    ...
    subgraph "IRON-9 v2 三层"
        SC["[SC] 共享契约层<br/>━━━━━━━━━━━━━<br/>include/airymax/<br/>airymax_ipc.h<br/>airymax_errno.h<br/>airymax_cap.h<br/>airymax_mem.h<br/>━━━━━━━━━━━━━<br/>完全共享代码"]
...
#### 2.2.1 airymax_ipc.h — IPC 协议头文件

| 字段 | 说明 |
|------|------|
| 文件名 | `include/airymax/airymax_ipc.h` |
| 维护方 | agentrt |
| 共享内容 | 128B 定长消息头结构体、magic 定义（0x41524531）、5 种 payload 协议枚举、trace_id 格式 |
| agentrt-liunx 使用方式 | 直接 `#include` 使用，内核态 IPC 子系统原生支持 128B 消息头 |

```c
// airymax_ipc.h — 共享契约（简化示意）
...
#### 2.2.2 airymax_errno.h — 错误码头文件

| 字段 | 说明 |
|------|------|
| 文件名 | `include/airymax/airymax_errno.h` |
| 维护方 | agentrt |
| 共享内容 | 通用用户态错误码定义（0x00010000 ~ 0x0008FFFF）、IPC 错误码、capability 错误码、记忆错误码 |
| agentrt-liunx 使用方式 | 直接 `#include` 使用，内核态映射到负整数错误码 |
...
| 落地位置 | 集成方式 | 说明 |
|----------|----------|------|
| 全部子仓 | 用户态直接 `#include` | 所有用户态代码使用相同错误码 |
| airymaxos-kernel | 内核态映射到负整数 | `AGENTRT_ERR_*` → `-E_*` 映射 |
| project_erp/error_code_reference.md | 文档同步 | 错误码文档与头文件同步 |

#### 2.2.3 airymax_cap.h — capability 令牌头文件

| 字段 | 说明 |
|------|------|
| 文件名 | `include/airymax/airymax_cap.h` |
| 维护方 | agentrt |
| 共享内容 | capability 令牌结构体、capability 类型枚举、capability 操作类型（委托/复制/限制/撤销） |
| agentrt-liunx 使用方式 | 内核态 capability 系统使用相同格式的令牌 |

```c
// airymax_cap.h — 共享契约（简化示意）
typedef struct __attribute__((aligned(64))) {
...
typedef enum {
    AIRYMAX_CAP_OP_DELEGATE = 0x01,
    AIRYMAX_CAP_OP_COPY     = 0x02,
    AIRYMAX_CAP_OP_RESTRICT = 0x03,
...
#### 2.2.4 airymax_mem.h — 记忆数据结构头文件

| 字段 | 说明 |
|------|------|
| 文件名 | `include/airymax/airymax_mem.h` |
| 维护方 | agentrt |
| 共享内容 | L1-L4 记忆卷载数据结构、记忆检索查询结构体、遗忘策略参数 |
| agentrt-liunx 使用方式 | 内核态 MemoryRovol 使用相同的数据结构 |

```c
// airymax_mem.h — 共享契约（简化示意）
typedef struct {
    uint64_t mem_id;       /* 全局唯一记忆 ID */
    uint64_t timestamp;    /* 创建时间戳 */
    uint32_t layer;        /* 记忆层级: 1=原始卷, 2=特征层, 3=结构层, 4=模式层 */
...
typedef struct {
    uint64_t mem_id;       /* 检索目标记忆 ID */
    uint32_t retrieval_type; /* 检索类型: 0=精确匹配, 1=语义检索 */
    uint32_t top_k;        /* 返回 Top-K 结果 */
    float    threshold;    /* 相似度阈值 */
    uint8_t  query_vector[256]; /* 查询向量 */
} airymax_mem_query_t;
```
...
| 测试层 | 说明 | 覆盖范围 |
|--------|------|----------|
| **L1: [SC] 层契约测试** | 验证共享契约层头文件的一致性 | 4 个头文件的结构体布局、枚举值、宏定义 |
...
graph LR
    subgraph "agentrt 仓库"
        A1[include/airymax/airymax_ipc.h]
        A2[include/airymax/airymax_errno.h]
        A3[include/airymax/airymax_cap.h]
        A4[include/airymax/airymax_mem.h]
    end
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.1.1 | 2026-07-07 | 初始版本（集成架构总览 + 4 个 [SC] 共享头文件 + 5 个 [SS] 语义域 + ABI 兼容性 + 版本对齐 + 5 层集成测试 + 性能基准 + Mermaid 数据流图） |
