...
# agentrt-liunx 集成标准总览

> **文档定位**: agentrt-liunx（AirymaxOS）集成标准的顶层入口，定义内核层→服务层→应用层的集成层次、与 agentrt 的集成规范、与 Euler 标准的兼容性集成、生态合作规范与第三方模块集成标准
...
agentrt-liunx（AirymaxOS）作为一个 OS 发行版，其核心价值不仅在于自身组件的高质量，更在于与外部系统的无缝集成能力。集成标准规范定义了 agentrt-liunx 如何与以下系统进行集成：

| 集成对象 | 集成层次 | 核心挑战 |
|----------|----------|----------|
| **agentrt** | 同源深度集成 | 共享 IRON-9 v2 三层架构（[SC]/[SS]/[IND]），确保无适配层天然契合 |
| **Euler 标准** | 生态兼容集成 | 协议层兼容 + 接入层兼容 + 模块化兼容 + 工具链兼容 |
...
```
Capital_Specifications/integration_standards/
...
agentrt-liunx（AirymaxOS）的集成遵循严格的三层架构（S-2 层次分解原则）：
...
agentrt-liunx（AirymaxOS）与 agentrt 的集成是基于 IRON-9 v2 工程铁律的同源深度集成。核心原则：

| 原则 | 说明 |
|------|------|
| **无适配层** | agentrt 在 agentrt-liunx 上运行时无需任何适配层，天然契合 |
| **同源优先** | 优先使用同源语义的特性（如 SCHED_AGENT 调度），但可选使用 |
| **契约共享** | [SC] 层代码完全共享，通过 `include/airymax/` 头文件库同步 |
| **独立演进** | 两者独立演进，通过契约层保持兼容 |
| **双向验证** | 契约层变更须经 agentrt + agentrt-liunx 两端 CI 双向校验 |
...
| 集成点 | 所属层级 | 说明 |
|--------|----------|------|
| 4 个共享头文件 | [SC] | `airymax_errno.h` / `airymax_ipc.h` / `airymax_cap.h` / `airymax_mem.h` |
...
agentrt-liunx（AirymaxOS）与 Euler 标准保持兼容性集成，确保在企业级 Linux 生态中的互操作性。兼容性集成涵盖四个层面：
...
| IPC 协议 | 128B 消息头 + 5 种 payload | 基于 Euler 标准 IPC 协议扩展 |
...
| 接入方式 | 兼容标准 | 说明 |
|----------|----------|------|
| 包格式 | RPM | 兼容 Euler 标准 RPM 包格式 |
| 包管理器 | dnf | 兼容 Euler 标准包管理器 |
| 服务管理 | systemd | 兼容 Euler 标准 systemd 单元 |
| 配置管理 | /etc + systemd unit | 兼容 Euler 标准配置路径 |
| 日志系统 | journald + syslog | 兼容 Euler 标准日志系统 |
| 用户管理 | /etc/passwd + PAM | 兼容 Euler 标准用户管理 |

### 4.4 模块化兼容

| 模块类型 | 兼容标准 | 说明 |
|----------|----------|------|
| 内核模块 | .ko 格式，MODULE_LICENSE | 兼容 Euler 标准内核模块 |
| eBPF 程序 | CO-RE（一次编译到处运行） | 兼容 Euler 标准 BPF 程序 |
| 动态库 | .so 格式，ELF 标准 | 兼容 Euler 标准动态链接 |
| 容器镜像 | OCI 格式 | 兼容 Euler 标准容器镜像 |
| Wasm 模块 | Wasm 3.0 标准 | 兼容 Euler 标准 Wasm 运行时 |

### 4.5 工具链兼容

| 工具类型 | 兼容标准 | 说明 |
|----------|----------|------|
| 编译器 | GCC 12+ / Clang 17+ | 兼容 Euler 标准编译器 |
| 构建系统 | Make / CMake / Kbuild | 兼容 Euler 标准构建系统 |
| 调试器 | GDB、LLDB | 兼容 Euler 标准调试器 |
| 性能分析 | perf、eBPF、ftrace | 兼容 Euler 标准性能分析工具 |
| 测试框架 | KUnit、kselftest、pytest | 兼容 Euler 标准测试框架 |
...
agentrt-liunx（AirymaxOS）建立开放的生态合作框架，支持以下合作类型：
...
| 错误处理 | 统一错误码 | 使用 agentrt-liunx 统一错误码 |
...
| 错误处理 | 统一错误码 | 返回 agentrt-liunx 统一错误码 |
...
| 基础镜像 | agentrt-liunx 基础镜像 | 基于官方基础镜像构建 |
...
| **兼容性测试** | 验证与 Euler 标准的兼容性 | 协议层、接入层、模块化、工具链 |
...
| 兼容性测试 | 通过 Euler 标准兼容性测试 | 所有兼容性测试通过 |
...
| **兼容性保证** | 集成不得破坏与 Euler 标准的兼容性 | 兼容性声明 |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.1.1 | 2026-07-07 | 初始版本（三层集成架构 + agentrt 集成概述 + Euler 标准四层兼容 + 生态合作规范 + 6 类第三方模块集成标准 + 集成测试标准） |
