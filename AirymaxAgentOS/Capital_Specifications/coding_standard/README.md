...
> **最后更新**: 2026-07-07
> **父文档**: [Capital_Specifications 总索引](../README.md)
...
|---------------|------|-------------|------|
| **[SC] 共享契约层** | 代码完全共享 | 共享 `include/airymax/` 4 个头文件中的类型定义、常量、宏 | `AGENTRT_IPC_MSG_HDR_SIZE`、`agentrt_errno.h` |
...
| `KMSAN` (Kernel Memory Sanitizer) | 未初始化内存使用 | C | 每次 PR (CI) |
...
| 头文件 | 内容 | 共享方式 | 变更流程 |
|--------|------|----------|----------|
| `ipc_msg.h` | AgentsIPC 128B 消息头结构定义 | 完全共享，双向同步 | 任一端变更需同步另一端 |
| `errno.h` | agentrt 错误码体系 | 完全共享，双向同步 | 语义版本号递增 |
| `types.h` | 基础类型定义（`agentrt_task_desc_t` 等） | 完全共享，双向同步 | 类型变更需兼容性评估 |
| `capability.h` | Cupolas capability 令牌结构体 | 完全共享，双向同步 | 安全审查强制 |
...
agentrt-liunx（AirymaxOS）编码规范在以下方面对齐 Euler 标准：
