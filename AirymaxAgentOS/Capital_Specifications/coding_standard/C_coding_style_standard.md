...
agentrt-liunx（AirymaxOS）内核态错误码对齐 `agentrt_errno.h`（[SC] 共享契约层），使用 `AGENTRT_E*` 前缀：

```c
/* [SC] 共享契约层：agentrt_errno.h 中定义的错误码 */
#define AGENTRT_OK              0
...
[SC] 共享契约层是 IRON-9 v2 三层模型中**代码完全共享**的层级。agentrt-liunx（AirymaxOS）与 agentrt 共享 `include/airymax/` 下的 4 个头文件：
- `ipc_msg.h`：AgentsIPC 128B 消息头定义
- `errno.h`：agentrt 错误码体系
- `types.h`：基础类型定义
- `capability.h`：Cupolas capability 令牌定义
...
```c
/* [SC] 共享契约层示例：include/airymax/ipc_msg.h */
#ifndef _AGENTRT_IPC_MSG_H
#define _AGENTRT_IPC_MSG_H
