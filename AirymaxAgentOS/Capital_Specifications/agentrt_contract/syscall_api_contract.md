...
| 4 | 调度 | `AGENTRT_SYS_SCHED_*` | 572-591 | sched_ext 策略设置、SCHED_AGENT 调度类管理 | kernel | MicroCoreRT sub-scheduler |
...
agentrt-liunx 与 agentrt 在以下 4 个头文件中实现代码字面共享：

| 头文件 | 共享内容 | 对应系统调用分类 |
|--------|---------|----------------|
| `bpf_struct_ops.h` | sched_ext BPF 调度器结构体操作定义 | 调度（SCHED） |
| `memory_types.h` | MemoryRovol L1-L4 数据结构定义 | 内存管理（ROVOL） |
| `security_types.h` | Cupolas capability 令牌结构与权限位图 | 安全（CAP） |
| `cognition_types.h` | CoreLoopThree 认知循环状态机与阶段枚举 | 认知（CLT） |
