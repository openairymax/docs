...
# agentrt-liunx 统一错误码参考

> **文档定位**: agentrt-liunx（AirymaxOS）全系统统一错误码的权威参考，定义内核态与用户态双错误码体系、分段规划、与 agentrt 的映射关系、使用规范与维护流程
...
agentrt-liunx（AirymaxOS）统一错误码体系的设计目标：
...
本错误码体系适用于 agentrt-liunx（AirymaxOS）的以下全部子仓和组件：
...
agentrt-liunx（AirymaxOS）采用双错误码体系的核心原因在于内核态与用户态的错误表示本质不同：
...
通用错误码复用 Linux 内核标准 errno，agentrt-liunx 不重新定义，直接使用内核已有定义：
...
根据 IRON-9 v2 工程铁律，agentrt-liunx 与 agentrt 的错误码映射遵循三层架构：

#### 3.1.1 [SC] 共享契约层 — 完全共享

`include/airymax/` 头文件库中的错误码定义完全共享，agentrt 和 agentrt-liunx 使用相同的错误码数值和语义：

| 头文件 | 共享内容 | 说明 |
|--------|----------|------|
| `airymax_errno.h` | 通用错误码定义 | 0x00010000 ~ 0x0008FFFF 的通用错误码 |
| `airymax_ipc_err.h` | IPC 错误码定义 | IPC 相关错误码（-300 ~ -399 对应内核态） |
| `airymax_cap_err.h` | capability 错误码定义 | capability 相关错误码 |
| `airymax_mem_err.h` | 记忆错误码定义 | 记忆系统相关错误码 |

#### 3.1.2 [SS] 语义同源层 — 语义一致，数值独立

以下错误码语义与 agentrt 一致，但具体数值可能不同：

| 语义域 | agentrt 实现 | agentrt-liunx 实现 | 说明 |
|--------|--------------|---------------------|------|
...
| agentrt 独立域 | agentrt-liunx 独立域 | 说明 |
|----------------|----------------------|------|
...
当 agentrt 在 agentrt-liunx 上运行时，错误码转换遵循以下规则：

```
[SC] 层错误码：直接透传，无需转换
[SS] 层错误码：通过映射表转换（agentrt 错误码 → agentrt-liunx 错误码）
[IND] 层错误码：不转换，各自独立处理
```

### 3.3 错误码双向映射表（[SS] 层）

| agentrt 错误码 | 语义 | agentrt-liunx 错误码 | 转换方向 |
|----------------|------|----------------------|----------|
...
/**
 * @brief 错误码宏定义 — 用户态
 * @file include/airymax/airymax_errno.h
 * @since 1.0.1
 */
...
| **修改 [SC] 层错误码** | 必须 agentrt + agentrt-liunx 双向同步 | 双端架构委员会联合审查 |

### 5.3 错误码弃用流程

```
1. 在 airymax_errno.h 中添加 @deprecated 注释，并注明替代错误码
