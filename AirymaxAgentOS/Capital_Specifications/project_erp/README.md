...
# agentrt-liunx 项目管理规范总览

> **文档定位**: agentrt-liunx（AirymaxOS）项目管理规范的顶层入口，定义项目资源、错误码、SBOM 等核心管理域的规范体系
...
操作系统级项目的管理不同于普通应用软件项目。agentrt-liunx（AirymaxOS）作为基于 Linux 内核的 Agentic OS 发行版，其项目管理面临以下独特挑战：
...
agentrt-liunx（AirymaxOS）的项目管理规范遵循五维正交 24 原则，尤其体现在以下工程观原则：
...
```
Capital_Specifications/project_erp/
...
| 文档 | 职责 | 受众 |
|------|------|------|
| **error_code_reference.md** | 定义 agentrt-liunx 全系统统一错误码体系，涵盖内核态与用户态双错误码空间、分段规划、与 agentrt 的映射关系、使用规范与维护流程 | 内核开发者、服务开发者、测试工程师 |
| **SBOM.md** | 定义 agentrt-liunx 软件物料清单的生成标准、组件追踪规则、许可证合规矩阵、漏洞扫描策略与自动生成流程 | 发布工程师、安全审计员、合规团队 |
...
agentrt-liunx（AirymaxOS）是一个跨内核态和用户态、跨 8 个子仓的复杂系统。没有统一的错误码体系，将导致以下问题：

1. **错误不可追溯**：同一错误在不同子仓中表现为不同的错误码，无法定位根源（违反 E-6 错误可追溯原则）
2. **跨层调试困难**：内核态错误码与用户态错误码无法关联，调试时无法获得完整错误链
3. **与 agentrt 无法对接**：agentrt 在 agentrt-liunx 上运行时，若错误码不一致，同源红利丧失
4. **文档与代码脱节**：错误码定义分散，维护成本高（违反 E-7 文档即代码原则）

### 3.2 双错误码体系总览

agentrt-liunx 采用**双错误码体系**：
...
根据 IRON-9 v2 工程铁律，agentrt-liunx 与 agentrt 共享契约层代码（[SC] 层）。错误码映射遵循以下规则：

| 层级 | 错误码关系 | 映射方式 |
|------|------------|----------|
| **[SC] 共享契约层** | 完全共享 | agentrt 错误码与 agentrt-liunx 内核态错误码直接映射，无需转换 |
...
软件物料清单（SBOM）是 agentrt-liunx（AirymaxOS）供应链安全的核心组成部分：
...
agentrt-liunx SBOM 覆盖以下组件类别：

| 类别 | 组件示例 | 优先级 |
|------|----------|--------|
| **Linux 内核基线** | Linux 内核 6.6 基线（含 agentrt-liunx 内核增强） | 最高 |
...
- **[SC] 共享契约层**：agentrt-liunx 与 agentrt 共享 `include/airymax/` 头文件库，SBOM 中共同标注此部分
...
agentrt-liunx（AirymaxOS）的资源管理遵循 E-3 资源确定性原则：每块内存的分配者必须明确其释放者，每个文件句柄的打开者必须明确其关闭者，每个线程的创建者必须明确其销毁者。
...
agentrt-liunx（AirymaxOS）作为 OS 级项目，面临以下关键风险，项目管理规范定义相应的应对策略：

| 风险类别 | 风险描述 | 影响等级 | 应对策略 |
|----------|----------|----------|----------|
| **内核基线漂移** | 引用未来内核特性作为 6.6 原生能力 | 严重 | IRON-10 / BAN-361 / ACC-OS04 三重防护 |
| **同源失配** | agentrt 与 agentrt-liunx 的 [SC] 层版本不一致 | 严重 | 跨仓 CI 双向校验 + 版本锁定 |
...
agentrt-liunx（AirymaxOS）的项目管理变更遵循以下流程：
...
graph TD
    ...
    D -->|单子仓| E[子仓治理组批准]
    D -->|跨子仓| F[架构委员会批准]
    D -->|涉及 [SC] 层| G[agentrt + agentrt-liunx 联合审查]
    E --> H[实施变更]
    F --> H
    G --> H
    H --> I[更新文档]
...
| [SC] 层变更 | agentrt + agentrt-liunx 架构委员会 | 双端联合审查 | 双端 ADR |
| 紧急安全修复 | Security 治理组 | Security 治理组负责人批准 | 安全公告 + 事后 ADR |

### 5.9 项目文档管理规范

agentrt-liunx（AirymaxOS）的项目文档遵循 E-7 文档即代码原则，全部文档纳入版本控制：

| 文档类型 | 存放位置 | 版本控制 | 审查要求 |
|----------|----------|----------|----------|
| 设计文档 | `docs/AirymaxAgentOS/` | Git | 子仓治理组审查 |
| 项目管理规范 | `docs/AirymaxAgentOS/Capital_Specifications/` | Git | 架构委员会审查 |
...
- [统一错误码参考](error_code_reference.md)：agentrt-liunx 全系统统一错误码体系
- [SBOM 规范](SBOM.md)：agentrt-liunx 软件物料清单规范
- [集成标准总览](../integration_standards/README.md)：agentrt-liunx 集成标准
