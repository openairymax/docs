...
1. **Euler 标准 兼容性**
   - 必须符合 Euler 标准 对操作系统内核的要求
   - 用户空间工具兼容 Euler 标准 包管理格式
   - 符合 Euler 标准 安全配置基线
   - 定期同步 Euler 标准 社区更新
...
5. **Euler 标准 兼容性矩阵**
   - 内核版本：与 Euler 标准 LTS 内核版本同步更新
   - 系统工具：兼容 Euler 标准 核心工具链
   - 安全基线：满足 Euler 标准 安全配置规范
   - 认证要求：通过 Euler 标准 兼容性认证测试套件
...
所有模块必须满足 Euler 标准 兼容性和 Linux ABI 稳定性要求，确保生态兼容性。通过严格的质量门禁和生命周期管理，保证 agentrt-liunx（AirymaxOS）整体质量可控。
