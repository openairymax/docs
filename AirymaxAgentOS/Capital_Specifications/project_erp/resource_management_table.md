...
agentrt-liunx（AirymaxOS）是一个面向实时智能体的轻量级操作系统发行版，其项目管理遵循 **五维正交** 原则进行资源切分与治理。所谓五维正交，是指从以下五个独立维度对项目资源进行结构化建模：
...
通过五维正交的资源管理模型，agentrt-liunx（AirymaxOS）能够在每个维度上独立演进，互不耦合，从而降低多仓库协同的复杂度。所有资源的分配、审计与回收均通过 agentrt-linux 管理仓库统一编排，基于 IRON-9 v2 流水线引擎执行自动化调度。

---

## 2. 八子仓库资源清单

agentrt-liunx（AirymaxOS）由 agentrt-linux 管理仓库统一纳管以下八个子仓库：
...
agentrt-liunx（AirymaxOS）的上游依赖涵盖内核、运行时、工具链和基础设施层：
...
以下项目依赖 agentrt-liunx（AirymaxOS）或其组件：

### 4.1 核心下游

| 下游项目 | 依赖组件 | 依赖类型 | 兼容版本 | 状态 |
|----------|----------|----------|----------|------|
| agentrt | airymaxos-kernel, airymaxos-services, airymaxos-cognition | 运行时平台 | agentrt-liunx >= 2.0 | 稳定 |
| agentrt SDK | airymaxos-services (API), airymaxos-system (CLI) | API 绑定 | agentrt-liunx >= 2.0 | 稳定 |
| agentctl | airymaxos-system (agentctl socket) | IPC 协议 | agentrt-liunx >= 2.0 | 稳定 |
| Airymax Desktop | airymaxos-cloudnative, airymaxos-cognition | 完整 OS 镜像 | agentrt-liunx >= 2.1 | Beta |
| Airymax Edge | airymaxos-kernel (minimal), airymaxos-memory | 裁剪镜像 | agentrt-liunx >= 2.0 | 稳定 |

### 4.2 生态下游

| 下游项目 | 依赖组件 | 依赖类型 | 兼容版本 | 状态 |
|----------|----------|----------|----------|------|
| Euler 标准兼容层 | airymaxos-kernel (ABI) | ABI 兼容 | agentrt-liunx >= 2.0 | 计划中 |
| K8s Airymax Node | airymaxos-cloudnative (CRI) | CRI 插件 | agentrt-liunx >= 2.1 | Alpha |
| SPHARX AI Platform | airymaxos-cognition | 推理运行时 | agentrt-liunx >= 2.0 | 稳定 |
...
| agentrt-liunx（AirymaxOS） 代码 | 允许 | 允许 | 首选 | 允许 | 允许 | 允许 | 禁止 |

**许可证策略**：agentrt-liunx（AirymaxOS）的原创代码统一采用 Apache-2.0 许可证，fork 自上游的代码保留其原始许可证。所有依赖的许可证兼容性已通过 IRON-9 v2 合规扫描流水线自动化检查。
...
agentrt-liunx（AirymaxOS）的资源监控基于以下三层架构：
...
以下为 agentrt-liunx（AirymaxOS）所有第三方依赖的审计检查清单，由 IRON-9 v2 合规扫描流水线在每次 CI 构建时自动执行：
...
| 术语 | 定义 |
|------|------|
| agentrt-liunx（AirymaxOS） | 面向实时智能体的轻量级操作系统发行版，项目核心术语 |
| agentrt-linux | agentrt-liunx（AirymaxOS）的顶层管理仓库，负责八子仓库的编排与集成 |
