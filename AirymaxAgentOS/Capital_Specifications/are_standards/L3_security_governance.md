...
# agentrt-liunx L3 安全与治理规范

**最新**: 2026-07-07  
**版本**: 0.1.1（文档体系完成）/ 1.0.1（开发）  
**状态**: 草案  
**路径**: OpenAirymax/docs/AirymaxAgentOS/Capital_Specifications/are_standards/L3_security_governance.md  
...
- **前置知识**: 理解 agentrt-liunx（AirymaxOS）架构概览，熟悉 Linux 安全模块（LSM），了解 capability 安全模型  
- **预计阅读时间**: 50 分钟  
...
L3 安全与治理是 agentrt-liunx（AirymaxOS）ARE Standards 的最上层规范，定义了系统的安全模型、权限管理、沙箱隔离、审计追溯和合规治理体系。L3 的设计哲学根植于五维正交24原则中 E-1（安全内生原则）：安全不是事后附加的功能，而是系统设计的内在属性。

agentrt-liunx（AirymaxOS）作为面向智能体协作的操作系统，其安全需求不同于传统操作系统：
...
Cupolas（安全穹顶）是 agentrt-liunx（AirymaxOS）的核心权限引擎，实现了基于能力（capability）的安全模型。其设计参考 seL4 的 capability-based security 模型，结合 Linux 现有能力体系，为智能体协作场景提供细粒度的权限控制。
...
agentrt-liunx（AirymaxOS）集成 Linux capabilities 并扩展为 38 ID 枚举：
...
> 注：ID 38-40 为 agentrt-liunx（AirymaxOS）在 Linux 标准 38 能力基础上新增的 Agent 专属能力，属于 [IND] 完全独立层。

---

## 3. 五级沙箱模型

agentrt-liunx（AirymaxOS）定义了五级沙箱安全隔离模型，从 Level 0（完全不隔离）到 Level 4（硬件级隔离），每一级对应不同的安全需求和性能开销。
...
agentrt-liunx（AirymaxOS）使用统一的错误码体系，确保错误信息在 OS 层和用户态层之间一致传递。该体系在 IRON-9 v2 [SC] 共享契约层与 agentrt 完全共享。
...
审计日志是 agentrt-liunx（AirymaxOS）不可妥协的安全底线，对应五维正交24原则中 E-1（安全内生原则）和 E-6（错误可追溯原则）。
...
agentrt-liunx（AirymaxOS）在 Linux 安全模块（LSM）框架基础上，为 Agent 场景增加了专属安全钩子点。
...
agentrt-liunx（AirymaxOS）在标准 Linux LSM 254 个钩子之上，增加了以下 Agent 专属钩子：

```c
/**
 * Agent 专属 LSM 钩子（[IND] 完全独立层）
 *
 * 这些钩子仅在 agentrt-liunx（AirymaxOS）中存在，
 * 不属于 agentrt 共享范畴。
 */
...
**图2: agentrt-liunx 安全治理流程**

---

## 8. 双许可证体系

agentrt-liunx（AirymaxOS）遵循双许可证体系，平衡开放性与商业可持续性。
...
1. **AGPL-3.0 for 用户态服务**：确保 agentrt-liunx 作为网络服务运行时，对其修改必须开源，维护社区生态健康
2. **Apache-2.0 for SDK**：允许商业公司基于 agentrt-liunx 构建闭源智能体应用，促进生态繁荣
...
agentrt-liunx（AirymaxOS）遵循 SPDX 2.3 标准生成软件物料清单（SBOM），确保供应链透明度。
...
agentrt-liunx（AirymaxOS）的 Cupolas 权限引擎严格参考了 seL4 的 capability 安全模型：

### 11.1 seL4 核心安全原则

| seL4 原则 | 描述 | agentrt-liunx 实现 |
|-----------|------|-------------------|
...
agentrt-liunx（AirymaxOS）在 seL4 基础上增加了：

1. **能力生存时间（TTL）**：seL4 的能力无过期机制，agentrt-liunx 支持 TTL，自动过期
2. **能力审计链**：每次能力派生和撤销都记录到审计日志
3. **五级沙箱模型**：seL4 只有进程隔离，agentrt-liunx 提供五级隔离
...
4. [agentrt-liunx（AirymaxOS）安全设计](../../110-security/README.md)
5. [agentrt-liunx（AirymaxOS）工程思想](../../50-engineering-standards/04-engineering-philosophy.md)
