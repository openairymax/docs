...
> **文档定位**: AirymaxOS（agentrt-linux）的全部设计思想、架构设计、子仓设计草案
...
AirymaxOS 是基于 Linux 内核的**操作系统发行版**，与 agentrt（AirymaxAgentRT / 极境智能体运行底座平台工程）**同源**。

**同源关系**:

- agentrt = 跨平台用户态运行时（Linux/macOS/Windows）
- AirymaxOS = 基于 Linux 内核的发行版（专为 Agent 工作负载优化）
- 两者共享 Airymax 设计理念（MicroCoreRT / AgentsIPC / Cupolas / MemoryRovol / CoreLoopThree 的语义）
- agentrt 在 AirymaxOS 上运行**天然更稳健和适配**（架构同源，无适配层）

## 2. 设计三大支柱

| 支柱 | 思想 | 理论根基 |
| --- | --- | --- |
| **微内核设计思想** | 最小化特权态代码、服务用户态化、消息传递通信、capability 安全 | seL4 / Zircon / Minix3 |
| **Linux 6.6 内核基线** | 基于 Linux 6.6 内核，sched_ext + eBPF + io_uring + Rust 微内核化改造 | Linux 内核工程基线 |
...
| - | --- | --- | --- | --- |
| 1 | [kernel](10-architecture/01-system-architecture.md) | 极境内核 | Linux 内核 + 微内核化改造（sched_ext + eBPF + io_uring + Rust） | atoms/corekern (MicroCoreRT) |
...
所有 AirymaxOS 仓库归属 `openairymax` 组织，托管于 atomgit.com：
...
| 版本 | AirymaxOS 范围 | agentrt 范围 |
| --- | --- | --- |
| **0.1.1** | 仅仓库占位（README + 设计草案 + 工程基线声明） | 全部三大支柱（奠基 + 29 仓拆分 + 生产就绪） |
| **1.0.1** | 内核和 OS 实际开发 | 与 AirymaxOS 协同验证 |

## 7. 前沿理论参考（2026-07-06 最新）

| 前沿理论 | 应用到子仓 |
| --- | --- |
| seL4 微内核（形式化验证、capability） | airymaxos-kernel + airymaxos-security |
...
| sched_ext（eBPF 用户态调度器、sub-scheduler） | airymaxos-kernel（SCHED_AGENT） |
| io_uring（零 syscall 高性能 I/O） | airymaxos-kernel + airymaxos-services |
...
agentrt（AirymaxAgentRT，跨平台用户态运行时）
   │
   └── 运行在 AirymaxOS 上: 天然更稳健和适配（同源）
       │
       ├── agentrt 的 MicroCoreRT 调度 ←→ AirymaxOS 的 SCHED_AGENT 调度类（语义同源）
       ├── agentrt 的 AgentsIPC 协议 ←→ AirymaxOS 的 IPC 子系统（128B 消息头同源）
       ├── agentrt 的 Cupolas 安全 ←→ AirymaxOS 的 capability + LSM（模型同源）
       ├── agentrt 的 MemoryRovol ←→ AirymaxOS 的 记忆子系统（记忆模型同源）
       └── agentrt 的 CoreLoopThree ←→ AirymaxOS 的 认知 kthread（循环模型同源）
```

**同源红利**: agentrt 的设计假设和 AirymaxOS 的实现假设一致，agentrt 在 AirymaxOS 上运行**无适配层**，天然契合，更稳健。

## 9. 相关文档

- RT 设计文档: `docs-closed/`（0.1.1 三大文档）
- OS 设计文档: `docs/AirymaxAgentOS/`（本目录）
- 工程规范: `docs-closed/0.1.1工程标准规范手册.md`（含 IRON-9 同源但独立）

---

> **文档结束** | AirymaxOS 0.1.1 P0 文档体系
