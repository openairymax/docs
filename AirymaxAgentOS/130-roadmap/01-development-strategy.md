...
> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）开发策略与三大支柱详解
...
AirymaxOS 采用**"工程标准先行 + 模块化并行 + 渐进式发布"**的开发策略，与 agentrt 0.1.1 奠基版本并行（agentrt 在前，AirymaxOS 在后）：

| 策略要素 | 含义 | 体现 |
|---------|------|------|
| **工程标准先行** | 工程标准规范在所有代码之前完成；规范是代码的"宪法" | Part 1 → 其余 Part |
| **模块化并行** | 架构、测试、可观测、安全、治理可并行推进 | Part 2-6 并行 |
| **渐进式发布** | 0.1.1 占位 → 1.0.1 开发 → 1.1.x 协同；每阶段可验证 | 双版本策略 |
...
graph LR
    ...
    subgraph OS[AirymaxOS]
        OS1[0.1.1 占位<br/>README + 工程标准]
...
agentrt 0.1.1 先完成奠基（atoms 解耦、CoreKern 精简、taskflow 修复、SDK 4 语言、许可证合规、29 仓拆分、生产就绪），随后 AirymaxOS 1.0.1 基于 agentrt 同源语义开始内核与 OS 开发，两端在 1.0.1 阶段协同验证。
...
AirymaxOS 开发方案由三大支柱构成。三大支柱对应"标准 → 设计 → 治理"的递进关系。
...
**与 agentrt 工程标准的关系（IRON-9 同源但独立）**:

- **同源**: 与 agentrt `0.1.1工程标准规范手册.md`（v28.0）共享 17 类规则编号体系骨架（IRON/BAN/STD/ACC 等）；共享五维正交 24 原则作为顶层设计哲学；共享 E-7 文档即代码、E-6 错误可追溯等核心工程观。
- **独立**: AirymaxOS 是 OS 发行版，工程标准必须独立处理内核 ABI 稳定性、内核内部 API 不稳定性、补丁生命周期、维护者层级制度等 agentrt 不涉及的领域；同时为内核态特有场景（驱动模型、构建系统、可观测性、安全 LSM）制定专门规范。
...
| 改造方向 | 当前（Linux 6.6） | 目标（AirymaxOS 1.0.1） |
|---------|-------------------|------------------------|
...
AirymaxOS 开发方案拆分为 9 个 Part。以下详述每个 Part 的范围、优先级、依赖与工时。
...
| **工时** | 240h |
| **里程碑** | M0（2 周） |
| **完成标准** | 8 文档完成 + OS 工程规则编号体系注册表建立 + 与 agentrt IRON-9 同源但独立关系明确 |
...
AirymaxOS 与 agentrt 是**同源**关系，两端通过同源 API 实现无适配层互操作：
...
graph TB
    ...
    subgraph AirymaxOS[AirymaxOS OS 发行版]
        O1[SCHED_AGENT 调度类]
        O2[IPC 子系统]
        O3[capability + LSM]
        O4[记忆子系统]
    ...
    style AirymaxOS fill:#e3f2fd
```

### 4.2 同源红利

agentrt 的设计假设和 AirymaxOS 的实现假设一致，agentrt 在 AirymaxOS 上运行**无适配层**，天然契合：

| 同源点 | agentrt 侧 | AirymaxOS 侧 | 互操作效果 |
|--------|-----------|--------------|-----------|
...
| 阶段 | agentrt | AirymaxOS | 协同内容 |
|------|---------|-----------|---------|
| 0.1.1 | 完成奠基 + 29 仓拆分 + 生产就绪 | 文档体系完成（~64 文档，不含内核/OS 实施） | AirymaxOS 同步工程标准语义 |
| 1.0.1 | 与 AirymaxOS 协同验证 | 完成 M0-M8 全部里程碑 | 同源 API 端到端验证 |
| 1.1.x | 在 AirymaxOS 上天然适配运行 | 稳定运行 | 无适配层互操作 |
...
AirymaxOS 工程标准与 agentrt 工程标准遵循 IRON-9 同源但独立原则：

- **同源**: 共享五维正交 24 原则、17 类规则编号体系骨架、核心工程观（E-7 文档即代码、E-6 错误可追溯、E-1 安全内生）。
- **独立**: AirymaxOS 独立处理内核 ABI 稳定性、补丁生命周期、维护者层级制度；新增 OS 专属编号前缀（OS-KER / OS-DRV / OS-BUILD / OS-TEST / OS-OBS / OS-SEC / OS-ABI）。
- **互操作**: 两端通过同源 API（MicroCoreRT / AgentsIPC / Cupolas / MemoryRovol / CoreLoopThree）实现无适配层互操作。

### 5.2 渐进式开发（S-4 涌现性管理）

- **渐进式发布**: 0.1.1 占位 → 1.0.1 开发 → 1.1.x 协同；每阶段可验证、可回退。
...
AirymaxOS 开发策略是五维正交 24 原则在"开发方法"维度的具体落地：
...
AirymaxOS 开发策略以三大支柱（工程标准 + 架构设计 + 开发治理）为骨架，以 9 个 Part（P0 七部分 + P1 两部分）为执行单元，以 9 个里程碑（M0-M8）为节奏节点，以五维正交 24 原则为设计哲学，以 IRON-9 同源但独立原则处理与 agentrt 的协同关系。
...
- `docs-closed/0.1.1工程标准规范手册.md` — 17 类规则编号体系（v28.0，含 IRON-9 同源但独立）
