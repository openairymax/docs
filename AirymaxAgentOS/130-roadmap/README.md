...
# AirymaxOS 开发详细方案（路线图）

> **文档定位**: AirymaxOS（agentrt-linux，极境智能体操作系统）开发详细方案的主索引与总纲
...
### 1.1 本模块在 AirymaxOS 文档体系中的位置

`130-roadmap/` 是 AirymaxOS 19 个文档模块中的第 13 个，定位为**开发详细方案的总索引**：

- **50-engineering-standards/** 定义"代码该怎么写"——工程标准与规范
- **130-roadmap/**（本模块）定义"什么时候写什么代码"——开发策略、里程碑与时间线

工程标准是"语法"，路线图是"时序"。两者共同回答 AirymaxOS 从"占位仓库"演进到"可投产发行版"的完整路径。

### 1.2 与 agentrt 0.1.1 / 1.0.1 的关系

AirymaxOS 与 agentrt（AirymaxAgentRT，跨平台用户态运行时）**同源**：

| 版本 | agentrt 范围 | AirymaxOS 范围 |
|------|--------------|----------------|
| 0.1.1 | 全部三大支柱（奠基 + 29 仓拆分 + 生产就绪，约 3,414.5h） | **文档体系完成**（工程标准 8 + 架构 6 + 模块 9 + 接口 5 + 数据流 4 + P0 模块 21 + 路线图 7，共 ~64 文档；不含内核/OS 实施） |
| 1.0.1 | 与 AirymaxOS 协同验证 | 内核和 OS 实际开发（本文档定义的 M0-M8 全部里程碑） |

agentrt 在前（0.1.1 已完成奠基），AirymaxOS 在后（1.0.1 基于 agentrt 同源语义开发）。两端通过 MicroCoreRT / AgentsIPC / Cupolas / MemoryRovol / CoreLoopThree 同源 API 实现无适配层互操作。
...
AirymaxOS 开发方案由三大支柱构成，分别对应工程标准、架构设计、开发治理三类工作：

| 支柱 | 范围 | 0.1.1 | 1.0.1 |
|------|------|-------|-------|
| **工程标准与规范** | `50-engineering-standards/`（8 文档） | 占位（全部完成） | 实施 |
| **架构与模块设计** | `10-architecture/` + `20-modules/` + `60-driver-model/` + `70-build-system/` | 占位（README） | 实施 |
| **开发流程与治理** | `120-development-process/`（9 文档）+ `50-engineering-standards/07` | 占位（README） | 实施 |
...
AirymaxOS 开发方案拆分为 9 个 Part，按优先级分两层（P0 / P1）：
...
- **P0（必修）**：Part 1-7，覆盖工程标准、架构、测试、可观测、安全、治理、路线图。P0 是 AirymaxOS 1.0.1 可投产的前提。
...
AirymaxOS 采用"占位 → 开发"两阶段版本策略，与 agentrt 的版本节奏对齐：

| 版本 | AirymaxOS 范围 | agentrt 范围 | AirymaxOS 工时 |
|------|----------------|--------------|----------------|
| **0.1.1** | 文档体系完成（~64 文档：工程标准 8 + 架构 6 + 模块 9 + 接口 5 + 数据流 4 + P0 模块 21 + 路线图 7 + 根 README 等） | 全部三大支柱（奠基 + 29 仓拆分 + 生产就绪） | ~150h（文档编写） |
| **1.0.1** | 内核和 OS 实际开发（M0-M8 全部里程碑） | 与 AirymaxOS 协同验证 | ~2,750h |

### 5.2 0.1.1 版本范围（文档体系完成）

0.1.1 是 AirymaxOS 的"文档体系奠基版本"，目标是完成全部设计文档与工程标准框架（不含内核/OS 代码实施）：
...
AirymaxOS 路线图是 Airymax 五维正交 24 原则在"开发时序"维度的具体落地：
...
AirymaxOS 路线图与 agentrt 路线图遵循 **IRON-9 v2 同源且部分代码共享**原则：

- **同源**：共享五维正交 24 原则作为顶层设计哲学；共享 17 类规则编号体系骨架（IRON/BAN/STD/ACC 等）；共享 E-7 文档即代码、E-6 错误可追溯等核心工程观。
- **独立**：AirymaxOS 是 OS 发行版，承担内核态严肃性责任，路线图必须独立处理内核 ABI 稳定性、补丁生命周期、维护者层级制度等 agentrt 不涉及的领域。
- **互操作**：agentrt 在 AirymaxOS 上运行时，两端通过同源 API（MicroCoreRT 调度、AgentsIPC 128B 消息头、Cupolas 安全模型）实现无适配层互操作。

### 8.2 同源 API 映射

| agentrt 概念 | AirymaxOS 对应 | 同源语义 |
|--------------|----------------|----------|
| MicroCoreRT（atoms/corekern） | airymaxos-kernel（SCHED_AGENT 调度类） | 调度语义同源 |
...
```mermaid
timeline
    title AirymaxOS 与 agentrt 版本协同时序
    0.1.1 阶段 : agentrt 完成奠基 + 29 仓拆分 + 生产就绪
              : AirymaxOS 文档体系完成（~64 文档，不含内核/OS 实施）
    1.0.1 阶段 : agentrt 与 AirymaxOS 协同验证
              : AirymaxOS 完成 M0-M8 全部里程碑
    1.1.x 阶段 : agentrt 在 AirymaxOS 上天然适配运行（无适配层）
```
