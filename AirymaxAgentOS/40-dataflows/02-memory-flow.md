...
# agentrt-liunx（AirymaxOS）记忆卷载数据流

> **文档定位**: agentrt-liunx（AirymaxOS）记忆卷载数据流的详细设计，刻画 L1→L4 四层递进与 CXL/PMEM/MGLRU 硬件协同
...
记忆卷载数据流是 agentrt-liunx 区别于通用操作系统的核心特征之一，落地于 `airymaxos-memory` 子仓（同源 agentrt memoryrovol + heapstore 模块）。该数据流借鉴认知科学的记忆分层理论，将记忆从原始感知到抽象模式分为四层递进：
...
- **[SC] 共享契约层**（`include/airymax/memory_types.h`）：L1-L4 数据结构定义（`agentrt_l1_record_t` 等）、GFP 掩码语义、PMEM 持久化接口——agentrt 用户态与 agentrt-liunx 内核态**完全共享代码**
- **[SS] 语义同源层**：MGLRU aging/eviction API、`memory.reclaim` 主动回收、userfaultfd 记忆迁移——agentrt 用户态与 agentrt-liunx 内核态**API 签名同源，实现独立**
- **[IND] 完全独立层**：CXL 驱动、PMEM nvdimm 驱动、Kbuild、`KABI_RESERVE` 移除——**各自独立实现**
...
CXL（Compute Express Link）内存池化（FR-035）是 agentrt-liunx 超节点 OS 的核心能力，支持跨节点内存共享。同源 agentrt atoms/corekern 的 CXL 池化扩展。
...
MGLRU（Multi-Gen LRU）是 Linux 6.6 内核基线的原生能力（FR-037），相比传统 LRU 内存回收效率提升 > 20%（NFR-P-006）。agentrt-liunx 利用 MGLRU 实现 L2/L3 内存的分级回收——MGLRU 的代际模型（aging/eviction）与 MemoryRovol 遗忘机制语义同源（[SS] 语义同源层）。

### 5.1 MGLRU 核心数据结构（Linux 6.6 内核基线）

Linux 6.6 内核基线在 `include/linux/mmzone.h` 定义了 MGLRU 的核心数据结构 `struct lru_gen_folio`。agentrt-liunx MemoryRovol L2/L3 遗忘机制与此语义同源：
...
Linux 6.6 内核基线提供 `memory.reclaim` cgroup v2 接口，允许用户态主动触发指定 memcg 的内存回收。agentrt-liunx 利用此机制实现 per-agent 主动回收——当 agent 实例 token 配额告警时，daemon 写入 `memory.reclaim` 触发该 agent 的 memcg 回收。
...
| **[SS] 语义同源层** | API 签名同源，实现独立 | MGLRU aging/eviction、`memory.reclaim`、userfaultfd | agentrt 用户态实现 + agentrt-liunx 内核态实现 |
| **[IND] 完全独立层** | 完全独立 | CXL 驱动、PMEM nvdimm 驱动、Kbuild、`KABI_RESERVE` 移除 | 各自独立仓库 |

### 10.2 [SC] 共享契约层 `include/airymax/memory_types.h`

[SC] 共享契约层定义 agentrt 用户态与 agentrt-liunx 内核态**完全共享的代码**——L1-L4 数据结构、GFP 掩码语义、PMEM 持久化接口。这避免双份定义导致不一致。
...
> **OS-MM-001**: `include/airymax/memory_types.h` 是 MemoryRovol 数据结构的 [SC] 共享契约层——agentrt 用户态与 agentrt-liunx 内核态通过此头文件共享 L1-L4 数据结构定义、GFP 掩码语义、PMEM 持久化接口，避免双份定义导致不一致。

### 10.3 [SS] 语义同源层

[SS] 语义同源层定义 agentrt 用户态与 agentrt-liunx 内核态**API 签名同源、实现独立**的接口——MGLRU aging/eviction 语义、`memory.reclaim` 主动回收、userfaultfd 记忆迁移。

| Linux 6.6 内核基线 API | agentrt-liunx 对应 | agentrt 用户态对应 | 同源说明 |
|------------------------|----------------|-------------------|----------|
...
[IND] 完全独立层定义 agentrt-liunx 内核态独有、agentrt 用户态不实现的设计——CXL 驱动、PMEM nvdimm 驱动、Kbuild、`KABI_RESERVE` 移除。

| agentrt-liunx 内核态特性 | 独立原因 |
|---------------------|----------|
| CXL 驱动（`drivers/cxl/`） | 硬件驱动层，agentrt 用户态不涉及 |
| PMEM nvdimm 驱动（`drivers/nvdimm/`） | 硬件驱动层，agentrt 用户态不涉及 |
| Kbuild 配置 | 内核构建系统，agentrt 不需要 |
| `KABI_RESERVE` 字段移除 | 与 IRON-1 唯一奠基版本策略冲突，全部移除 |
| `lru_gen_memcg` 全局 memcg LRU | per-agent 隔离由 airymaxos-services daemon 管理 |
| Bloom 过滤器双缓冲 | 1.0.1 后按需引入 |

### 10.5 跨态协作流

MemoryRovol 数据流涉及 agentrt 用户态与 agentrt-liunx 内核态的协作：
...
sequenceDiagram
    ...
    participant KR as agentrt-liunx 内核态
...
| 检查项 | agentrt 状态 | agentrt-liunx 状态 | 一致性 | 备注 |
|--------|--------------|----------------|--------|------|
...
| `KABI_RESERVE` | 不适用（用户态） | [IND] 全部移除 | ✅ 合理 | 与 IRON-1 冲突 |

### 12.3 agentrt 合理性验证结论

经过全面推理与系统验证，**agentrt memoryrovol/heapstore 的设计在 [SC]/[SS] 层与 agentrt-liunx 内核态保持一致**，未发现不合理之处：
