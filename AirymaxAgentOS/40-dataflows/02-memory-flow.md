...
- **[SC] 共享契约层**（`include/airymax/memory_types.h`）：L1-L4 数据结构定义（`agentrt_l1_record_t` 等）、GFP 掩码语义、PMEM 持久化接口——agentrt 用户态与 AirymaxOS 内核态**完全共享代码**
- **[SS] 语义同源层**：MGLRU aging/eviction API、`memory.reclaim` 主动回收、userfaultfd 记忆迁移——agentrt 用户态与 AirymaxOS 内核态**API 签名同源，实现独立**
...
L1 存储原始执行记录与感知数据，**仅追加（append-only），不可变**（FR-031，NFR-S-005 哈希链保护）。L1 数据结构 `agentrt_l1_record_t` 是 [SC] 共享契约层的核心——agentrt 用户态与 AirymaxOS 内核态通过 `include/airymax/memory_types.h` 共享此定义。
...
MGLRU（Multi-Gen LRU）是 Linux 6.6 内核基线的原生能力（FR-037），相比传统 LRU 内存回收效率提升 > 20%（NFR-P-006）。AirymaxOS 利用 MGLRU 实现 L2/L3 内存的分级回收——MGLRU 的代际模型（aging/eviction）与 MemoryRovol 遗忘机制语义同源（[SS] 语义同源层）。

### 5.1 MGLRU 核心数据结构（Linux 6.6 内核基线）

Linux 6.6 内核基线在 `include/linux/mmzone.h` 定义了 MGLRU 的核心数据结构 `struct lru_gen_folio`。AirymaxOS MemoryRovol L2/L3 遗忘机制与此语义同源：
...
Linux 6.6 内核基线提供 `memory.reclaim` cgroup v2 接口，允许用户态主动触发指定 memcg 的内存回收。AirymaxOS 利用此机制实现 per-agent 主动回收——当 agent 实例 token 配额告警时，daemon 写入 `memory.reclaim` 触发该 agent 的 memcg 回收。
...
| **[SS] 语义同源层** | API 签名同源，实现独立 | MGLRU aging/eviction、`memory.reclaim`、userfaultfd | agentrt 用户态实现 + AirymaxOS 内核态实现 |
| **[IND] 完全独立层** | 完全独立 | CXL 驱动、PMEM nvdimm 驱动、Kbuild、`KABI_RESERVE` 移除 | 各自独立仓库 |

### 10.2 [SC] 共享契约层 `include/airymax/memory_types.h`

[SC] 共享契约层定义 agentrt 用户态与 AirymaxOS 内核态**完全共享的代码**——L1-L4 数据结构、GFP 掩码语义、PMEM 持久化接口。这避免双份定义导致不一致。
...
> **OS-MM-001**: `include/airymax/memory_types.h` 是 MemoryRovol 数据结构的 [SC] 共享契约层——agentrt 用户态与 AirymaxOS 内核态通过此头文件共享 L1-L4 数据结构定义、GFP 掩码语义、PMEM 持久化接口，避免双份定义导致不一致。

### 10.3 [SS] 语义同源层

[SS] 语义同源层定义 agentrt 用户态与 AirymaxOS 内核态**API 签名同源、实现独立**的接口——MGLRU aging/eviction 语义、`memory.reclaim` 主动回收、userfaultfd 记忆迁移。

| Linux 6.6 内核基线 API | AirymaxOS 对应 | agentrt 用户态对应 | 同源说明 |
|------------------------|----------------|-------------------|----------|
...
[IND] 完全独立层定义 AirymaxOS 内核态独有、agentrt 用户态不实现的设计——CXL 驱动、PMEM nvdimm 驱动、Kbuild、`KABI_RESERVE` 移除。

| AirymaxOS 内核态特性 | 独立原因 |
|---------------------|----------|
...
MemoryRovol 数据流涉及 agentrt 用户态与 AirymaxOS 内核态的协作：
...
sequenceDiagram
    ...
    participant KR as AirymaxOS 内核态
...
| 检查项 | agentrt 状态 | AirymaxOS 状态 | 一致性 | 备注 |
|--------|--------------|----------------|--------|------|
...
经过全面推理与系统验证，**agentrt memoryrovol/heapstore 的设计在 [SC]/[SS] 层与 AirymaxOS 内核态保持一致**，未发现不合理之处：
