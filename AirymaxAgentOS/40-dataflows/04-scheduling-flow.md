...
agentrt 的 MicroCoreRT 调度语义与 agentrt-liunx 的 SCHED_AGENT 调度类一致：
...
agentrt-liunx 的内核原生 IPC（airymaxos-kernel + airymaxos-services）:
  - 内核原生支持 128 字节消息头（同源 [SC]）
  - magic 0x41524531 作为内核识别码（同源 [SC]）
  - agentrt_task_desc_t 直接作为内核调度描述符（同源 [SC]）

→ agentrt 在 agentrt-liunx 上运行时，调度消息无需任何转换层，天然契合
```
