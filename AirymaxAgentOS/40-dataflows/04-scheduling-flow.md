...
2. **sched_ext**（FR-001, FR-002）：agentrt-liunx 内核增强（主线 6.12+），基于 eBPF 的可插拔调度框架。agentrt-liunx 通过 sched_ext 实现 SCHED_AGENT 调度类，调度策略以 BPF 程序形式加载，支持运行时替换（FR-050）。
3. **SCHED_AGENT 调度类**（FR-002）：agentrt-liunx 自研调度类，专为 Agent 工作负载优化。优先级范围 0-139，支持抢占式调度与策略可插拔。
...
SCHED_AGENT 是 agentrt-liunx 基于 sched_ext 实现的自研调度类（FR-002），专为 Agent 工作负载优化。同源 agentrt atoms/corekern 的 MicroCoreRT 调度器。
 ...
 * @since 1.0.1
 * @see sched_ext（agentrt-liunx 内核增强，主线 6.12+）
 * @note IRON-9 v2 [SC] 共享契约层：agentrt 与 agentrt-liunx 共享此定义
 */
...
agentrt 用户态 MicroCoreRT 调度语义与 agentrt-liunx SCHED_AGENT 调度类、AgentsIPC 与内核原生 IPC 均共享 [SC] 契约层，运行于 agentrt-liunx 上天然契合无需适配层：
...
| 调度类编号 | 不涉及内核调度类 | `SCHED_AGENT = 7` | [SC] |
| IPC 消息头 | 128 字节定长 + magic `0x41524531 'ARE1'` | 内核原生支持同源 | [SC] |
| 调度 payload | `agentrt_task_desc_t` 直接承载 | 直接作为内核调度描述符 | [SC] |

**同源红利**：agentrt 作为普通用户态应用（libc/POSIX）在 agentrt-liunx 上天然更稳健；可选 `sched_setscheduler(p, SCHED_AGENT, &param)` 享受内核原生调度优先级，无需任何转换/适配层。
