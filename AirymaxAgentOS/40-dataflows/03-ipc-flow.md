...
> **文档定位**: AirymaxOS IPC 消息流的详细设计，刻画 io_uring 零拷贝、SQ/CQ ring 机制与 128B 消息头生命周期
...
IPC 消息流是 AirymaxOS 进程间通信的核心数据流，落地于 `airymaxos-kernel` 子仓，并贯穿 `airymaxos-services` 的 12 daemons。该数据流基于 Linux 6.6 内核基线原生的 io_uring 子系统实现零拷贝、零 syscall 的高性能消息传递。
...
AirymaxOS IPC 基于 Linux 6.6 内核基线原生的 io_uring 子系统实现。本节描述 io_uring 的核心机制，作为 IPC 消息流的底层传输基础。
...
> **OS-IPC-001**: SQ/CQ ring 的单生产者-单消费者所有权契约是 io_uring 无锁设计的基石。AirymaxOS IPC 必须严格遵守——daemon 只写 `sq.tail` + `cq.head`，内核只写 `sq.head` + `cq.tail`，避免任何一方越权导致竞态。
...
| `src_pid` / `dst_pid` | AirymaxOS 扩展（io_uring 无此字段） | [IND] 独立扩展 |
...
AirymaxOS daemon 间高频 IPC 采用 SQPOLL 模式——内核创建专用 `io_sq_thread` 线程持续轮询 SQ ring，应用只需写入 SQE + 推进 `sq.tail`，无需任何 syscall。
...
> **OS-IPC-002**: SQPOLL 模式下 `sq_thread_idle` 超时是平衡延迟与 CPU 占用的关键。AirymaxOS 推荐：高频 IPC 场景设为 5-10s（避免频繁睡眠），低延迟场景设为 100ms（快速响应）。
...
> **OS-IPC-003**: DEFER_TASKRUN + SINGLE_ISSUER 组合是 AirymaxOS daemon 间 IPC 的推荐配置——单线程提交 + 单线程完成，避免跨 CPU IPI，最大化缓存局部性。此为 [SS] 语义同源层。
...
> **OS-IPC-004**: registered buffer 的 `bvec[]` scatter-gather 模型允许非连续物理页的零拷贝传输。AirymaxOS IPC 继承此模型，支持 daemon 间大块 payload（如 1MB STREAM）的零拷贝传递。
...
> **OS-IPC-005**: `IOSQE_BUFFER_SELECT` + `IOSQE_CQE_SKIP_SUCCESS` 组合是 AirymaxOS IPC 高吞吐场景的关键——发送方从预分配 buffer 池选择，成功时不回写 CQE，将 CQE 开销降到最低。
...
AirymaxOS daemon 间 IPC 的核心机制是 `IORING_OP_MSG_RING`——允许一个 io_uring 实例（源 ring）向另一个 io_uring 实例（目标 ring）提交 CQE，无需任何中间路由或 syscall。
...
两个 daemon 互相 MSG_RING 时可能死锁（A 持有 A.uring_lock 等待 B.uring_lock，B 持有 B.uring_lock 等待 A.uring_lock）。AirymaxOS 继承 io_uring 的 trylock + punt 策略：
...
> **OS-IPC-006**: `IORING_OP_MSG_RING` 的 trylock + punt-to-io-wq 死锁避免策略是 AirymaxOS daemon 间双向 IPC 的关键同源机制——两个 daemon 互相提交消息时不能持有自己的 uring_lock 等待对方释放。

> **OS-IPC-007**: `IORING_MSG_SEND_FD` 模式允许 daemon 间传递固定 fd（如共享内存 memfd、socket fd），这是 AirymaxOS 实现跨 daemon 共享内存 IPC 的同源基础——发送方通过 MSG_SEND_FD 传递 memfd，接收方获得固定 fd 后 mmap 共享内存。
...
> **OS-IPC-008**: SEND_ZC 的双 CQE 模型（数据发送完成 + 缓冲区回收）是 AirymaxOS 跨节点 IPC 零拷贝的同源参考——RDMA/CXL 发送后，缓冲区通过独立 CQE 回收，避免 daemon 轮询。
...
AirymaxOS IPC 的 IRON-9 v2 三层共享模型（同源且部分代码共享）落地如下：

### 12.1 [SC] 共享契约层（完全共享代码）

`include/airymax/ipc.h` 头文件库，agentrt 与 AirymaxOS 完全共享：
...
| io_uring 接口 | AirymaxOS IPC 接口 | 同源语义 |
|---------------|-------------------|----------|
...
|------|-------------------|----------|
| io-wq 工作队列 | daemon 独立线程池（基于 MicroCoreRT） | AirymaxOS daemon 走用户态调度 |
| `IORING_SETUP_NO_MMAP` | daemon 主动分配 ring 内存 | daemon 内存走 MemoryRovol 管理 |
| `IORING_REGISTER_RESTRICTIONS` | capability 模型（Cupolas） | AirymaxOS 用 capability 替代 SQE 白名单 |
| io_uring cgroup 集成 | cgroup v2 + AirymaxOS 扩展 | cgroup 策略独立 |
| 跨节点路由 | CXL/RDMA/NVLink 扩展 | AirymaxOS 超节点 OS 专属 |

### 12.4 同源红利示例

**IPC 同源红利**：agentrt AgentsIPC 与 AirymaxOS IPC 在 [SC] 共享契约层完全一致（magic/消息头/5 种 payload 协议/trace_id 语义），使得：

1. **跨系统互通无适配层**：agentrt 用户态进程可直接与 AirymaxOS daemon 通信，消息头格式一致
2. **测试用例共享**：[SC] 层的单元测试可在两个系统间复用
3. **文档同步**：[SC] 层接口变更在两个系统同步更新
4. **实现独立**：agentrt 走用户态 mmap 路径（跨平台），AirymaxOS 走 io_uring 内核加速路径（Linux 专属）
...
| 14 | [IND] cgroup 集成 | 不涉及 | cgroup v2 + AirymaxOS 扩展独立 | ✅ PASS 独立正确 |
| 15 | 跨平台兼容性 | 跨 Linux/macOS/Windows | 仅 Linux（agentrt-linux 专属） | ✅ PASS agentrt 保持跨平台，AirymaxOS 仅 Linux |

**结论**：15 项检查全部 PASS。IPC 数据流与 agentrt AgentsIPC 在 [SC] 共享契约层完全一致（消息头/magic/结构/5 协议/trace_id/entries 6 项），[SS] 语义同源层 API 签名一致，[IND] 独立层正确分离（io-wq/NO_MMAP/RESTRICTIONS/cgroup 内核态专属机制不污染 agentrt）。agentrt 设计无需修改，保持跨平台用户态；AirymaxOS 在 [IND] 独立层正确引入 io_uring 内核加速路径，遵循 IRON-9 v2 同源且部分代码共享原则。
