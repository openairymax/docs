...
| 层次 | 共享程度 | 内容 | 组织方式 |
|------|---------|------|---------|
| **共享契约层**（Shared-Contract） | 完全共享代码 | **4 个 [SC] 头文件清单**（`include/airymax/`）：(1) `bpf_struct_ops.h`——struct_ops 状态机 + common_value；(2) `memory_types.h`——MemoryRovol L1-L4 数据结构 + GFP 掩码语义 + PMEM 持久化接口；(3) `security_types.h`——POSIX capability 38 ID + LSM 钩子 254 ID + Cupolas blob 布局 + capability 派生模型 + Vault backend + 策略裁决 4 值枚举；(4) `cognition_types.h`——CoreLoopThree 阶段枚举 + Thinkdual 模式枚举 + LLM 推理阶段枚举 + 上下文结构 + Token 能效指标 + GPU/NPU 描述符。另含 IPC 消息头结构（`agentrt_ipc_msg_hdr_t` + magic=0x41524531/version/type 常量）、syscall 编号（`AGENTRT_SYS_*`）、capability 令牌格式、错误码（`agentrt_error_t`）、规则编号体系（IRON/BAN/STD/ACC）、五维正交 24 原则 | `include/airymax/` 独立头文件库，两端共同依赖 |
...
- **互操作**：agentrt 在 agentrt-liunx 上运行时，两端工程标准形成天然互补——agentrt 遵循其用户态运行时规范（如 `agentrt_log_write` 日志），agentrt-liunx 遵循其内核发行版规范（如 `printk` 等级），两端通过**共享契约层代码**（如 `include/airymax/ipc_msg.h` 的 IPC 消息头定义）实现无适配层互操作。
...
- **agentrt ↔ agentrt-liunx**：[SC] 共享契约层完全共享代码（`include/airymax/` 4 个头文件）
