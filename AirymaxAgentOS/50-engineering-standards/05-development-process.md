...
# agentrt-liunx（AirymaxOS）开发流程标准

> **文档定位**: agentrt-liunx（AirymaxOS，极境智能体操作系统）工程标准规范第 5 卷——开发流程。本卷规定从设计构想进入主线、稳定版到长期维护的完整生命周期，以及维护者层级制度、补丁格式、审查响应与稳定版规则。
> **版本**: 1.0.1（开发）
> **最后更新**: 2026-07-06
...
本卷是 agentrt-liunx 工程标准 7 主题文档中的第 5 卷，回答"代码怎么进入主线"这一问题。它在 04 工程思想（双层稳定性、策略机制分离）与 06 工具链自动化（7 层验证、CI/CD）之间形成承上启下的桥梁：
...
| MicroCoreRT | Airymax 微核心运行时基座（Minimal Core Runtime），agentrt-liunx 内核态对其保持同源语义 |
...
agentrt-liunx 继承 Linux 6.6 内核基线的 6 阶段补丁生命周期模型，并将其适配到 GitHub PR 流程。每个阶段有明确的输入、输出、责任人和 SLA。
...
- **目标审查者**：由 `MAINTAINERS` 文件（agentrt-liunx 等价物，详见 07 卷）和 `CODEOWNERS` 自动识别。
...
- **合并窗口**：参照 Linux Merge Window 模型，agentrt-liunx 采用 2 周 Merge Window + 6 周 RC 周期的发布节奏（详见 130-roadmap）。
- **RC 期间仅接受修复**：RC1 发布后，新特性补丁必须等待下一个 Merge Window。
- **OS-STD-107**：合并入 `main` 的补丁必须包含至少 1 个 `Reviewed-by:` 标签（来自非作者维护者）。
- **OS-STD-108**：合并入 `main` 的补丁若修改 AgentsIPC 128B 消息头布局，必须由 agentrt-liunx 协议委员会额外签字（详见 30-interfaces）。
...
- **"the development community remembers developers who lose interest in their code after it's merged"**——这是 Linux 内核社区的明确警告，agentrt-liunx 完全继承此原则。
...
agentrt-liunx 继承 Linux 内核的 Lieutenant System（副手系统）——一条从普通贡献者到总维护者的信任链。
...
- agentrt-liunx 的 8 子仓（kernel/services/security/memory/cognition/clouds/system/tests）各设 1 名顶级子系统维护者，其下可有 2-3 层子系统维护者。
...
### 2.4 agentrt-liunx 适配：从 git send-email 到 GitHub PR

Linux 内核使用邮件列表 + `git send-email` 流程，agentrt-liunx 将其适配为 GitHub PR 流程，但保留同源语义：
...
- `develop` 分支是 agentrt-liunx 的预览集成分支，等价 Linux 的 linux-next 树。
...
Linux 内核历史上由 Andrew Morton 维护的 -mm 树，承担"无明确子系统归属的补丁"。agentrt-liunx 等价物为 `airymax-mm` 分支：
...
Linux 内核的 `drivers/staging/` 是质量未达标的代码暂存地。agentrt-liunx 等价物为 `staging/` 目录与 `feature/staging-*` 分支：
...
### 3.4 agentrt-liunx 专属：feature/* 分支命名规范
...
agentrt-liunx 继承 Linux 内核的"一补丁一逻辑"原则，并增加量化约束：
...
- **不要过度拆分**：曾有人单文件提交 500 补丁，被社区嫌弃为噪音；agentrt-liunx 设定单 PR 上限。
...
agentrt-liunx 采用与 Linux 内核同源的补丁格式，适配为 GitHub commit message 规范：
...
补丁描述必须说服审查者"这个问题值得修复"。agentrt-liunx 强制以下要素：
...
- **引用 commit 必须用至少 12 字符 SHA-1 + 单行摘要**：
  ...
  agentrt-liunx 仓库对象众多，6-8 字符 SHA-1 有碰撞风险，必须使用至少 12 字符。
...
agentrt-liunx 完全继承 Linux 内核审查礼仪，并将其适配到 GitHub：
...
- **忽略审查是致命错误**：在 Linux 内核社区，忽略审查者的开发者会被社区忽略；agentrt-liunx 同样如此。
...
- **"the development community remembers developers who lose interest in their code after it's merged"**——这是 Linux 内核社区的明确警告，agentrt-liunx 完全继承。

### 5.3 审查者声明（Reviewer's Statement of Oversight）

agentrt-liunx 沿用 Linux 内核的 Reviewed-by 标签语义——给出 `Reviewed-by:` 即声明：
...
agentrt-liunx 的 `release/<version>` 分支等价 Linux -stable 树，遵循以下规则：
...
- 安全补丁**不走常规稳定版审查流程**，由 agentrt-liunx 安全团队直接处理（详见 110-security）。
...
Linux 内核社区将部分内核指定为 "long term" 内核，提供长期维护。agentrt-liunx 继承此模型：
...
agentrt-liunx 采用与 agentrt 同源的 6 级贡献者成熟度模型（详见 04-engineering-philosophy.md §10）：
...
继承 Linux `Documentation/networking/netdevices.rst`，agentrt-liunx 网络子系统手册规定：

- 网络设备驱动的注册/注销流程。
- NAPI 轮询接口契约。
- XDP / AF_XDP 在 agentrt-liunx 中的扩展（用于 Agent 高速 IPC）。

### 9.2 SoC 平台手册

继承 Linux `Documentation/devicetree/` 与 SoC 平台手册，agentrt-liunx SoC 平台手册规定：

- 设备树绑定规范。
- SoC 平台移植 checklist。
- MicroCoreRT 在不同 SoC 上的适配指南。

### 9.3 agentrt-liunx 专属：8 子仓各自手册

agentrt-liunx 的 8 子仓各有独立的子系统手册，作为本卷的补丁：
...
- git 是 agentrt-liunx 唯一主导 SCM，所有补丁以 git commit 形式存在。
...
- GitHub 是 agentrt-liunx 的代码托管与审查平台。
- `CODEOWNERS` 自动识别审查者。
...
- agentrt-liunx 等价物为 GitHub Projects 看板，每个子仓一个 Projects。
...
> **文档结束** | agentrt-liunx 工程标准第 5 卷 | 共 13 章节 + 规则编号汇总
