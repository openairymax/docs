...
> **文档定位**: AirymaxOS 工程基线（Engineering Baseline）的完整定义与落地规范
...
**工程基线**（Engineering Baseline）是 AirymaxOS 在工程实践层面统一遵循的最小完备规范集合。它锁定内核版本、架构支持、包管理、安全模型、系统服务、AI 原生能力、治理模式与版本节奏等关键工程维度，为 8 子仓的协同开发、跨团队协作与跨版本演进提供不可动摇的参考基准。
...
| **表述纯净** | 所有文档使用 AirymaxOS 自身工程语言表述，不引入任何外部参考来源措辞 | ACC-OS04 |
| **同源约束** | 与 agentrt 共享设计理念但代码独立，基线落地遵循 IRON-9 | IRON-9 |
| **演进可控** | 基线演进必须通过 ADR 评审，禁止未经评审的基线漂移 | ADR 流程 |

### 1.3 工程基线与三大支柱的关系

AirymaxOS 架构设计建立在三大支柱之上：微内核设计思想、AirymaxOS 工程基线、Airymax 同源性。工程基线作为其中之一，提供"如何对接生态与工程实践"的标准：

- **微内核设计思想** 提供"如何设计内核"的方法论
- **AirymaxOS 工程基线** 提供"如何对接生态、如何表述工程"的基线（本文档）
...
AirymaxOS 1.0.1 锁定 **Linux 6.6 内核基线** 作为系统基础。内核版本选择直接影响硬件兼容性、生态兼容性、新特性支持与长期支持周期。

| 基线版本 | Linux 内核 | AirymaxOS 定位 |
|----------|------------|-----------------|
...
Linux 6.6 内核基线提供以下原生特性，全部纳入 AirymaxOS 工程基线：

| 特性 | 内核版本 | 来源 | AirymaxOS 落地 |
|------|----------|------|----------------|
...
| sched_ext（SCHED_AGENT） | AirymaxOS 内核增强 | 主线 6.12+ | airymaxos-kernel 用户态调度 |

### 2.3 内核增强策略

对于主线 6.12+ 才稳定但 AirymaxOS 1.0.1 必需的特性（如 sched_ext），采用 **AirymaxOS 内核增强** 策略引入，而非等待主线 6.6 原生支持：

```
Linux 6.6 原生特性（EEVDF / Rust / MGLRU / XFS fsck / eBPF kfunc / io_uring）
    +
AirymaxOS 内核增强（sched_ext 等，主线 6.12+ 特性向前移植）
    =
AirymaxOS 1.0.1 完整内核基线
```

**增强边界**：仅增强 AirymaxOS 微内核化改造与 AI 原生能力所必需的特性，不引入与基线目标无关的主线特性。
...
| 增强透明 | 所有 AirymaxOS 内核增强必须标注"主线 X.Y+ 特性增强"，禁止伪称为原生 |
...
AirymaxOS 采用 **子仓治理组**（Sub-repository Governance Group）模式组织工程协作。每个子仓对应一个治理组，负责技术决策与代码审查：

| AirymaxOS 治理组 | 对应子仓 | 核心职责 |
|------------------|----------|----------|
...
AirymaxOS 作为 Agentic OS，AI 原生能力是工程基线的核心组成。以下能力构成 AirymaxOS 1.0.1 ~ 2.0+ 的 AI 原生演进路线。

### 5.1 认知中枢系统

**AirymaxOS 认知中枢**（Cognitive Hub）是 airymaxos-cognition 子仓的核心能力，对应 agentrt CoreLoopThree 同源语义：
...
**AirymaxOS 落地**：airymaxos-cognition 实现 AirymaxOS 认知中枢，与 agentrt 的 CoreLoopThree 同源，作为内核态认知运行时。

### 5.2 超节点 OS

**AirymaxOS 超节点 OS** 是 airymaxos-cloudnative 子仓的核心能力，支持大规模 Agent 容器化部署：
...
**AirymaxOS 落地**：airymaxos-cloudnative 实现超节点 OS，支持大规模 Agent 容器化部署与跨节点低时延协同。

### 5.3 超节点沙箱

**AirymaxOS 超节点沙箱** 是 airymaxos-cognition 子仓的 Agent 快速冷启动能力：
...
**AirymaxOS 落地**：airymaxos-cognition 实现超节点沙箱，支持 Agent 快速冷启动，与 Wasm 3.0 沙箱运行时集成（ADR-008）。

### 5.4 Token 能效框架

**AirymaxOS Token 能效框架**（Token Energy Efficiency Framework）实现 Token 消耗的边际递减：
...
**AirymaxOS 落地**：airymaxos-cognition 实现 Token 能效优化，让 Token 消耗走向边际递减，支撑 Agentic OS 的成本可控性。

### 5.5 具身智能 Claw

**AirymaxOS 具身智能** 支持 Agent 与物理世界的交互：
...
**AirymaxOS 落地**：airymaxos-cognition 实现具身智能支持，由 Embedded 治理组协同推进。

### 5.6 Agentic AI 架构演进

AirymaxOS 对标 Agentic OS 三阶段演进路线：

| 阶段 | 名称 | 特征 | AirymaxOS 定位 |
|------|------|------|-----------------|
| 阶段 1 | 资源抽象 | 传统 OS | 不在此阶段 |
| 阶段 2 | 异构融合 | AI 原生 OS | 1.0.1 部分覆盖 |
| 阶段 3 | 意图协同 | Agentic OS | AirymaxOS 直接对标此阶段 |

**AirymaxOS 落地**：AirymaxOS 直接对标第三阶段"意图协同"，作为 Agentic OS，认知中枢 + 超节点 OS + Token 能效框架共同构成意图协同的工程基座。
...
AirymaxOS 工程基线要求所有代码遵循 `docs/Capital_Specifications/coding_standard/` 中的 15 个规范文件，覆盖 C / Rust / Python / Go / TypeScript 等语言。
...
AirymaxOS 工程基线受以下工程铁律约束：

| 铁律 | 内容 |
|------|------|
| IRON-9 | agentrt 和 AirymaxOS 同源但独立，禁止代码共享 |
...
| 文档类型 | AirymaxOS 基线 |
|----------|----------------|
| 技术白皮书 | AirymaxOS 技术白皮书 |
| 安装指南 | AirymaxOS 安装指南 |
| 管理员指南 | AirymaxOS 管理员指南 |
| 开发者指南 | AirymaxOS 开发者指南 |
| API 参考 | docs.airymaxos.org |
| 安全公告 | AirymaxOS SA |

### 9.2 文档层级

AirymaxOS 设计文档采用 19 模块三层体系，与根 [README.md](../README.md) 导航保持一致：
...
- ✅ 兼容 AirymaxOS 超节点 OS 设计（大规模容器低时延通信）
