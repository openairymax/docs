...
# agentrt-liunx（AirymaxOS）kselftest 系统级测试

> **文档定位**: agentrt-liunx（AirymaxOS）测试工程体系第 2 卷——kselftest 用户态系统级测试。本卷规定 kselftest 框架结构、`tools/testing/selftests/` 目录组织、`make kselftest` 入口、各子系统测试集（sched/mm/fs/net/...）、kselftest 与 KUnit 区别、运行环境要求，以及 agentrt-liunx Agent 契约测试的系统级扩展。
...
> **核心约束**: IRON-9 v2 同源且部分代码共享——kselftest 框架与 Linux 6.6 上游保持源码同源，agentrt-liunx 扩展必须以独立子目录形式注入，禁止改写上游 kselftest 框架代码。

---

## 0. 章节定位

本卷是 agentrt-liunx 测试工程 10 主题文档中的第 2 卷，回答"内核系统级测试怎么跑"。它在 01-kunit-framework（白盒单元测试）与 03-kernel-selftests（内核自检）之间形成系统级测试执行层：
...
agentrt-liunx 完整继承 Linux 6.6 内核基线的 kselftest 框架（`tools/testing/selftests/`），不修改任何上游源文件。agentrt-liunx 专属测试以独立 `airymax_*` 子目录形式驻留于 `tools/testing/selftests/`，遵循 IRON-9 v2 同源且部分代码共享原则。
...
flowchart TB
    ...
    D --> F
    G -.->|串行/并行调度| D
    subgraph "agentrt-liunx 扩展层（独立子目录）"
...
**OS-TEST-013**：所有 agentrt-liunx 内核特性必须有对应的 kselftest 子系统测试；特性无 kselftest 覆盖时，PR 评审必须显式标注"kselftest 豁免理由"。
...
**OS-STD-006**：所有 agentrt-liunx 扩展子目录必须有 `config` 文件列出依赖 CONFIG_*；缺失 `config` 导致测试在 CI 上无法启用的，禁止合入。
...
**OS-KER-007**：agentrt-liunx 子目录禁止修改上游 `Makefile`（顶层）；新增子目录通过 `TARGETS += airymax_xxx` 单行追加，并经 CI 静态检查（第 7 层）验证追加合规。

---

## 4. 子系统测试集

Linux 6.6 内核基线在 `tools/testing/selftests/` 下提供 100+ 子系统测试集，覆盖 sched_ext、mm、filesystems、net、bpf、cgroup、livepatch、resctrl、rseq、seccomp、tpm2 等。本节仅以 sched_ext（与 MicroCoreRT 共享调度器扩展机制）为例说明 agentrt-liunx 系统级测试范式。

### 4.1 sched_ext 与 MicroCoreRT 调度系统测试

`tools/testing/selftests/sched_ext/` 测试 sched_class_ext（agentrt-liunx 与 MicroCoreRT 共享的调度器扩展机制），包含 `create_dsq`、`ddsp_bogus_dsq_fail`、`enq_last_no_enq_fails`、`exit`、`hotplug` 等 BPF + 用户态主程序对。MicroCoreRT 调度算法的系统级验证通过 `airymax_microcorert/` 测试集承载：
...
agentrt-liunx 维护 `tools/testing/selftests/airymax_enabled_targets.list`，列出 CI 默认启用的 TARGETS；该列表与 README 第 1.1 节的 L2 系统级测试集对齐。
...
```mermaid
flowchart LR
    subgraph "agentrt-liunx 测试分层（agentrt L1-L7）"
...
**OS-TEST-017**：KUnit 测试与 kselftest 测试不可相互替代；每个 agentrt-liunx 子系统必须同时有 KUnit 白盒测试（覆盖函数）与 kselftest 系统级测试（覆盖系统调用接口）。
...
agentrt-liunx CI 推荐 virtme 跑 kselftest：
...
**OS-TEST-018**：agentrt-liunx CI 必须提供 root 与非 root 两套 kselftest 运行矩阵；非 root 测试集标记在 `airymax_enabled_targets.list` 第 2 列。

**OS-STD-009**：所有 agentrt-liunx 扩展测试在 `settings` 文件中必须声明 `timeout` 字段；CI 默认超时 60 秒，超时测试由 CI 标记为 `KSFT_FAIL`。
...
**OS-TEST-019**：agentrt-liunx 扩展测试优先使用 `kselftest_harness.h`（高层 API）；仅在需要精细控制 TAP 输出格式时使用 `kselftest.h`（低层 API）。

**OS-STD-010**：每个 `TEST_F` 必须有对应 `FIXTURE_SETUP` 与 `FIXTURE_TEARDOWN`；资源在 setup 中获取，在 teardown 中释放，禁止在 `TEST_F` 体中分配而不释放。

---

## 8. agentrt-liunx Agent 契约测试的系统级扩展
...
**OS-TEST-020**：agentrt-liunx Agent SDK 的每个公共 ioctl 必须有 kselftest 系统级测试；契约覆盖正常路径 + `EINVAL`/`ENOMEM`/`EBUSY` 各一例异常路径。

**OS-KER-009**：agentrt-liunx Agent 契约的系统级测试与 KUnit 契约测试必须共享同一份契约规范文档（`docs/agents/contract.md`），二者实现由 CI 第 7 层检查契约一致性。
...
| **IRON-9 v2 同源且部分代码共享** | agentrt-liunx 扩展子目录独立注入，不改上游（OS-KER-007/008） |
...
- **同源**：agentrt-liunx 不修改上游 `tools/testing/selftests/Makefile` 顶层结构；上游测试集与 Linux 6.6 内核基线一一对应。
- **独立**：agentrt-liunx 扩展子目录以 `airymax_*` 前缀命名，独立 `config`/`settings`/`Makefile`，独立 Kconfig 依赖。
- **互操作**：agentrt-liunx 套件与上游套件共享同一 `run_kselftest.sh` 调度器，CI 通过 `--filter` 选择性运行。

**OS-KER-010**：agentrt-liunx 子目录禁止引入对上游测试集源码的依赖（`#include "../sched_ext/..."` 等）；扩展测试自包含，避免上游重构时被破坏。
...
**OS-KER-011**：agentrt L2（系统级）的所有用例必须用 kselftest 实现；与上游 kselftest 共享同一 `tools/testing/selftests/` 目录与 `run_kselftest.sh` 调度器，禁止 agentrt-liunx 自建独立调度器。

**OS-KER-012**：agentrt 与 agentrt-liunx 之间的 kselftest 子目录命名空间必须正交（`airymax_*` vs `agentrt_*` 前缀）；命名冲突由 CI 第 7 层（静态检查）在 PR 阶段检测。
...
- `80-testing/08-agent-contract-testing.md`（agentrt-liunx 专属：Agent 行为契约测试）
...
- agentrt-liunx 扩展子目录的新增/删除必须同步更新本卷第 8 节与 `80-testing/README.md` 的 L2 章节。
- 本卷所有规则编号（OS-KER-XXX/OS-STD-XXX/OS-TEST-XXX）注册于 07 维护者制度的"规则编号注册表"。

### 13.2 待办（1.0.1 版本）

- [ ] 补充 NOLIBC 模式下 agentrt-liunx 测试的编译模板
...
> **文档结束** | agentrt-liunx 测试工程第 2 卷（kselftest 系统级测试）| 0.1.1 P0 优先完成 | 同源 Linux 6.6 内核基线 kselftest
