...
> **文档定位**: AirymaxOS 编码规范的遵循声明、命名规范、C/Rust 风格、日志标准、Doxygen 与安全编码
...
AirymaxOS 8 子仓的全部源代码严格遵循 `docs/Capital_Specifications/coding_standard/` 下的 15 个规范文件。本文件给出 AirymaxOS 专属的命名空间与补充约定，作为 15 个规范文件的前置索引。

| # | 规范文件 | 适用范围 | AirymaxOS 遵循要点 |
|---|---------|---------|-------------------|
...
- **AirymaxOS 专属约定**: 本文件第 2-5 章给出 AirymaxOS 专属的命名空间与风格补充，与 15 个规范文件叠加生效。
...
AirymaxOS 全部公共符号使用统一命名空间前缀，与 agentrt 同源保持兼容：
...
C 代码风格遵循 `C_coding_style_standard.md`，AirymaxOS 补充约定如下。
...
Rust 代码风格遵循 `Rust_coding_style_standard.md`，AirymaxOS 补充约定如下。
...
日志标准遵循 `Log_standard.md`，AirymaxOS 补充约定如下。
 ...
 * @brief AirymaxOS IPC 128 字节定长消息头定义
 *
...
安全编码遵循 `Security_design_standard.md` + `C_Cpp_secure_coding_standard.md` + `Rust_secure_coding_standard.md`，AirymaxOS 补充约定如下。
