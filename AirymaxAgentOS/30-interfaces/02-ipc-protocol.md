...
> **文档定位**: AirymaxOS 进程间通信协议的 128B 消息头、5 种 payload、io_uring 零拷贝与同源映射
...
AirymaxOS IPC 协议与 agentrt AgentsIPC 同源，保留 128 字节定长消息头设计，并在底层升级为基于 io_uring 的零拷贝实现。设计原则：

1. **定长消息头**: 128 字节定长，64 字节对齐，便于 cache line 对齐与零拷贝 page flipping。
2. **同源兼容**: 消息头字段布局与 agentrt AgentsIPC 保持兼容，agentrt 在 AirymaxOS 上运行无需协议转换。
...
/**
 * @brief AirymaxOS IPC 128 字节定长消息头
 *
...
AirymaxOS IPC 基于 Linux 6.6 内核基线的 io_uring 子系统实现零拷贝消息传递。
                                     ...
                                     const void *payload, size_t payload_len)
{
    ...
    /* 提交 IORING_OP_IPC_SEND（AirymaxOS 注册的固定 OP） */
...
AirymaxOS 在 io_uring 注册以下 IPC 专用 OP（详见 [20-modules/01-kernel.md](../20-modules/01-kernel.md) 第 4.2 节）：
...
AirymaxOS IPC 与 agentrt AgentsIPC 同源，保留 128B 定长消息头布局兼容。

| 维度 | agentrt AgentsIPC | AirymaxOS IPC | 同源关系 |
|------|------------------|---------------|---------|
...
| 底层传输 | 用户态消息队列 | io_uring 零拷贝 | AirymaxOS 升级 |
| payload 协议 | 自定义 | 5 种（REQUEST/RESPONSE/EVENT/STREAM/CONTROL） | AirymaxOS 扩展 |
| capability | 应用权限模型 | seL4 风格 capability | AirymaxOS 升级 |

**同源红利**: agentrt 在 AirymaxOS 上运行时，IPC 消息头布局完全兼容，无需协议转换层，天然契合。

**独立性**: AirymaxOS IPC 为 OS 级实现（基于 io_uring + 内核固定 OP），agentrt AgentsIPC 为应用级实现（用户态消息队列），二者在分层上独立。遵循 IRON-9"同源且部分代码共享"原则。
