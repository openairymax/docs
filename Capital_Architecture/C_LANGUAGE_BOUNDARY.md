...
C 语言在 AgentRT 中承载系统最核心、最底层的基础设施。C 语言边界内的模块构成系统的"骨骼"与"神经"，所有其他语言通过 FFI 层与 C 核心交互。
...
Agent 运行时执行引擎是 AgentRT 的"心脏"，全部由 C 语言实现：

| 模块 | 说明 |
|------|------|
| **CoreKern** | 微内核调度器，负责系统级调度、任务路由、上下文管理。所有服务请求的中央仲裁者。 |
| **CoreLoopThree** | 核心认知循环，实现 Cognition → Planning → Action 三阶段流水线。 |
| **Cupola** | 服务胶囊框架，可插拔的服务加载与生命周期管理。 |
...
Rust 在 AgentRT 中负责面向用户和开发者的交互层，利用其内存安全特性提供可靠的开发体验：
...
Python 在 AgentRT 中负责编排层和实验层，利用其生态丰富性和开发效率：
...
Go 在 AgentRT 中负责网络密集型服务，利用其并发模型和网络库优势：
...
TypeScript 在 AgentRT 中负责 Web 前端和前端集成 SDK：
...
- **@agentrt/sdk**：npm 包，提供类型安全的 AgentRT API 调用。
